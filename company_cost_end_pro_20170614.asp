...
'		dbconn.execute(sql)
		...
		sql = "update pay_alba_cost set cost_center = '상주직접비' where (cost_company <> '공통' and cost_company <> '기타' and cost_company <> '본사' and cost_company <> '케이원정보통신' and cost_company <> '') and (rever_yymm ='"&end_month&"')"
		dbconn.execute(sql)

		sql = "select company,org_name from pay_alba_cost where (cost_company = '공통' or cost_company = '기타' or cost_company = '본사' or cost_company = '케이원정보통신' or cost_company = '') and (rever_yymm ='"&end_month&"') group by company,org_name"
		rs.Open sql, Dbconn, 1
		do until rs.eof
