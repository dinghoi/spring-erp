...
' serial no삭제		
	...
	if filenm1 <> "" then
		...
		if filenm1 <> "" then 
			...
			filename1 = "NW입고" + cstr(buy_no) + file_name1 + "." + fileType1
...
' serial_no 저장					 
		         ...
		         if filenm <> "" then 
			         ...
			         filename = "NW입고_serial" + cstr(buy_no) + file_name + "." + fileType
...
'                    response.End()
                    ...
                    if rowcount > -1 then
		               for si=0 to rowcount
				         ...
				         sql = "select * from met_goods_serial where goods_code = '"&code_tab(i)&"' and serial_no = '"&serial_seq&"' and serial_seq = '"&serial_no&"'"
		                 set Rs_seri=dbconn.execute(sql)				
		                 if Rs_seri.eof or Rs_seri.bof then
			                   sql="insert into met_goods_serial (goods_code,serial_no,serial_seq,goods_gubun,goods_name,goods_standard,in_date,in_order_no,in_order_seq,unit_wonga,reg_date,reg_user) values ('"&code_tab(i)&"','"&serial_no&"','"&serial_seq&"','"&goods_gubun(i)&"','"&goods_name(i)&"','"&goods_standard(i)&"','"&stin_in_date&"','"&buy_no&"','"&buy_seq&"','"&buy_cost(i)&"',now(),'"&emp_user&"')"
		                       dbconn.execute(sql)
		                 end if
		               next
	                end if	
...
'입고품목 등록
			     goods_seq = right(("00" + cstr(i)),2)
		  	     sql="insert into met_stin_goods (stin_date,stin_order_no,stin_order_seq,stin_goods_code,stin_goods_seq,stin_goods_type,stin_goods_gubun,stin_goods_name,stin_standard,stin_unit_cost,stin_qty,stin_amt,stin_id,stin_type,stin_stock_company,stin_stock_code,stin_stock_name,reg_date,reg_user,excel_att_file) values ('"&stin_in_date&"','"&stin_order_no&"','"&stin_order_seq&"','"&code_tab(i)&"','"&goods_seq&"','"&goods_type(i)&"','"&goods_gubun(i)&"','"&goods_name(i)&"','"&goods_standard(i)&"','"&buy_cost(i)&"','"&qty_tab(i)&"','"&buy_amt(i)&"','"&stin_id&"','"&stin_type&"','"&stin_stock_company&"','"&stin_stock_code&"','"&stin_stock_name&"',now(),'"&user_name&"','"&filename&"')"
			     dbconn.execute(sql)
