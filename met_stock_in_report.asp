	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="met_stock_in_report.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>회사 검색</dt>
			   ...
			   %>
				      <tr>
                        ...
                        <td>
        <%  if rs("stin_id") = "대기전표" or rs("stin_id") = "수주전표" then  %>   
                        <%=rs("stin_id")%>&nbsp;  
        <%       else  %>                                                  
                        <a href="#" onClick="pop_Window('met_buy_detail_list.asp?buy_no=<%=stin_buy_no%>&buy_date=<%=stin_buy_date%>&buy_seq=<%=stin_buy_seq%>&u_type=<%=""%>','met_buy_detail_pop','scrollbars=yes,width=930,height=650')"><%=stin_buy_no%>&nbsp;<%=stin_buy_seq%></a>
        <%  end if %>                                        
                        </td>
                        <td><%=order_org_name%></td>
                        <td>
                        <a href="#" onClick="pop_Window('met_buy_order_detail.asp?order_no=<%=stin_order_no%>&order_date=<%=stin_order_date%>&order_seq=<%=stin_order_seq%>&u_type=<%=""%>','met_buy_order_detail_pop','scrollbars=yes,width=930,height=650')"><%=stin_order_no%>&nbsp;<%=stin_order_seq%></a>
                        </td>
                        <td><%=stin_order_date%>&nbsp;</td>
                        <td><%=order_emp_name%>&nbsp;</td>
                        <td><%=rs("stin_trade_name")%>&nbsp;</td>
                        <td>
                        <a href="#" onClick="pop_Window('met_stock_in_detail.asp?stin_in_date=<%=stin_in_date%>&order_no=<%=stin_order_no%>&order_seq=<%=stin_order_seq%>&u_type=<%=""%>','met_stin_detail_pop','scrollbars=yes,width=930,height=650')"><%=rs("stin_in_date")%></a>
                        </td>
			  ...
			  %>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td width="20%">
					<div class="btnCenter">
                    <a href="met_stock_in_excel.asp?view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>" class="btnType04">엑셀다운로드</a>
					</div>                  
                  	</td>
                    <td>
                    <div id="paging">
                        <a href="met_stock_in_report.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="met_stock_in_report.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
   	        <% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="met_stock_in_report.asp?page=<%=i%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
   	        <% if 	intend < total_page then %>
                        <a href="met_stock_in_report.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[다음]</a> <a href="met_stock_in_report.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
                    ...
                    <td width="20%">
					<div class="btnCenter">
                        <a href="#" onClick="pop_Window('met_stock_in_print.asp?view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>','met_stock_in_print_pop','scrollbars=yes,width=1250,height=600')" class="btnType04">입고현황 출력</a>
					</div>                  
                    </td>
			      </tr>
				  </table>
