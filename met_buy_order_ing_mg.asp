...
be_pg = "met_buy_order_ing_mg.asp"
...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function delcheck(order_no,order_seq,order_date) 
				{
				a=confirm('정말 삭제하시겠습니까?')
				if (a==true) {
					document.frm.action = "met_buy_order_del_ok.asp?order_no="+order_no+'&order_seq='+order_seq+'&order_date='+order_date;
					document.frm.submit();
				}
				return false;
				}
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="met_buy_order_ing_mg.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>회사 검색</dt>
        ...
        <%  end if %>                                                             
                        ...
                        <td><%=buy_ing_gubun%>&nbsp;</td>
                        <td>
                        <a href="#" onClick="pop_Window('met_buy_order_print.asp?order_no=<%=rs("order_no")%>&order_date=<%=rs("order_date")%>&order_seq=<%=rs("order_seq")%>','met_buy_order_print_pop','scrollbars=yes,width=1250,height=750')">출력</a>
                        </td>
        <%  if order_ing = "1" or order_ing = "2" and user_id = "900002" then  %> 
                        <td>
                        <a href="#" onClick="pop_Window('met_buy_order_modify.asp?order_no=<%=rs("order_no")%>&order_date=<%=rs("order_date")%>&order_seq=<%=rs("order_seq")%>&u_type=<%="U"%>','met_buy_order_modify_pop','scrollbars=yes,width=1230,height=650')">수정</a>
                        </td>
        <%     else  %>
                        <td>-</td>
        <%  end if %>                                 
        ...
        %>
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td width="20%">
					<div class="btnCenter">
                    <a href="met_buy_order_detail_excel.asp?view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>" class="btnType04">엑셀다운로드</a>
					</div>                  
                  	</td>
				    <td>
                    <div id="paging">
                        <a href="met_buy_order_ing_mg.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="met_buy_order_ing_mg.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
   	        <% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="met_buy_order_ing_mg.asp?page=<%=i%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
   	        <% if 	intend < total_page then %>
                        <a href="met_buy_order_ing_mg.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[다음]</a> <a href="met_buy_order_ing_mg.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
