...
title_line = cstr(inc_yyyy) + "년 " + pmg_id_name + "현황(개인)"
			...
			<div id="container">
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td>
                    <div id="paging">
                        <a href = "insa_pay_bonus_report_person.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&inc_yyyy=<%=inc_yyyy%>&owner_view=<%=owner_view%>&condi=<%=condi%>&in_pmg_id=<%=in_pmg_id%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="insa_pay_bonus_report_person.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&inc_yyyy=<%=inc_yyyy%>&owner_view=<%=owner_view%>&condi=<%=condi%>&in_pmg_id=<%=in_pmg_id%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
