...
' 조건별 조회.........
' 포지션별
posi_sql = " and (reg_id = '"&user_id&"' or emp_no = '"&user_id&"')"
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="outside_cost_mg.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>조건 검색</dt>
					...
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						%>
							...
							<%   if (rs("reg_id") = user_id) or (rs("emp_no") = user_id) or cost_grade = "0" or position ="사업부장" or position = "본부장"  then	%>
                                <a href="#" onClick="pop_Window('outside_cost_add.asp?slip_date=<%=rs("slip_date")%>&slip_seq=<%=rs("slip_seq")%>&u_type=<%="U"%>','outside_cost_add_pop','scrollbars=yes,width=800,height=280')">수정</a>
							<%     else	%>
								불가
                            <%	 end if	%>
							<%  else	%>
						...
						%>
							<tr bgcolor="#FFE8E8">
								<td class="first" colspan="6">합 계</td>
							  	<td class="right"><%=formatnumber(price_sum,0)%></td>
							  	<td class="right"><%=formatnumber(cost_sum,0)%></td>
							  	<td class="right"><%=formatnumber(cost_vat_sum,0)%></td>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td width="30%">
					<div class="btnRight">
					<a href="#" onClick="pop_Window('outside_cost_batch.asp?slip_month=<%=slip_month%>&slip_gubun=<%=slip_gubun%>','outside_cost_batch_pop','scrollbars=yes,width=800,height=550')" class="btnType04">외주비 전월 이월처리</a>
					<a href="#" onClick="pop_Window('outside_cost_add.asp','outside_cost_add_pop','scrollbars=yes,width=800,height=280')" class="btnType04">외주비 세금계산서 등록</a>
					</div>                  
                    </td>
			      </tr>
				  </table>
