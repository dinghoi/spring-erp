...
where_sql = " WHERE (ask_company_process = '0') and (ask_company = '"+view_condi+"') and (Cast(ask_reg_date as date) >= '" + from_date + "' AND Cast(ask_reg_date as date) <= '"+to_date+"')"
	...
	<body oncontextmenu="return false" ondragstart="return false" onselectstart="return false">
		<div id="wrap">
			...
			<div id="container">
				...
				<form action="insa_welfare_ask_mg.asp" method="post" name="frm">
                <fieldset class="srch">
					<legend>조회영역</legend>
					<dl>
						<dt>◈ 검색◈</dt>
					...
					<tbody>
						...
						%>
							<tr>
                                ...
                                <td><a href="#" onClick="pop_Window('insa_sawo_giveadd.asp?sawo_empno=<%=rs("ask_empno")%>&emp_name=<%=rs("ask_emp_name")%>&ask_seq=<%=rs("ask_seq")%>&ask_date=<%=rs("ask_date")%>&give_ask_process=<%=give_ask_process%>&u_type=<%=""%>','insa_sawo_giveadd_pop','scrollbars=yes,width=750,height=500')">지급</a>&nbsp;</td>
 							</tr>
						<%
							rs.movenext()
						loop
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td>
                    <div id="paging">
                        <a href="insa_welfare_ask_mg.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="insa_welfare_ask_mg.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
                  	<% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="insa_welfare_ask_mg.asp?page=<%=i%>&view_condi=<%=view_condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
                  	<% if 	intend < total_page then %>
                        <a href="insa_welfare_ask_mg.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[다음]</a> <a href="insa_welfare_ask_mg.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
