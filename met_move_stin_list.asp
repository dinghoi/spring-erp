...
if view_condi = "전체" then
   if goods_type = "전체" then
          where_sql = " WHERE (mvin_id = '"&mvin_id&"') and (mvin_in_date >= '"+from_date+"' and mvin_in_date <= '"+to_date+"')" 
	  else
	      where_sql = " WHERE (mvin_id = '"&mvin_id&"') and (mvin_goods_type = '"&goods_type&"') and (mvin_in_date >= '"+from_date+"' and mvin_in_date <= '"+to_date+"')" 
   end if
 else  
   if goods_type = "전체" then
          where_sql = " WHERE (mvin_id = '"&mvin_id&"') and (mvin_stock_company = '"&view_condi&"') and (mvin_in_date >= '"+from_date+"' and mvin_in_date <= '"+to_date+"')"
	  else
	      where_sql = " WHERE (mvin_id = '"&mvin_id&"') and (mvin_goods_type = '"&goods_type&"') and (mvin_stock_company = '"&view_condi&"') and (mvin_in_date >= '"+from_date+"' and mvin_in_date <= '"+to_date+"')"
   end if
end if   
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="met_move_stin_list.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>회사 검색</dt>
				...
				<div class="gView">
                  ...
                  <%
						...
						do until rs.eof
						   ...
						   if Rs_reg.eof or Rs_reg.bof then
								...
								chulgo_memo = Rs_reg("chulgo_memo")
						   end if
						   Rs_reg.close()
				  ...
				  %>
				      <tr>
                        ...
                        <td><%=chulgo_type%>&nbsp;</td>
                        <td><%=chulgo_stock_name%>(<%=chulgo_stock%>)&nbsp;</td>
                        <td><%=chulgo_emp_name%>(<%=chulgo_emp_no%>)&nbsp;</td>
                        <td><%=bg_goods_name%>&nbsp;외</td>
                        <td><%=rs("mvin_in_date")%>&nbsp;</td>
                        <td>
						<a href="#" onClick="pop_Window('met_move_stin_detail.asp?mvin_in_date=<%=rs("mvin_in_date")%>&mvin_in_stock=<%=rs("mvin_in_stock")%>&mvin_in_seq=<%=rs("mvin_in_seq")%>&u_type=<%=""%>','met_move_stin_detail_pop','scrollbars=yes,width=930,height=650')"><%=mvin_no%>&nbsp;<%=rs("mvin_in_stock")%><%=rs("mvin_in_seq")%></a>
                        </td>
                        <td><%=rs("mvin_stock_name")%>&nbsp;</td>
                        <td><%=rs("mvin_emp_name")%>&nbsp;</td>
                        <td class="left"><%=chulgo_memo%>&nbsp;</td>
			          </tr>
				<%
							rs.movenext()
							seq = seq -1
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td>
                    <div id="paging">
                        <a href="met_move_stin_list.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&stock=<%=stock%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="met_move_stin_list.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&stock=<%=stock%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
   	        <% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="met_move_stin_list.asp?page=<%=i%>&view_condi=<%=view_condi%>&stock=<%=stock%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
   	        <% if 	intend < total_page then %>
                        <a href="met_move_stin_list.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&stock=<%=stock%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[다음]</a> <a href="met_move_stin_list.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&stock=<%=stock%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
