...
if view_condi = "전체" then
   Sql = "SELECT * FROM OVERTIME "&_
         " WHERE work_date >= '"+from_date+"' "&_
         "   AND work_date <= '"+to_date+"' "&_
         "   AND cancel_yn = 'N' "&_
         " ORDER BY emp_company, team, org_name, mg_ce_id, work_date ASC"
else
   Sql = "SELECT * FROM OVERTIME "&_
         " WHERE emp_company = '"+view_condi+"' "&_
  ...
  if isnull(rs("mg_ce_id")) or rs("mg_ce_id") = "" then
    ...
    Set rs_emp = DbConn.Execute(SQL)
	  ...
	  if not Rs_emp.eof then
			...
			view_condi = rs_emp("emp_company")
	  end if
	  
	  rs_emp.close()

    Sql = "SELECT * FROM pay_month_give "&_
          " WHERE (pmg_yymm = '"+pmg_yymm+"' ) "&_
          "   AND (pmg_id = '"+pmg_id+"') "&_
          "   AND (pmg_emp_no = '"+bi_mg_ce_id+"') "&_
          "   AND (pmg_company = '"+view_condi+"')"
      ...
      pmg_give_tot = pmg_tax_yes + pmg_tax_no
			...
			sql = "UPDATE  pay_month_give "&_
			      "   SET  pmg_job_support='"&sum_overtime_cost&"' "&_
			      "      , pmg_tax_yes='"&pmg_tax_yes&"' "&_
			      "      , pmg_tax_no='"&pmg_tax_no&"' "&_
			      "      , pmg_give_total='"&pmg_give_tot&"' "&_
			      " WHERE  (pmg_yymm = '"+pmg_yymm+"' ) "&_
			      "   AND  (pmg_id = '"+pmg_id+"') "&_
			      "   AND  (pmg_emp_no = '"+bi_mg_ce_id+"') "&_
			      "   AND  (pmg_company = '"+view_condi+"')"
