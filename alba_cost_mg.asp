...
if ck_sw = "y" then
	...
	srchOrgName = Request.form("srchOrgName")
end if

if rever_yymm = "" then
	'rever_yymm = mid(now(),1,4) + mid(now(),6,2)
	rever_yymm = mid(DateAdd("m", -1, now()) ,1 ,4) + mid(DateAdd("m", -1, now()), 6 ,2)
end if
...
'//사업부 리스트 가져오기
Sql = "SELECT DISTINCT org_name FROM pay_alba_cost WHERE (rever_yymm = '"+rever_yymm+"' )"&posi_sql&" ORDER BY org_name ASC "
Rs_org.Open Sql, Dbconn, 1


Sql = "select count(*) from pay_alba_cost where (rever_yymm = '"+rever_yymm+"' )"&org_sql&posi_sql
...
Sql = "select * from pay_alba_cost "
Sql = Sql & " Where(rever_yymm = '"+rever_yymm+"' )"&org_sql&posi_sql
	...
	<body>
		<div id="wrap">
			...
			<div id="container">
				...
				<form action="alba_cost_mg.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>
						<dt>조건 검색</dt>
                        <dd>
                            <p>
								...
								<label>
								<strong>사업부 : </strong>
								<select name="srchOrgName" id="srchOrgName">
								<option value="">전체</option>
								<%
								If Not(Rs_org.bof Or Rs_org.eof) Then
									Do Until Rs_org.eof
										If srchOrgName = Rs_org("org_name") Then
											isSelected = " selected=""selected"""
										Else
											isSelected = ""
										End If
										Response.write "<option value=""" & Rs_org("org_name")& """" & isSelected & ">" & Rs_org("org_name") & "</option>"
				...
				<div class="gView">
					<table cellpadding="0" cellspacing="0" class="tableList">
                        ...
                        %>
                                ...
                                <td>
								<%
								If rs("bonbu") = bonbu Or cost_grade = "0" Then
								%>
								<a href="#" onClick="pop_Window('alba_cost_add.asp?draft_no=<%=rs("draft_no")%>&rever_yymm=<%=rever_yymm%>&give_date=<%=rs("give_date")%>&u_type=<%="U"%>','insa_pay_alba_add_pop','scrollbars=yes,width=800,height=520')">수정</a>
								<%
								End If
								%>
								</td>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
                  	<td width="15%">
					<div class="btnCenter">
                    <a href="insa_excel_albacost.asp?rever_yymm=<%=rever_yymm%>&srchOrgName=<%=srchOrgName%>" class="btnType04">엑셀다운로드</a>
					</div>
                  	</td>
				    <td>
                    <div id="paging">
                        <a href = "alba_cost_mg.asp?page=<%=first_page%>&rever_yymm=<%=rever_yymm%>&srchOrgName=<%=srchOrgName%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="alba_cost_mg.asp?page=<%=intstart -1%>&rever_yymm=<%=rever_yymm%>&srchOrgName=<%=srchOrgName%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
           	<% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="alba_cost_mg.asp?page=<%=i%>&rever_yymm=<%=rever_yymm%>&srchOrgName=<%=srchOrgName%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
           	<% if 	intend < total_page then %>
                        <a href="alba_cost_mg.asp?page=<%=intend+1%>&rever_yymm=<%=rever_yymm%>&srchOrgName=<%=srchOrgName%>&ck_sw=<%="y"%>">[다음]</a> <a href="alba_cost_mg.asp?page=<%=total_page%>&rever_yymm=<%=rever_yymm%>&srchOrgName=<%=srchOrgName%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
