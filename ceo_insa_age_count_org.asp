...
be_pg = "ceo_insa_age_count_org.asp"
...
do until rs.eof 
   ...
   if isnull(rs("emp_birthday")) or rs("emp_birthday") = "1900-01-01" then
           emp_birthday = ""
       else 
   ...
   for i = 1 to k_org
	...
	if emp_birthday = "" then
	        j = 0
	   else 	
	        year_cnt = datediff("yyyy", emp_birthday, curr_date)
            mon_cnt = datediff("m", emp_birthday, curr_date)
            day_cnt = datediff("d", emp_birthday, curr_date) 
	...
	if year_cnt >= 20 and year_cnt < 30 then 
	           j = 1
	   elseif  year_cnt >= 30 and year_cnt < 40 then 
	           j = 2
	       elseif  year_cnt >= 40 and year_cnt < 50 then 
		           j = 3    
	           elseif  year_cnt >= 50 and year_cnt < 60 then 
			           j = 4  
			       elseif  year_cnt >= 60 then 
				           j = 5 
	 end if
	
    if j <> 0 then
       if emp_sex = "남" then		
	           age_m_cnt(k,j) = age_m_cnt(k,j) + 1
	           sum_m_cnt(j) = sum_m_cnt(j) + 1
	      else 
	           age_w_cnt(k,j) = age_w_cnt(k,j) + 1
...
title_line = ""+ view_condi +" - 연령별 인원분포 "
	...
	<body oncontextmenu="return false" ondragstart="return false" onselectstart="return false">
		<div id="wrap">			
			...
			<div id="container">
                ...
                <form action="ceo_insa_age_count_org.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>조건 검색</dt>
					...
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						<thead>
							<tr>
                                ...
                                <th colspan="2" scope="col" style=" border-bottom:1px solid #e3e3e3;">20대</th>
                                <th colspan="2" scope="col" style=" border-bottom:1px solid #e3e3e3;">30대</th>
                                <th colspan="2" scope="col" style=" border-bottom:1px solid #e3e3e3;">40대</th>
                                <th colspan="2" scope="col" style=" border-bottom:1px solid #e3e3e3;">50대</th>
                                <th colspan="2" scope="col" style=" border-bottom:1px solid #e3e3e3;">60대이상</th>
						...
						%>	
                            <tr>
								...
								hap_w_cnt = 0
								for j = 1 to 5 
								    hap_m_cnt = hap_m_cnt + age_m_cnt(i,j)
									hap_w_cnt = hap_w_cnt + age_w_cnt(i,j)
								next
								...
								%>
                                    <td>
                                    <a href="#" onClick="pop_Window('insa_age_count_view.asp?emp_company=<%=view_condi%>&condi=<%=condi%>&emp_bonbu=<%=com_tab(i)%>&age=<%=j%>&sex=<%=1%>','insa_age_count_pop','scrollbars=yes,width=890,height=600')"><%=formatnumber(age_m_cnt(i,j),0)%></a>
                                    </td>
                                    <td>
                                    <a href="#" onClick="pop_Window('insa_age_count_view.asp?emp_company=<%=view_condi%>&condi=<%=condi%>&emp_bonbu=<%=com_tab(i)%>&age=<%=j%>&sex=<%=2%>','insa_age_count_pop','scrollbars=yes,width=890,height=600')"><%=formatnumber(age_w_cnt(i,j),0)%></a>
                                    </td>
								<%
								next
								%>
