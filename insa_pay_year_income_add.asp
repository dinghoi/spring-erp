...
'if u_type = "U" then
 ...
 if not rs.eof then
	...
	incom_retirement_bank = rs("incom_retirement_bank") 
...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function chkfrm() {
				if(document.frm.incom_total_pay.value =="") {
					alert('총 연봉을 입력하세요');
					frm.incom_total_pay.focus();
					return false;}
				if(document.frm.incom_base_pay.value =="") {
					alert('기본급을 입력하세요');
					frm.incom_base_pay.focus();
					return false;}
				if(document.frm.incom_meals_pay =="") {
					alert('식대를 입력하세요');
					frm.incom_meals_pay.focus();
					return false;}
				if(document.frm.incom_severance_pay.value =="") {
					alert('퇴직금을 입력하세요');
					frm.incom_severance_pay.focus();
					return false;}
				if(document.frm.incom_month_amount.value =="") {
					alert('과세평균소득월액을 입력하세요');
					frm.incom_month_amount.focus();
					return false;}
			...
			function meals_chk(txtObj){
				total_pay = parseInt(document.frm.incom_total_pay.value.replace(/,/g,""));
				acpt_ot_pay = parseInt(document.frm.incom_overtime_pay.value.replace(/,/g,""));
				acpt_meals_pay = parseInt(document.frm.incom_meals_pay.value.replace(/,/g,""));
			
				mon13_pay = parseInt(total_pay / 13);

				meals_pay = acpt_meals_pay;
				ot_pay = acpt_ot_pay
				base_pay = parseInt(mon13_pay - meals_pay - ot_pay);
				mon_amt = base_pay + ot_pay;
				se_pay = parseInt(mon13_pay);
	...
	<body>
			<div id="container">
				...
				<form action="insa_pay_year_income_save.asp" method="post" name="frm">
				<div class="gView">
				  <table cellpadding="0" cellspacing="0" class="tableWrite">
                    ...
                    <tr>
                      ...
                      <input name="incom_total_pay" type="text" id="incom_total_pay" style="width:80px;text-align:right" value="<%=formatnumber(incom_total_pay,0)%>" onKeyUp="num_chk(this);">
                      <th>수습기간<br>급여율</th>
                      <td class="left">
                      <input name="incom_first3_percent" type="text" id="incom_first3_percent" style="width:60px;text-align:right" value="<%=formatnumber(incom_first3_percent,0)%>" onKeyUp="checklength(this,3)">%</td>
                    </tr>
                    <tr>
                      <th>기본급</th>
                      <td class="left">
                      <input name="incom_base_pay" type="text" id="incom_base_pay" style="width:80px;text-align:right" value="<%=formatnumber(incom_base_pay,0)%>" readonly="true"></td>
                      <th>연장근로<br>수당</th>
                      <td class="left">
                      <input name="incom_overtime_pay" type="text" id="incom_overtime_pay" style="width:80px;text-align:right" value="<%=formatnumber(incom_overtime_pay,0)%>" onKeyUp="meals_chk(this);"></td>
                      <th>식대</th>
                      <td class="left">
                      <input name="incom_meals_pay" type="text" id="incom_meals_pay" style="width:80px;text-align:right" value="<%=formatnumber(incom_meals_pay,0)%>" onKeyUp="meals_chk(this);"></td>
                      <th>퇴직금</th>
                      <td class="left">
                      <input name="incom_severance_pay" type="text" id="incom_severance_pay" style="width:80px;text-align:right" value="<%=formatnumber(incom_severance_pay,0)%>" readonly="true"></td>
			    	</tr>
                    <tr>
                      <th style="background:#F5FFFA">과세평균<br>소득월액</th>
                      <td colspan="3" class="left">
                      <input name="incom_month_amount" type="text" id="incom_month_amount" style="width:80px;text-align:right" value="<%=formatnumber(incom_month_amount,0)%>" onKeyUp="month_chk(this);"></td>
                      <th style="background:#F5FFFA">퇴직연금<br>가입은행</th>
                      <td colspan="3" class="left">
					<%
					  Sql="select * from emp_etc_code where emp_etc_type = '52' order by emp_etc_code asc"
					  Rs_etc.Open Sql, Dbconn, 1
					%>
					  <select name="incom_retirement_bank" id="incom_retirement_bank" style="width:120px">
                         <option value="" <% if bank_name = "" then %>selected<% end if %>>선택</option>
                			  <% 
								do until rs_etc.eof 
			  				  %>
                					<option value='<%=rs_etc("emp_etc_name")%>' <%If incom_retirement_bank = rs_etc("emp_etc_name") then %>selected<% end if %>><%=rs_etc("emp_etc_name")%></option>
                			  <%
									rs_etc.movenext()  
								loop 
								rs_etc.Close()
                    ...
                    <tr>
                      ...
                      <td class="left">
                      <input name="incom_nps_amount" type="text" id="incom_nps_amount" style="width:80px;text-align:right" value="<%=formatnumber(incom_nps_amount,0)%>" onKeyUp="nps_chk(this);"></td>
                      ...
                      <th style="background:#F5FFFA">건강보험<br>표준월액</th>
                      <td class="left">
                      <input name="incom_nhis_amount" type="text" id="incom_nhis_amount" style="width:80px;text-align:right" value="<%=formatnumber(incom_nhis_amount,0)%>" onKeyUp="nhis_chk(this);"></td>
                      <th style="background:#F5FFFA">건강보험</th>
                      <td class="left">
