...
Set rsCompanyAs = DBConn.Execute(objBuilder.ToString())
objBuilder.Clear()

If Not rsCompanyAs.EOF Then
	arrCompanyAs = rsCompanyAs.getRows()
End If
rsCompanyAs.Close() : Set rsCompanyAs = Nothing

If IsArray(arrCompanyAs) Then
	For i = LBound(arrCompanyAs) To UBound(arrCompanyAs, 2)
		as_company = arrCompanyAs(0, i)
		as_bonbu = arrCompanyAs(1, i)
		as_cnt = arrCompanyAs(2, i)
		as_cost = arrCompanyAs(3, i)
		...
		objBuilder.Append "'"&end_month&"', '"&as_company&"', '"&as_bonbu&"', '"&as_cnt&"', 0, 0, 0, "
		objBuilder.Append "'"&as_cost&"', '"&user_id&"', '"&user_name&"', NOW());"
...
objBuilder.Append "SELECT COUNT(*) AS tot_cnt "
objBuilder.Append "FROM as_acpt_end "
...
Set rsRemoteCnt = DBConn.Execute(objBuilder.ToString())
objBuilder.Clear()

If Not rsRemoteCnt.EOF Then
	arrRemoteCnt = rsRemoteCnt.getRows()
End If
rsRemoteCnt.Close() : Set rsRemoteCnt = Nothing

If IsArray(arrRemoteCnt) Then
	For i = LBound(arrRemoteCnt) To UBound(arrRemoteCnt, 2)
		re_company = arrRemoteCnt(0, i)
		re_cnt = arrRemoteCnt(1, i)

		'sql = "select saupbu from trade where trade_name = '"&rs("company")&"'"
		objBuilder.Append "SELECT saupbu "
		objBuilder.Append "FROM trade "
		objBuilder.Append "WHERE trade_name = '"&re_company&"' "
		...
		rsRemoteTrade.Close() : Set rsRemoteTrade = Nothing

		charge_per = CLng(re_cnt) / won_cnt * won_per / 100
		...
		objBuilder.Append "'"&end_month&"', '"&re_company&"', '"&saupbu&"',"
		objBuilder.Append "'"&re_cnt&"', '"&charge_per&"', "&cost_amt&","
...
Set rsNoRemoteCnt = DBConn.Execute(objBuilder.ToString())
objBuilder.Clear()

If Not rsNoRemoteCnt.EOF Then
	arrNoRemote = rsNoRemoteCnt.getRows()
End If
rsNoRemoteCnt.Close() : Set rsNoRemoteCnt = Nothing

If IsArray(arrNoRemote) Then
	For i = LBound(arrNoRemote) To UBound(arrNoRemote, 2)
		n_company = arrNoRemote(0, i)
		n_cnt = arrNoRemote(1, i)

		'sql = "select saupbu from trade where trade_name = '"&rs("company")&"'"
		objBuilder.Append "SELECT saupbu "
		objBuilder.Append "FROM trade "
		objBuilder.Append "WHERE trade_name = '"&n_company&"' "
		...
		End If
		rsNoRemoteTrade.Close() : Set rsNoRemoteTrade = Nothing
		...
		objBuilder.Append "	AND as_company = '"&n_company&"' "
		...
		If rsCompAsEtc.EOF Or rsCompAsEtc.BOF Then
			...
			charge_per = CLng(n_cnt) / bang_cnt
			cost_amt = Int(charge_per * tot_cost)
			...
			objBuilder.Append "reg_id, reg_name, reg_date)VALUES("
			objBuilder.Append "'"&end_month&"', '"&n_company&"', '"&trade_bonbu&"', "
			objBuilder.Append "'"&n_cnt&"', '"&charge_per&"', "&cost_amt&", "
			objBuilder.Append "'"&user_id&"', '"&user_name&"', NOW()) "
		Else
			charge_per = CLng(n_cnt) / bang_cnt * bang_per / 100 + rsCompAsEtc("charge_per")
			cost_amt = Int(charge_per * tot_cost)
			...
			objBuilder.Append "	visit_cnt='"&n_cnt&"', "
			objBuilder.Append "	charge_per='"&charge_per&"', "
			objBuilder.Append "	cost_amt="&cost_amt&" "
			objBuilder.Append "WHERE as_company='"&n_company&"' "
			objBuilder.Append "	AND as_month = '"&end_month&"' "
		End If
		DBConn.Execute(objBuilder.ToString())
		objBuilder.Clear()
		rsCompAsEtc.Close() : Set rsCompAsEtc = Nothing
	Next
End If
%>