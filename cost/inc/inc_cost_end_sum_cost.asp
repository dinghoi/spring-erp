...
objBuilder.Append "SELECT glct.slip_date, glct.slip_seq "
objBuilder.Append "FROM general_cost AS glct "
objBuilder.Append "INNER JOIN emp_master_month AS emmt ON glct.emp_no = emmt.emp_no "
...
objBuilder.Append "WHERE (glct.slip_date >= '"&from_date&"' AND glct.slip_date <= '"&to_date&"') "
objBuilder.Append "	AND eomt.org_bonbu = '"&deptName&"' "

Set rsGeneralEnd = Server.CreateObject("ADODB.RecordSet")
rsGeneralEnd.Open objBuilder.ToString(), DBConn, 1
objBuilder.Clear()

Do Until rsGeneralEnd.EOF
	objBuilder.Append "UPDATE general_cost SET "
	objBuilder.Append "	end_yn = 'Y' "
	objBuilder.Append "WHERE slip_date = '"&rsGeneralEnd("slip_date")&"' "
	objBuilder.Append "	AND slip_seq = '"&rsGeneralEnd("slip_seq")&"' "

	DBConn.Execute(objBuilder.ToString())
	objBuilder.Clear()

	rsGeneralEnd.MoveNext()
Loop
