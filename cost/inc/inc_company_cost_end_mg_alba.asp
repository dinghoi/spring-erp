...
objBuilder.Append "WHERE rever_yymm = '"&end_month&"' "
objBuilder.Append "	AND cost_company NOT IN ('공통', '전사', '부문', '기타', '본사', '케이원정보통신', '케이원', '') "
...
objBuilder.Append "SELECT company, org_name "
objBuilder.Append "FROM pay_alba_cost "
objBuilder.Append "WHERE rever_yymm = '"&end_month&"' "
objBuilder.Append "	AND cost_company IN ('공통', '전사', '부문', '기타', '본사', '케이원정보통신', '케이원', '') "
objBuilder.Append "GROUP BY company, org_name "
...
Do Until rsAlbaOutCost.EOF
	...
	If rsAlbaOutCostSales.EOF Or rsAlbaOutCostSales.BOF Then
		...
		Else
			...
			objBuilder.Append "WHERE trade_name = '"&rsAlbaOutCost("cost_company")&"' "
