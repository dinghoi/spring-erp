...
For i = 1 To 10
	...
	If saupbu_tab(i, 1) <> "기타사업부" Then
		...
		Set rsSalesCompCost = DBConn.Execute(objBuilder.ToString())
		objBuilder.Clear()
...
Set rsMgCost = DBConn.Execute(objBuilder.ToString())
objBuilder.Clear()
...
Do Until rsMgCost.EOF
	...
	Set rsMgCompCost = DBConn.Execute(objBuilder.ToString())
	objBuilder.Clear()
...
Set rsMgDeptCost = DBConn.Execute(objBuilder.ToString())
objBuilder.Clear()
...
Do Until rsMgDeptCost.EOF
	...
	Set rsMgDeptCompany = DBConn.Execute(objBuilder.ToString())
	objBuilder.Clear()
