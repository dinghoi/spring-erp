...
base_sql = "SELECT gect.emp_no, gect.end_yn, gect.emp_company, gect.org_name, gect.customer_no, gect.slip_date, gect.reg_date, "
base_sql = base_sql & "	gect.mg_saupbu, gect.emp_name, gect.company, gect.customer, gect.price, gect.cost, gect.cost_vat, "
base_sql = base_sql & "	gect.slip_gubun, gect.account, gect.slip_memo, gect.pl_yn, gect.reg_id, gect.slip_seq "
'base_sql = base_sql & "	eomt.org_name AS orgName, eomt.org_company, eomt.org_bonbu, eomt.org_saupbu, eomt.org_team "
base_sql = base_sql & "FROM general_cost AS gect "
'base_sql = base_sql & "INNER JOIN emp_master AS emtt ON gect.emp_no = emtt.emp_no "
'base_sql = base_sql & "INNER JOIN emp_org_mst AS eomt ON emtt.emp_org_code = eomt.org_code "
base_sql = base_sql & "where tax_bill_yn = 'Y' "
	...
	<body onLoad="condi_view()">
		<div id="wrap">
			...
			<div id="container">
				...
				<form action="/cost/tax_bill_in_mg.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>
						<dt>조회조건</dt>
                        <dd>
                            <p>
                                ...
                                <select name="slip_gubun" id="slip_gubun" style="width:120px">
                                  ...
                                  <%
                                    Sql = "select type_name from type_code where etc_seq = '4' and etc_id = 'T' order by type_name asc"
                                ...
                                <tr style="background-color: <%=bgcolor%>;">
                                    ...
                                    <% if rs("end_yn") = "C" or rs("end_yn") = "N" then %>
                                        <% if (rs("reg_id") = user_id) or (rs("emp_no") = user_id) or cost_grade = "0" or position ="사업부장" or position = "본부장"  then	%>
                                            <a href="#" onClick="pop_Window('/cost/tax_bill_in_mod.asp?slip_date=<%=rs("slip_date")%>&slip_seq=<%=rs("slip_seq")%>&u_type=U','tax_bill_in_mod_pop','scrollbars=yes,width=1000,height=300')">수정</a>
                                        <% else	%>
                                            불가
                                        <% end if %>
                                    <% else	%>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td width="24%">
					<div class="btnCenter">
                    <a href="/cost/tax_bill_in_excel.asp?bill_month=<%=bill_month%>&slip_gubun=<%=slip_gubun%>&view_c=<%=view_c%>&view_d=<%=view_d%>&emp_name=<%=emp_name%>" class="btnType04">엑셀다운로드</a>
					</div>
                  	</td>
				    <td>
                    <div id="paging">
                        <a href="<%=be_pg%>?page=<%=first_page%>&bill_month=<%=bill_month%>&slip_gubun=<%=slip_gubun%>&view_c=<%=view_c%>&view_d=<%=view_d%>&emp_name=<%=emp_name%>&ck_sw=<%="y"%>">[처음]</a>
                        <% if intstart > 1 then %>
                            <a href="<%=be_pg%>?page=<%=intstart -1%>&bill_month=<%=bill_month%>&slip_gubun=<%=slip_gubun%>&view_c=<%=view_c%>&view_d=<%=view_d%>&emp_name=<%=emp_name%>&ck_sw=<%="y"%>">[이전]</a>
                        <% end if %>
                        <% for i = intstart to intend %>
                            <% if i = int(page) then %>
                                <b>[<%=i%>]</b>
                            <% else %>
                                <a href="<%=be_pg%>?page=<%=i%>&bill_month=<%=bill_month%>&slip_gubun=<%=slip_gubun%>&view_c=<%=view_c%>&view_d=<%=view_d%>&emp_name=<%=emp_name%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                            <% end if %>
                        <% next %>
                        <% if intend < total_page then %>
                            <a href="<%=be_pg%>?page=<%=intend+1%>&bill_month=<%=bill_month%>&slip_gubun=<%=slip_gubun%>&view_c=<%=view_c%>&view_d=<%=view_d%>&emp_name=<%=emp_name%>&ck_sw=<%="y"%>">[다음]</a>
                            <a href="<%=be_pg%>?page=<%=total_page%>&bill_month=<%=bill_month%>&slip_gubun=<%=slip_gubun%>&view_c=<%=view_c%>&view_d=<%=view_d%>&emp_name=<%=emp_name%>&ck_sw=<%="y"%>">[마지막]</a>
                        <% else %>
                            [다음]&nbsp;[마지막]
                        <% end if %>
                    </div>
