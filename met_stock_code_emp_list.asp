...
be_pg = "met_stock_code_emp_list.asp"
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="met_stock_code_emp_list.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt> 창고 검색</dt>
                        <dd>
                            <p>
                                <strong>창고유형 : </strong>
                                <label>
                                <input name="stock_level" type="text" id="stock_level" value="<%=stock_level%>" readonly="true" style="width:90px; text-align:left">
                                </label>
                                <label>
				  ...
				  <table cellpadding="0" cellspacing="0" class="tableList">
				    ...
				    <tbody>
				      ...
				      <tr>
                        ...
                        <td><a href="#" onClick="pop_Window('met_stock_code_empadd.asp?stock_code=<%=rs("stock_code")%>&stock_name=<%=rs("stock_name")%>&stock_level=<%=rs("stock_level")%>&u_type=<%="U"%>','met_stock_code_pop','scrollbars=yes,width=750,height=300')">수정</a>&nbsp;</td>
			          </tr>
				      <%
							rs.movenext()
						loop
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td width="20%">
					<div class="btnCenter">
                    <a href="met_excel_stock_empcode.asp?stock_level=<%=stock_level%>" class="btnType04">엑셀다운로드</a>
					</div>                  
                  	</td>
				    <td>
                    <div id="paging">
                        <a href="met_stock_code_emp_list.asp?page=<%=first_page%>&stock_level=<%=stock_level%>&owner_view=<%=owner_view%>&condi=<%=condi%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="met_stock_code_emp_list.asp?page=<%=intstart -1%>&stock_level=<%=stock_level%>&owner_view=<%=owner_view%>&condi=<%=condi%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
   	        <% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="met_stock_code_emp_list.asp?page=<%=i%>&stock_level=<%=stock_level%>&owner_view=<%=owner_view%>&condi=<%=condi%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
   	        <% if 	intend < total_page then %>
                        <a href="met_stock_code_emp_list.asp?page=<%=intend+1%>&stock_level=<%=stock_level%>&owner_view=<%=owner_view%>&condi=<%=condi%>&ck_sw=<%="y"%>">[다음]</a> <a href="met_stock_code_emp_list.asp?page=<%=total_page%>&stock_level=<%=stock_level%>&owner_view=<%=owner_view%>&condi=<%=condi%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
				    ...
				    <td width="20%">
					<div class="btnCenter">
                    <a href="#" onClick="pop_Window('met_stock_code_empadd.asp?stock_level=<%=stock_level%>&owner_view=<%=owner_view%>&condi=<%=condi%>&u_type=<%=""%>','met_stock_code_pop','scrollbars=yes,width=750,height=300')" class="btnType04">신규 창고 등록</a>
					</div>                  
                    </td>
			      </tr>
				  </table>
