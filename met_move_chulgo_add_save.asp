...
'출고내용	
	...
	for i = 1 to 20	
		...
		if code_tab(i) = "" or isnull(code_tab(i)) then
			...
			b_qty_tab(i) = int(request.form("b_qty"&i))
	...
	sql="select max(chulgo_seq) as max_seq from met_mv_go where chulgo_date = '"&chulgo_date&"' and chulgo_stock = '"&chulgo_stock&"'"
...
'출고의뢰 update
    sql = "Update met_mv_reg set chulgo_ing='"&chulgo_ing&"',chulgo_date='"&chulgo_date&"',mod_date=now(),mod_user='"&user_name&"' where rele_date = '"&rele_date&"' and rele_stock = '"&rele_stock&"' and rele_seq = '"&rele_seq&"'"
		dbconn.execute(sql)
    ...
    for i = 1 to 20
		if code_tab(i) = "" or isnull(code_tab(i)) then
		    ...
		    if c_chk_tab(i) <> "1" then
				 sql = "select * from met_mv_reg_goods where rele_date = '"&rele_date&"' and rele_stock = '"&rele_stock&"' and rele_seq = '"&rele_seq&"' and rl_goods_seq = '"&seq_tab(i)&"' and rl_goods_code = '"&code_tab(i)&"'"
			     set Rs_buy=dbconn.execute(sql)  
			     if  not Rs_buy.eof then
			          cg_qty = Rs_buy("cg_qty") + chul_qty_tab(i)
				 
				      sql = "Update met_mv_reg_goods set chulgo_ing='"&chulgo_ing_tab(i)&"',cg_qty='"&cg_qty&"',mod_date=now(),mod_user='"&user_name&"' where rele_date = '"&rele_date&"' and rele_stock = '"&rele_stock&"' and rele_seq = '"&rele_seq&"' and rl_goods_seq = '"&seq_tab(i)&"' and rl_goods_code = '"&code_tab(i)&"'"
			          dbconn.execute(sql)
			      end if
...
'출고등록	
	...
	sql="insert into met_mv_go (chulgo_date,chulgo_stock,chulgo_seq,chulgo_goods_type,chulgo_id,chulgo_type,chulgo_stock_company,chulgo_stock_name,chulgo_emp_no,chulgo_emp_name,chulgo_company,chulgo_bonbu,chulgo_saupbu,chulgo_team,chulgo_org_name,in_stock_date,chulgo_memo,rele_stock,rele_seq,rele_date,reg_date,reg_user) values ('"&chulgo_date&"','"&chulgo_stock&"','"&chulgo_seq&"','"&chulgo_goods_type&"','"&chulgo_id&"','"&chulgo_ing&"','"&chulgo_stock_company&"','"&chulgo_stock_name&"','"&chulgo_emp_no&"','"&chulgo_emp_name&"','"&chulgo_company&"','"&chulgo_bonbu&"','"&chulgo_saupbu&"','"&chulgo_team&"','"&chulgo_org_name&"','"&in_stock_date&"','"&chulgo_memo&"','"&rele_stock&"','"&rele_seq&"','"&rele_date&"',now(),'"&user_name&"')"
		...
		if code_tab(i) = "" or isnull(code_tab(i)) then
		    ...
		    if c_chk_tab(i) <> "1" and chul_qty_tab(i) <> 0 then
				 ...
				 cg_goods_seq = right(("00" + cstr(j)),2)
		  	     sql="insert into met_mv_go_goods (chulgo_date,chulgo_stock,chulgo_seq,cg_goods_seq,cg_goods_code,cg_goods_type,cg_goods_gubun,cg_standard,cg_goods_name,cg_goods_grade,rl_qty,cg_qty,cg_type,reg_date,reg_user) values ('"&chulgo_date&"','"&chulgo_stock&"','"&chulgo_seq&"','"&cg_goods_seq&"','"&code_tab(i)&"','"&goods_type(i)&"','"&goods_gubun(i)&"','"&goods_standard(i)&"','"&goods_name(i)&"','"&goods_grade(i)&"','"&b_qty_tab(i)&"','"&chul_qty_tab(i)&"','"&chulgo_id&"',now(),'"&user_name&"')"
