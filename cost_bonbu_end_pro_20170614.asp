...
' 개인별 비용 정산 
'	sql = "delete from person_cost where (cost_month = '"&end_month&"') and (saupbu = '')"
'	dbconn.execute(sql)

	sql = "select * from emp_master_month where emp_month = '"&end_month&"' and  (emp_saupbu = '' or emp_bonbu = '') and (emp_end_date = '1900-01-01' or isnull(emp_end_date) or emp_end_date >= '"&start_date&"')"
...
' 급여 SUM
	sql = "select pmg_company,pmg_bonbu,pmg_saupbu,pmg_team,pmg_org_name,pmg_id,sum(pmg_give_total) as cost,sum(pmg_give_total) as tot_cost,sum(pmg_base_pay) as base_pay,sum(pmg_meals_pay) as meals_pay,sum(pmg_overtime_pay) as overtime_pay,sum(pmg_tax_no) as tax_no from pay_month_give where (pmg_saupbu = '' or pmg_bonbu = '') and (pmg_yymm ='"&end_month&"') and (pmg_id ='1') group by pmg_company,pmg_bonbu,pmg_saupbu,pmg_team,pmg_org_name"
	rs.Open sql, Dbconn, 1
	do until rs.eof
...
' 2015-04-27 End
	...
	rs.close()
' 상여 SUM
	sql = "select pmg_company,pmg_bonbu,pmg_saupbu,pmg_team,pmg_org_name,pmg_id,sum(pmg_give_total) as cost from pay_month_give where (pmg_saupbu = '' or pmg_bonbu = '') and (pmg_yymm ='"&end_month&"') and (pmg_id ='2') group by pmg_company,pmg_bonbu,pmg_saupbu,pmg_team,pmg_org_name,pmg_id"
	rs.Open sql, Dbconn, 1
	do until rs.eof
...
' 알바비
	sql = "select company,bonbu,saupbu,team,org_name,sum(alba_give_total) as cost from pay_alba_cost where (saupbu = '') and (rever_yymm ='"&end_month&"') group by company,bonbu,saupbu,team,org_name"
	rs.Open sql, Dbconn, 1
	do until rs.eof
...
'야특근 마감
	sql = "Update overtime set end_yn='Y' where work_date >= '"&from_date&"' and work_date <= '"&to_date&"' and (saupbu = '')"
	dbconn.execute(sql)

'일반비용	
	sql = "Update general_cost set end_yn='Y' where (slip_date >= '"&from_date&"' and slip_date <= '"&to_date&"') and (saupbu = '')"
	dbconn.execute(sql)
' DB SUM 처리 (비용)
	sql = "select emp_company,bonbu,saupbu,team,org_name,account,sum(cost) as cost from general_cost where (slip_gubun = '비용') and (cancel_yn = 'N') and (saupbu = '') and (slip_date >='"&from_date&"' and slip_date <='"&to_date&"') group by emp_company,bonbu,saupbu,team,org_name,account"
	rs.Open sql, Dbconn, 1
	do until rs.eof
	...
	rs.close()
' DB SUM 처리 (비용 외)
	sql = "select slip_gubun,emp_company,bonbu,saupbu,team,org_name,account,sum(cost) as cost from general_cost where (slip_gubun <> '비용') and (cancel_yn = 'N') and (saupbu = '') and (slip_date >='"&from_date&"' and slip_date <='"&to_date&"') group by slip_gubun,emp_company,bonbu,saupbu,team,org_name,account"
	rs.Open sql, Dbconn, 1
	do until rs.eof
...
'교통비
	sql = "Update transit_cost set end_yn='Y' where (run_date >= '"&from_date&"' and run_date <= '"&to_date&"') and (saupbu = '')"
	dbconn.execute(sql)

' DB SUM 교통비
	sql = "select emp_company,bonbu,saupbu,team,org_name,car_owner,sum(somopum+oil_price+fare+parking+toll) as cost from transit_cost where (cancel_yn = 'N') and (saupbu = '') and (run_date >='"&from_date&"' and run_date <='"&to_date&"') group by emp_company,bonbu,saupbu,team,org_name,car_owner"
	rs.Open sql, Dbconn, 1
	do until rs.eof
...
' DB SUM 교통비 (차량수리비)
	sql = "select emp_company,bonbu,saupbu,team,org_name,sum(repair_cost) as cost from transit_cost where (cancel_yn = 'N') and (repair_cost > 0) and (saupbu = '') and (run_date >='"&from_date&"' and run_date <='"&to_date&"') group by emp_company,bonbu,saupbu,team,org_name"
	rs.Open sql, Dbconn, 1
	do until rs.eof
...
' 회사 차량 운행 주유카드 셋팅
		sql = "select mg_ce_id from transit_cost where (car_owner = '회사') and (saupbu = '') and (run_date >='"&from_date&"' and run_date <='"&to_date&"') group by mg_ce_id"
		rs.Open sql, Dbconn, 1
		do until rs.eof
...
' 카드비용 집계
	sql = "select owner_company as emp_company,bonbu,saupbu,team,org_name,account,sum(cost) as cost from card_slip where (saupbu = '') and (slip_date >='"&from_date&"' and slip_date <='"&to_date&"') and (card_type not like '%주유%' or com_drv_yn = 'Y') group by owner_company,bonbu,saupbu,team,org_name,account"
	rs.Open sql, Dbconn, 1
	do until rs.eof
...
'			sum_cost = clng(rs("cost")) + clng(rs_cost(9+cost_month))
	...
	if end_yn = "C" then
		...
		"','사업부외나머지','Y','N','N','N','"&user_id&"','"&user_name&"',now())"
	end if
	dbconn.execute(sql)
end if
