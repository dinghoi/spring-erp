          	     ...
          	     <br>
     				<div class="noprint">
                        ...
                        <% if rele_sign_yn = "N" then	%>
                            <span class="btnType01"><input type="button" value="결재요청" onclick="javascript:approve_request('<%=rele_date%>','<%=rele_stock%>','<%=rele_seq%>');"></span>
                        <% end if	%>
                        <% if cancel_yn = "Y" then	%>
                        ...
                        <% end if	%>
                            ...
                            <span class="btnType01"><input type="button" value="의뢰서 출력" onClick="pop_Window('met_move_referral_print.asp?rele_stock=<%=rele_stock%>&rele_date=<%=rele_date%>&rele_seq=<%=rele_seq%>','met_move_referral_print_pop','scrollbars=yes,width=750,height=600')"></span>
                        </div>
					</div>
					<br>               		
