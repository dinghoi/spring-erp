	...
	<body oncontextmenu="return false" ondragstart="return false">
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="etc_code_mg.asp" method="post" name="condi_frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>조건 검색</dt>
				    ...
				    <tr>
				      <td width="64%" height="356" valign="top"><table cellpadding="0" cellspacing="0" class="tableList">
                        ...
                        do until rs.eof
							...
							if rs("mg_group") = "" or isnull(rs("mg_group")) then
								mg_group_view = "공통"
							  else
				        ...
				        <tr>
				          ...
				          <td><a href="etc_code_mg.asp?etc_code=<%=rs("etc_code")%>&etc_type=<%=rs("etc_type")%>&u_type=<%="U"%>"><%=rs("etc_name")%></a></td>
				          <td>&nbsp;<%=rs("etc_group")%></td>
				          <td>&nbsp;<%=rs("group_name")%></td>
				          <td><%=mg_group_view%></td>
				      ...
				      <td width="34%" valign="top"><form method="post" name="frm" action="etc_code_reg_ok.asp">
				        <table cellpadding="0" cellspacing="0" summary="" class="tableWrite">
				          <tbody>
				            ...
				            <tr>
				              <th>그룹코드</th>
				              <td class="left"><input name="etc_group" type="text" id="etc_group" size="2" maxlength="2" onKeyUp="checkNum(this);" value="<%=etc_group%>"></td>
			                </tr>
				            <tr>
				              <th>그룹명</th>
				            ...
				            <tr>
				              ...
				              <td class="left">
							  	<%
									sql="select * from type_code where etc_type = '91' order by etc_seq asc"
                                    rs_type.Open Sql, Dbconn, 1
                                %>
        						<select name="mg_group" id="mg_group">
          					<% 
								While not rs_type.eof 
							%>
          							<option value='<%=rs_type("etc_seq")%>' <%If rs_type("etc_seq") = mg_group  then %>selected<% end if %>><%=rs_type("type_name")%></option>
          					<%
									rs_type.movenext()  
								Wend 
								rs_type.Close()
