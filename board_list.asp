	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
					...
					<dl>					
                        ...
                        <dd>
                            <p>
                              	...
                              	<input type="radio" name="board_gubun" value="2" <% if board_gubun = "2" then %>checked<% end if %> style="width:30px">사내게시판
					...
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						%>
							<tr>
								...
								<td class="left"><strong>[<%=title_line%>]</strong>&nbsp;<a href="board_view.asp?board_back=<%=board_gubun%>&board_gubun=<%=rs("board_gubun")%>&board_seq=<%=rs("board_seq")%>&page=<%=page%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>"><%=rs("board_title")%></a>
                ...
                %>
				<div id="paging">
					<a href = "board_list.asp?page=<%=first_page%>&board_gubun=<%=board_gubun%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>">[처음]</a>
                  <% if intstart > 1 then %>
                  	<a href="board_list.asp?page=<%=intstart -1%>&board_gubun=<%=board_gubun%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>">[이전]</a>
                  <% end if %>
                  <% for i = intstart to intend %>
              <% if i = int(page) then %>
                  	<b>[<%=i%>]</b>
                  <% else %>
                  	<a href="board_list.asp?page=<%=i%>&board_gubun=<%=board_gubun%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                  <% end if %>
                  <% next %>
              <% if 	intend < total_page then %>
              		<a href="board_list.asp?page=<%=intend+1%>&board_gubun=<%=board_gubun%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>">[다음]</a> <a href="board_list.asp?page=<%=total_page%>&board_gubun=<%=board_gubun%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>">&nbsp;[마지막]</a>
        			<%	else %>
    				다음&nbsp;마지막
    			  <% end if %>
					&nbsp;&nbsp;&nbsp;&nbsp;
