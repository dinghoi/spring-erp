	...
	<body oncontextmenu="return false" ondragstart="return false" onselectstart="return false">
		<div id="wrap">			
			...
			<div id="container">
                ...
                <form action="insa_grade_count.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>조건 검색</dt>
				...
				<div class="gView">
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						%>	
                            <tr>
                                <td><%=com_tab(i)%></td>
                                <td><%=formatnumber(grade_cnt(i,1),0)%></td>
                                <td><%=formatnumber(grade_cnt(i,2),0)%></td>
                                <td><%=formatnumber(grade_cnt(i,3),0)%></td>
                                <td><%=formatnumber(grade_cnt(i,4),0)%></td>
                                <td><%=formatnumber(grade_cnt(i,5),0)%></td>
                                <td><%=formatnumber(grade_cnt(i,6),0)%></td>
                                <td><%=formatnumber(grade_cnt(i,7),0)%></td>
                                <td><%=formatnumber(grade_cnt(i,8),0)%></td>
                                <td><%=formatnumber(grade_cnt(i,9),0)%></td>
                                <td><%=formatnumber(grade_cnt(i,10),0)%></td>
                                <td><%=formatnumber(grade_cnt(i,11),0)%></td>
                                <td><%=formatnumber(grade_cnt(i,12),0)%></td>
                                <td><%=formatnumber(grade_cnt(i,13),0)%></td>
                                <td><%=formatnumber(grade_cnt(i,14),0)%></td>
                                <td><%=formatnumber(clng(grade_cnt(i,1)+grade_cnt(i,2)+grade_cnt(i,3)+grade_cnt(i,4)+grade_cnt(i,5)+grade_cnt(i,6)+grade_cnt(i,7)+grade_cnt(i,8)+grade_cnt(i,9)+grade_cnt(i,10)+grade_cnt(i,11)+grade_cnt(i,12)+grade_cnt(i,13)+grade_cnt(i,14)+grade_cnt(i,15)),0)%>&nbsp;</td>
                             </tr>
                        <%
							end if
						next
                        %>
							<tr>
                              ...
                              <th><%=formatnumber(clng(sum_cnt(1)+sum_cnt(2)+sum_cnt(3)+sum_cnt(4)+sum_cnt(5)+sum_cnt(6)+sum_cnt(7)+sum_cnt(8)+sum_cnt(9)+sum_cnt(10)+sum_cnt(11)+sum_cnt(12)+sum_cnt(13)+sum_cnt(14)+sum_cnt(15)),0)%>&nbsp;</th>
							</tr>
 						</tbody>
					</table>
				</div>
