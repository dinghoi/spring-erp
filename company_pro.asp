...
tot_cnt = clng(rs("tot_cnt"))
if tot_cnt = "" or isnull(tot_cnt) then
	tot_cnt = 0
end if
rs.close()

sql = " select company from as_acpt Where (mg_group='"+mg_group+"') and (Cast(acpt_date as date) >= '" + from_date + "' and Cast(acpt_date as date) <= '"+to_date+"') GROUP BY company Order By company Asc"
Rs.Open Sql, Dbconn, 1

title_line = "회사별 유형별 처리현황"
%>
	...
	<body oncontextmenu="return false" ondragstart="return false" onselectstart="return false">
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="waiting.asp?pg_name=company_pro.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>조건 검색</dt>
				...
				<div class="gView">
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						<thead>
							<tr>
								<th class="first" scope="col" rowspan="2">회사</th>
								<th scope="col" rowspan="2">총건수</th>
								<th scope="col" rowspan="2">차지율</th>
                        ...
                        do until rs.eof 
                ' 월간 유형별 미처리
                            sql = "select as_type, as_process, count(*) as end_cnt from as_acpt where (company='"+rs("company")+"') and (Cast(acpt_date as date) >= '" + from_date + "' and Cast(acpt_date as date) <= '"+to_date+"') GROUP BY as_type, as_process"		
                        ...
                        %>
							<tr>
                              ...
                              <td bgcolor="#FFFFCA" class="right"><%=formatnumber(com_cnt(0)/tot_cnt*100,2)%>%</td>
