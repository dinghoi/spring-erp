...
be_pg = "met_move_undeliver_list.asp"
...
rele_id = "창고이동"
chulgo_ing = "출고의뢰" '부분출고

order_Sql = " ORDER BY rele_date,rele_stock,rele_seq DESC"

if view_condi = "전체" then
   if goods_type = "전체" then
          where_sql = " WHERE (chulgo_ing = '출고의뢰' or chulgo_ing = '부분출고') and (chulgo_rele_date >= '"+from_date+"' and chulgo_rele_date <= '"+to_date+"')" 
	  else
	      where_sql = " WHERE (chulgo_ing = '출고의뢰' or chulgo_ing = '부분출고') and (rele_goods_type = '"&goods_type&"') and (chulgo_rele_date >= '"+from_date+"' and chulgo_rele_date <= '"+to_date+"')" 
   end if
 else  
   if goods_type = "전체" then
          where_sql = " WHERE (chulgo_ing = '출고의뢰' or chulgo_ing = '부분출고') and (rele_stock_company = '"&view_condi&"') and (chulgo_rele_date >= '"+from_date+"' and chulgo_rele_date <= '"+to_date+"')"
	  else
	      where_sql = " WHERE (chulgo_ing = '출고의뢰' or chulgo_ing = '부분출고') and (rele_goods_type = '"&goods_type&"') and (rele_stock_company = '"&view_condi&"') and (chulgo_rele_date >= '"+from_date+"' and chulgo_rele_date <= '"+to_date+"')"
   end if
end if   
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="met_move_undeliver_list.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>회사 검색</dt>
				  ...
				  <table cellpadding="0" cellspacing="0" class="tableList">
				    ...
				    <tbody>
				      ...
				      <tr>
                        ...
                        <td>
        <% if rs("chulgo_ing") = "출고의뢰" then	%>						
						<%=rs("chulgo_ing")%>&nbsp;
		<%   else  if rs("chulgo_ing") = "부분출고" or rs("chulgo_ing") = "출고완료" then	%>                        
        ...
        <%             else  if rs("chulgo_ing") = "입고완료" then	%>  
        ...
        <% if rs("rele_sign_yn") = "N" then	%>
                        <a href="#" onClick="pop_Window('met_move_reg_modify.asp?rele_date=<%=rs("rele_date")%>&rele_stock=<%=rs("rele_stock")%>&rele_seq=<%=rs("rele_seq")%>&u_type=<%="U"%>','met_move_reg_modify_pop','scrollbars=yes,width=1230,height=650')">수정</a>
		<%   else	%>
								-
		<% end if	%>
                        </td>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td width="20%">
					<div class="btnCenter">
                    <a href="met_move_undeliver_excel.asp?view_condi=<%=view_condi%>&stock=<%=stock%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>" class="btnType04">엑셀다운로드</a>
					</div>                  
                  	</td>
				    <td>
                    <div id="paging">
                        <a href="met_move_undeliver_list.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&stock=<%=stock%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="met_move_undeliver_list.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&stock=<%=stock%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
   	        <% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="met_move_undeliver_list.asp?page=<%=i%>&view_condi=<%=view_condi%>&stock=<%=stock%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
   	        <% if 	intend < total_page then %>
                        <a href="met_move_undeliver_list.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&stock=<%=stock%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[다음]</a> <a href="met_move_undeliver_list.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&stock=<%=stock%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
