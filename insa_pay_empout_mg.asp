	...
	<body oncontextmenu="return false" ondragstart="return false" onselectstart="return false">
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="insa_pay_empout_mg.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>조건 검색</dt>
					...
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						<thead>
							<tr>
								...
								<th scope="col">조&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;직</th>
                                <th colspan="2" scope="col">퇴직급여</th>
							</tr>
						</thead>
						<tbody>
						<%
						...
						if rs("emp_grade_date") = "1900-01-01" then
	           			...
	           			%>
							<tr>
                                ...
                                <td><a href="#" onClick="pop_Window('insa_pay_empout_add.asp?emp_no=<%=rs("emp_no")%>&emp_name=<%=rs("emp_name")%>&view_condi=<%=view_condi%>&u_type=<%=""%>','insa_pay_empout_add_pop','scrollbars=yes,width=1250,height=800')">처리</a></td>
                                <td><a href="#" onClick="pop_Window('insa_pay_empout_add.asp?emp_no=<%=rs("emp_no")%>&emp_name=<%=rs("emp_name")%>&pmg_yymm=<%=pmg_yymm%>&give_date=<%=give_date%>&view_condi=<%=view_condi%>&u_type=<%="U"%>','insa_pay_empout_add_pop','scrollbars=yes,width=1250,height=600')">수정</a></td>
							</tr>
						<%
							rs.movenext()
						loop
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td>
                  <div id="paging">
                        <a href = "insa_pay_empout_mg.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="insa_pay_empout_mg.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
           	<% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="insa_pay_empout_mg.asp?page=<%=i%>&view_condi=<%=view_condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
           	<% if 	intend < total_page then %>
                        <a href="insa_pay_empout_mg.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[다음]</a> <a href="insa_pay_empout_mg.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
