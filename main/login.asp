...
'신규 가입 이동[허정호_20220307]
If id = "kone" And pass = "kone"&Replace(Date(), "-", "")&"!" Then
	Response.Cookies("nkp_member")("coo_user_id") = id
	Response.Cookies("nkp_member")("coo_m_seq") = ""
	Response.Cookies("nkp_member")("coo_m_name") = ""
...
objBuilder.Append "UPDATE memb SET org_name = REPLACE(org_name, '\r', '') WHERE org_name LIKE '%\r%';"

DBConn.Execute(objBuilder.ToString())
objBuilder.Clear()

'sql="UPDATE memb SET org_name = REPLACE(org_name, '\n', '') WHERE org_name LIKE '%\n%' "
objBuilder.Append "UPDATE memb SET org_name = REPLACE(org_name, '\n', '') WHERE org_name LIKE '%\n%';"
...
objBuilder.Append "SELECT user_id, user_name, emp_no, pass, grade, emp_company, saupbu, bonbu, "
objBuilder.Append "	team, reside, reside_place, reside_company, hp, cost_grade, "
objBuilder.Append "	insa_grade, pay_grade, met_grade, account_grade, sales_grade, "
objBuilder.Append "	user_grade, mg_group, help_yn, email, position, "
objBuilder.Append "	org_name, asset_company, login_cnt "
objBuilder.Append "FROM memb WHERE user_id = '"&id&"';"
...
Else
	...
	objBuilder.Append "UPDATE memb SET login_cnt="&login_cnt&", login_date = NOW() WHERE user_id='"&id&"' "
	DBConn.Execute(objBuilder.ToString())
	objBuilder.Clear()
	...
	Else
		Response.write "<script type='text/javascript'>"
		Response.write "	location.replace('/main/nkp_main.asp?first_sw=y');"
		Response.write "</script>"
	End If
End If
