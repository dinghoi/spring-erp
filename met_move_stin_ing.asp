...
if condi = "전체" then  
	   ...
	   where_sql = " WHERE (rele_stock = '"&stock_condi&"') and (chulgo_date >= '"+from_date+"' and chulgo_date <= '"+to_date+"') "
end if
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="met_move_stin_ing.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>검색</dt>
        ...
        <% if rs("chulgo_type") = "출고완료" or rs("chulgo_type") = "부분출고" then	%>
                        <a href="#" onClick="pop_Window('met_move_stin_add.asp?chulgo_date=<%=rs("chulgo_date")%>&chulgo_stock=<%=rs("chulgo_stock")%>&chulgo_seq=<%=rs("chulgo_seq")%>&stock_in_man=<%=stock_in_man%>&u_type=<%=""%>','met_move_stin_add_pop','scrollbars=yes,width=1230,height=650')">등록</a>
		<%   else	%>
								-
		<% end if	%>
                        </td>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td>
                    <div id="paging">
                        <a href="met_move_stin_ing.asp?page=<%=first_page%>&condi=<%=condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="met_move_stin_ing.asp?page=<%=intstart -1%>&condi=<%=condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
   	        <% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="met_move_stin_ing.asp?page=<%=i%>&condi=<%=condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
   	        <% if 	intend < total_page then %>
                        <a href="met_move_stin_ing.asp?page=<%=intend+1%>&condi=<%=condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[다음]</a> <a href="met_move_stin_ing.asp?page=<%=total_page%>&condi=<%=condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
