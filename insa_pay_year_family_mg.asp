...
be_pg = "insa_pay_year_family_mg.asp"
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="insa_pay_year_family_mg.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>◈ 검색◈</dt>
				...
				<div class="gView">
					<table cellpadding="0" cellspacing="0" class="tableList">
                        ...
                        <%
                              ...
                              rs_year.close()
                          %>
                                <td class="right"><%=incom_wife_yn%>&nbsp;</td>
                                <td class="right"><%=formatnumber(incom_age20,0)%>&nbsp;</td>
                                <td class="right"><%=formatnumber(incom_age60,0)%>&nbsp;</td>
                                <td class="right"><%=formatnumber(incom_old,0)%>&nbsp;</td>
                                <td class="right"><%=formatnumber(incom_disab,0)%>&nbsp;</td>
                                <td class="right"><%=incom_woman%>&nbsp;</td>
                                <td class="left"><%=rs("emp_company")%>-<%=rs("emp_bonbu")%>-<%=rs("emp_saupbu")%>-<%=rs("emp_team")%></td>
                                <td><a href="#" onClick="pop_Window('insa_pay_year_family_add.asp?emp_no=<%=rs("emp_no")%>&emp_name=<%=rs("emp_name")%>&inc_yyyy=<%=inc_yyyy%>&u_type=<%="U"%>','insa_income_add_pop','scrollbars=yes,width=800,height=280')">등록</a></td>
							</tr>
						<%
							rs.movenext()
						loop
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td>
                    <div id="paging">
                        <a href = "insa_pay_year_family_mg.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&inc_yyyy=<%=inc_yyyy%>&owner_view=<%=owner_view%>&condi=<%=condi%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="insa_pay_year_family_mg.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&inc_yyyy=<%=inc_yyyy%>&owner_view=<%=owner_view%>&condi=<%=condi%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
           	<% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="insa_pay_year_family_mg.asp?page=<%=i%>&view_condi=<%=view_condi%>&inc_yyyy=<%=inc_yyyy%>&owner_view=<%=owner_view%>&condi=<%=condi%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
           	<% if 	intend < total_page then %>
                        <a href="insa_pay_year_family_mg.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&inc_yyyy=<%=inc_yyyy%>&owner_view=<%=owner_view%>&condi=<%=condi%>&ck_sw=<%="y"%>">[다음]</a> <a href="insa_pay_year_family_mg.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&inc_yyyy=<%=inc_yyyy%>&owner_view=<%=owner_view%>&condi=<%=condi%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
                    ...
                    <% if owner_view = "T" then 
							  ...
							  rs_emp.close()
				    %>
					<a href="#" onClick="pop_Window('insa_pay_year_family_add.asp?emp_no=<%=emp_no%>&inc_yyyy=<%=inc_yyyy%>&emp_name=<%=emp_name%>&u_type=<%=""%>','insa_income_add_pop','scrollbars=yes,width=800,height=280')" class="btnType04">부양가족수 등록</a>
					</div> 
                    <% end if %>                 
                    </td>
			      </tr>
