...
if reside = "9" and company = "전체" then
	...
	grade_sql = " (company = '" + reside_company + "' or company = '" + company + "') and "
end if
...
do until rs.eof
	...
	if rs("as_type") = "신규설치" or rs("as_type") = "신규설치공사" then
		end_tab(3) = end_tab(3) + end_cnt
	end if
	if rs("as_type") = "이전설치" or rs("as_type") = "이전설치공사" then
	...
	<body oncontextmenu="return false" ondragstart="return false" onselectstart="return false">
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="waiting.asp?pg_name=area_term_pro_req_user.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>조건 검색</dt>
					...
					<table cellpadding="0" cellspacing="0" class="tableList">
                		...
                		%>
							<tr>
                              ...
                              <th><%=formatnumber(clng(sum_cnt(5)),0)%>(<%=sum_in(5)%>)&nbsp;</th>
                              <th><%=formatnumber(clng(sum_cnt(6)),0)%>(<%=sum_in(6)%>)&nbsp;</th>
                              <th><%=formatnumber(clng(sum_cnt(7)),0)%>(<%=sum_in(7)%>)&nbsp;</th>
                              <th><%=formatnumber(clng(sum_cnt(8)),0)%>(<%=sum_in(8)%>)&nbsp;</th>
                              <th><%=formatnumber(clng(sum_cnt(9)),0)%>(<%=sum_in(9)%>)&nbsp;</th>
                              <th><%=formatnumber(clng(sum_cnt(5)+sum_cnt(6)+sum_cnt(7)+sum_cnt(8)+sum_cnt(9)),0)%>(<%=sum_in(5)+sum_in(6)+sum_in(7)+sum_in(8)+sum_in(9)%>)&nbsp;</th>
                		...
                		%>
							<tr>
                              ...
                              <td><%=formatnumber(clng(com_cnt(i,5)),0)%>(<%=com_in(i,5)%>)&nbsp;</td>
                              <td><%=formatnumber(clng(com_cnt(i,6)),0)%>(<%=com_in(i,5)%>)&nbsp;</td>
                              <td><%=formatnumber(clng(com_cnt(i,7)),0)%>(<%=com_in(i,5)%>)&nbsp;</td>
                              <td><%=formatnumber(clng(com_cnt(i,8)),0)%>(<%=com_in(i,5)%>)&nbsp;</td>
                              <td><%=formatnumber(clng(com_cnt(i,9)),0)%>(<%=com_in(i,5)%>)&nbsp;</td>
                              <td><%=formatnumber(clng(mi_sum(i)),0)%></a>(<%=com_in(i,5)+com_in(i,6)+com_in(i,7)+com_in(i,8)+com_in(i,9)%>)&nbsp;</td>
