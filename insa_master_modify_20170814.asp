					...
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						if rs("emp_grade_date") = "1900-01-01" then
                          ...
                          <% 'if  user_id = "900002"  then
						     if  user_id ="100018" or  user_id = "100787" or user_id = "100952" or user_id = "101063"or user_id = "101168" or user_id = "101100" or user_id = "101485"  then  %>
                                <td><a href="#" onClick="pop_Window('insa_emp_master_modify.asp?view_condi=<%=rs("emp_company")%>&emp_no=<%=rs("emp_no")%>&u_type=<%="U"%>','insa_emp_modify_popup','scrollbars=yes,width=1250,height=600')">수정</a></td>
                          <%    else %>
                                <td>&nbsp;</td>
                          <% end if %>
                          <% if user_id = "900002" or user_id = "100018" or user_id = "100067" or user_id = "100952" or user_id = "101063" or user_id = "101237"   then %>     
                              <td>
                              <a href="#" onClick="emp_master_del('<%=rs("emp_no")%>', '<%=rs("emp_name")%>', '<%=rs("emp_company")%>', '<%=view_condi%>');return false;">삭제</a></td>
                         <%     else %>
                              <td>&nbsp;</td>
                         <% end if %>
							</tr>
