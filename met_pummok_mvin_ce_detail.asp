            	...
            	<table cellpadding="0" cellspacing="0" class="tableList">
						...
						%>
							<tr>
                                ...
                                <td class="right"><%=formatnumber(Rs("in_cg_qty"),0)%>&nbsp;</td>
								<td>&nbsp;</td>
                                <td>
						<% if jj_qty > 0 then %>                                
                                <a href="#" onClick="pop_Window('met_chulgo_trade_add03.asp?mvin_in_date=<%=Rs("mvin_in_date")%>&stock_code=<%=Rs("mvin_in_stock")%>&mvin_in_seq=<%=Rs("mvin_in_seq")%>&in_goods_seq=<%=Rs("in_goods_seq")%>&stock_goods_type=<%=Rs("in_goods_type")%>&stock_goods_code=<%=Rs("in_goods_code")%>&u_type=<%=""%>','met_chulgo_trade_add03_pop','scrollbars=yes,width=1230,height=650')">고객(사)</a>
						<%    else   %>     
                                 -  
						<% end if %>                                                               
                                </td>
