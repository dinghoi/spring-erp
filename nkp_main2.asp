		...
		<div id="wrap">			
			...
			<div id="container">
              ...
              <% if i = int(page) then %>
                  ...
                  <% next %>
              <% if 	intend < total_page then %>
              		<a href="nkp_main2.asp?page=<%=intend+1%>&board_gubun=<%=board_gubun%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>">[다음]</a> <a href="nkp_main2.asp?page=<%=total_page%>&board_gubun=<%=board_gubun%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>">&nbsp;[마지막]</a>
        			<%	else %>
                        [다음]&nbsp;[마지막]
    			  <% end if %>
					</div>
				    ...
				    <td width="25%">
 					<div class="btnCenter">
                    <a href="#" onClick="pop_Window('board_write2.asp?board_gubun=<%=board_gubun%>','board_write_popup','scrollbars=yes,width=1250,height=600')" class="btnType04">글올리기</a>
					</div>                  
                    </td>
			      </tr>
				  </table>
