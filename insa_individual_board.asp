	...
	<body oncontextmenu="return false" ondragstart="return false" onselectstart="return false">
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="insa_individual_board.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>조건 검색</dt>
					...
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						%>
							<tr>
								...
								<td class="left"><a href="insa_indiviboard_view.asp?board_back=<%=board_gubun%>&board_gubun=<%=rs("board_gubun")%>&board_seq=<%=rs("board_seq")%>&page=<%=page%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>"><%=rs("board_title")%></a>
                ...
                %>
				<div id="paging">
					<a href = "insa_individual_board.asp?page=<%=first_page%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>">[처음]</a>
                  <% if intstart > 1 then %>
                  	<a href="insa_individual_board.asp?page=<%=intstart -1%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>">[이전]</a>
                  <% end if %>
                  <% for i = intstart to intend %>
              <% if i = int(page) then %>
                  	<b>[<%=i%>]</b>
                  <% else %>
                  	<a href="insa_individual_board.asp?page=<%=i%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                  <% end if %>
                  <% next %>
              <% if 	intend < total_page then %>
              		<a href="insa_individual_board.asp?page=<%=intend+1%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>">[다음]</a> <a href="insa_individual_board.asp?page=<%=total_page%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>">&nbsp;[마지막]</a>
        			<%	else %>
    				다음&nbsp;마지막
    			  <% end if %>
					&nbsp;&nbsp;&nbsp;&nbsp;
