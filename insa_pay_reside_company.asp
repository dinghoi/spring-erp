...
be_pg = "insa_pay_reside_company.asp"
...
if view_condi = "전체" then
      Sql = " SELECT pmg_reside_company, saup_count, pmg_base_pay, pmg_meals_pay, pmg_postage_pay, pmg_re_pay, pmg_overtime_pay, " & _
			...
			"   FROM ( " & _
			" select pmg_reside_company,count(*) as saup_count,sum(pmg_base_pay) as pmg_base_pay,sum(pmg_meals_pay) as pmg_meals_pay," & _
			...
			"   where (pmg_yymm = '"+pmg_yymm+"' ) and (pmg_id = '1') group by pmg_reside_company " & _
			"   order by pmg_reside_company " & _
			"   ) a, " & _
			" ( select de_reside_company,sum(de_nps_amt) as de_nps_amt,sum(de_nhis_amt) as de_nhis_amt,sum(de_epi_amt) as de_epi_amt," & _
			...
			"   where (de_yymm = '"+pmg_yymm+"' ) and (de_id = '1') group by de_reside_company " & _	
			"   order by de_reside_company " & _
			"   ) b " & _		
			"  WHERE a.pmg_reside_company = b.de_reside_company " & _
			"  ORDER BY pmg_reside_company ASC " 
    else
      Sql = " SELECT pmg_reside_company, saup_count, pmg_base_pay, pmg_meals_pay, pmg_postage_pay, pmg_re_pay, pmg_overtime_pay, " & _
            "   pmg_car_pay, pmg_position_pay, pmg_custom_pay,pmg_job_pay,pmg_job_support,pmg_jisa_pay,pmg_long_pay, " & _
			"   pmg_disabled_pay,pmg_give_total, " & _
			...
			"   sum(pmg_long_pay) as pmg_long_pay,sum(pmg_disabled_pay) as pmg_disabled_pay,sum(pmg_give_total) as pmg_give_total " & _
			"   from pay_month_give " & _
			"   where (pmg_yymm = '"+pmg_yymm+"' ) and (pmg_id = '1') and (pmg_company = '"+view_condi+"') group by pmg_reside_company " & _
			"   order by pmg_company,pmg_reside_company " & _
			"   ) a, " & _
			" ( select de_reside_company,sum(de_nps_amt) as de_nps_amt,sum(de_nhis_amt) as de_nhis_amt,sum(de_epi_amt) as de_epi_amt," & _
            "   sum(de_longcare_amt) as de_longcare_amt,sum(de_income_tax) as de_income_tax,sum(de_wetax) as de_wetax," & _
			"   sum(de_year_incom_tax) as de_year_incom_tax,sum(de_year_wetax) as de_year_wetax," & _
			...
			"   sum(de_deduct_total) as de_deduct_total " & _
			"   from pay_month_deduct " & _
			"   where (de_yymm = '"+pmg_yymm+"' ) and (de_id = '1') and (de_company = '"+view_condi+"') group by de_reside_company " & _	
			"   order by de_company,de_reside_company " & _
			"   ) b " & _		
			"  WHERE a.pmg_reside_company = b.de_reside_company " & _
			"  ORDER BY pmg_reside_company ASC " 
end if
	...
	<body oncontextmenu="return false" ondragstart="return false" onselectstart="return false">
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="insa_pay_reside_company.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>◈ 검색◈</dt>
				...
				<div class="gView">
					...
					<%
						do until rs.eof
							...
							saupbu_name = rs("pmg_reside_company")
					   ...
					   <%
						...
						%>
                          	<tr>
                                ...
                                <th class="right"><%=formatnumber(pay_count(6),0)%>&nbsp;명</th>
                                <th class="right"><%=formatnumber(sum_base_pay(6),0)%>&nbsp;</th>
                                <th class="right"><%=formatnumber(sum_meals_pay(6),0)%>&nbsp;</th>
                                <th class="right"><%=formatnumber(sum_overtime_pay(6),0)%>&nbsp;</th>
                                <th class="right"><%=formatnumber(sum_give_hap,0)%>&nbsp;</th>
                                <th class="right"><%=formatnumber(sum_give_tot(6),0)%>&nbsp;</th>
                                ...
                                <th class="right"><%=formatnumber(sum_deduct_tot(6),0)%>&nbsp;</th>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td>
                    <div id="paging">
                        <a href = "insa_pay_reside_company.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&pmg_yymm=<%=pmg_yymm%>ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="insa_pay_reside_company.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&pmg_yymm=<%=pmg_yymm%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
           	<% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="insa_pay_reside_company.asp?page=<%=i%>&view_condi=<%=view_condi%>&pmg_yymm=<%=pmg_yymm%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
           	<% if 	intend < total_page then %>
                        <a href="insa_pay_reside_company.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&pmg_yymm=<%=pmg_yymm%>&ck_sw=<%="y"%>">[다음]</a> <a href="insa_pay_reside_company.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&pmg_yymm=<%=pmg_yymm%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
