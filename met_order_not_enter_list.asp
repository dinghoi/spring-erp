...
be_pg = "met_order_not_enter_list.asp"
...
order_ing = "4" '입고제외 and order_ing <> '4' sql에 추가할것

order_Sql = " ORDER BY order_company,order_date,order_no,order_seq DESC"
if view_condi = "전체" then
   if goods_type = "전체" then
      where_sql = " WHERE (order_date >= '"+from_date+"' and order_date <= '"+to_date+"') and (order_ing <> '4')" 
	  else
	  where_sql = " WHERE (order_goods_type = '"&goods_type&"') and (order_date >= '"+from_date+"' and order_date <= '"+to_date+"') and (order_ing <> '4')" 
   end if
 else  
   if goods_type = "전체" then
      where_sql = " WHERE (order_company = '"&view_condi&"') and (order_date >= '"+from_date+"' and order_date <= '"+to_date+"') and (order_ing <> '4')"
	  else
	  where_sql = " WHERE (order_goods_type = '"&goods_type&"') and (order_company = '"&view_condi&"') and (order_date >= '"+from_date+"' and order_date <= '"+to_date+"') and (order_ing <> '4')"
   end if
end if   
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="met_order_not_enter_list.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>회사 검색</dt>
		...
		%>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td>
                    <div id="paging">
                        <a href="met_order_not_enter_list.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="met_order_not_enter_list.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
   	        <% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="met_order_not_enter_list.asp?page=<%=i%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
   	        <% if 	intend < total_page then %>
                        <a href="met_order_not_enter_list.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[다음]</a> <a href="met_order_not_enter_list.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
