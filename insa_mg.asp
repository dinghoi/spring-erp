...
if  team <> "인사총무" then
    where_sql = where_sql & " and emp_team = '"&team&"' "
end if
			...
			<div id="container">
				...
				<div class="gView">
					...
					<tbody>
                        ...
                        <tr>
                            ...
                            <% if insa_grade = "0" and user_id = "900002" then  %>
                                <td><a href="#" onClick="pop_Window('insa_emp_add01.asp?view_condi=<%=view_condi%>&emp_no=<%=rs("emp_no")%>&u_type=<%="U"%>','insa_emp_add01_popup','scrollbars=yes,width=1250,height=600')">수정</a></td>
                            <% else %>
                                <td>&nbsp;</td>
                            <% end if %>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td>
                    <div id="paging">
                        <a href="insa_mg.asp?page=<%=first_page%>&view_sort=<%=view_sort%>&view_condi=<%=view_condi%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="insa_mg.asp?page=<%=intstart -1%>&view_sort=<%=view_sort%>&view_condi=<%=view_condi%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
                  	<% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="insa_mg.asp?page=<%=i%>&view_sort=<%=view_sort%>&view_condi=<%=view_condi%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
                  	<% if 	intend < total_page then %>
                        <a href="insa_mg.asp?page=<%=intend+1%>&view_sort=<%=view_sort%>&view_condi=<%=view_condi%>&ck_sw=<%="y"%>">[다음]</a> <a href="insa_mg.asp?page=<%=total_page%>&view_sort=<%=view_sort%>&view_condi=<%=view_condi%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
