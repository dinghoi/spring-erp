...
' 원격외
sql = " select count(*) as tot_cnt from as_acpt Where (as_type <> '원격처리' and as_process <> '취소' and as_type <> '야특근') and ( team not  like '%지원%') and (company <> '기타' and company <> '케이원정보통신') and  mg_ce not in ('파라','박양준','김시욱','도현석','백지운','이종욱') and reside='0'  and reside_place=' ' and (Cast(visit_date as date) >= '" + from_date + "' and Cast(visit_date as date) <= '"+to_date+"')"
Set rs=DbConn.Execute(SQL)
...
sql = " select company, count(*) as as_cnt from as_acpt Where (as_type <> '원격처리' and as_process <> '취소' and as_type <> '야특근') and ( team not  like '%지원%') and (company <> '기타' and company <> '케이원정보통신') and  mg_ce not in ('파라','박양준','김시욱','도현석','백지운','이종욱') and reside='0'  and reside_place=' ' and (Cast(visit_date as date) >= '" + from_date + "' and Cast(visit_date as date) <= '"+to_date+"') GROUP BY company Order By company Asc"
Rs.Open Sql, Dbconn, 1
...
do until rs.eof 
	...
	if rs_etc.eof or rs_etc.bof then
		...
		cost_amt = int(charge_per * tot_cost)
		sql="insert into company_as (as_month,as_company,saupbu,visit_cnt,charge_per,cost_amt,reg_id,reg_name,reg_date) values ('"&end_month&"','"&rs("company")&"','"&saupbu&"','"&rs("as_cnt")&"','"&charge_per&"',"&cost_amt&",'"&user_id&"','"&user_name&"',now())"
		dbconn.execute(sql)
	  else
		charge_per = clng(rs("as_cnt")) / bang_cnt * bang_per / 100 + rs_etc("charge_per")
		cost_amt = int(charge_per * tot_cost)
		sql = "update company_as set visit_cnt='"&rs("as_cnt")&"', charge_per='"&charge_per&"', cost_amt="&cost_amt&" where as_company='" &rs("company")& "' and as_month = '" &end_month& "'"
		dbconn.execute(sql)	  
	end if
...
' 추가 로직
' 사업부별 인원수 집계
sql = " select * from sales_org Order By saupbu Asc"
Rs.Open Sql, Dbconn, 1
i = 0
tot_person = 0
do until rs.eof 
