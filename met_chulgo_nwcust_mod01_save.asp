...
'    end if
	...
	if filenm <> "" then
		...
		if filenm <> "" then 
			...
			filename = "NW출고" + cstr(rele_no) + file_name + "." + fileType
		...
		if code_tab(i) = "" or isnull(code_tab(i)) then
		    ...
		    if qty_tab(i) <> 0 then
				 ...
				 cg_goods_seq = right(("00" + cstr(j)),2)
		  	     sql="insert into met_chulgo_goods (chulgo_date,chulgo_stock,chulgo_seq,cg_goods_seq,cg_goods_code,cg_goods_type,cg_goods_gubun,cg_standard,cg_goods_name,cg_goods_grade,rl_qty,cg_qty,cg_type,rl_service_no,rl_trade_name,rl_trade_dept,rl_stock_code,rl_stock_company,rl_stock_name,rl_company,rl_saupbu,rl_bigo,reg_date,reg_user,chulgo_stock_company,chulgo_stock_name) values ('"&chulgo_date&"','"&chulgo_stock&"','"&chulgo_seq&"','"&cg_goods_seq&"','"&code_tab(i)&"','"&goods_type(i)&"','"&goods_gubun(i)&"','"&goods_standard(i)&"','"&goods_name(i)&"','"&goods_grade(i)&"',0,'"&qty_tab(i)&"','"&chulgo_id&"','"&service_no(i)&"','"&trade_name(i)&"','"&trade_dept(i)&"','"&rele_stock&"','"&rele_stock_company&"','"&rele_stock_name&"','"&rele_company&"','"&rele_saupbu&"','"&bigo(i)&"',now(),'"&user_name&"','"&chulgo_stock_company&"','"&chulgo_stock_name&"')"
...
' 재고 등록				 
	...
	'response.write"location.replace('meterials_stock_nwout_mg.asp');"
