...
<html>
						...
						<tbody>
							...
							<tr>
								...
								<% if not rs_a.eof Then %> 
									<a href="insa_board_view.asp?board_seq=<%=rs_a("board_seq")%>&board_gubun=<%=rs_a("board_gubun")%>&page=<%=page%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>"><%=rs_a("board_title")%></a>
								<% Else %>
									처음 자료입니다.
								<% End If %>
                                </td>
							...
							<tr>
								...
								<% if not rs_p.eof Then %> 
									<a href="insa_board_view.asp?board_seq=<%=rs_p("board_seq")%>&board_gubun=<%=rs_p("board_gubun")%>&page=<%=page%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>"><%=rs_p("board_title")%></a>
								<% Else %>
									처음 자료입니다.
								<% End If %>
                                </td>
	...
	<div align=center>
		...
		<%	else	%>        
		<a href="insa_board_list.asp?board_gubun=<%=board_back%>&page=<%=page%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>" class="btnType04">
		<%	end if	%>
		목록</a> &nbsp;
		<a href="insa_board_write.asp?board_seq=<%=rs("board_seq")%>&page=<%=page%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>&u_type=<%="U"%>" class="btnType04">
        수정</a> &nbsp;
