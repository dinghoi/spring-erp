...
'2014-01-25 기존에 설치사진 첨부 (종료)
	...
	yymmdd = mid(cstr(sales_date),3,2) + mid(cstr(sales_date),6,2)  + mid(cstr(sales_date),9,2)
	...
	sql="select max(slip_no) as max_no from sales_slip where sales_date = '"&sales_date&"'"
