	...
	<body>
		...
		<!--#include virtual = "/include/as_sub_menu.asp" -->
			<div id="container">
				...
				<form action="as_acpt_reg_ok_new.asp" method="post" name="frm">
			  <div class="gView">
					<table cellpadding="0" cellspacing="0" class="tableWrite">
						...
						<tbody>
							...
							<tr>
                                ...
                                <th>협업여부</th>
                                <td class="left">
                                    <input type="radio" name="cowork_yn" value="N" <% if cowork_yn = "N" then %>checked<% end if %>>일반
                                    <input type="radio" name="cowork_yn" value="Y" <% if cowork_yn = "Y" then %>checked<% end if %>>협업
                                </td>
							</tr>
							<tr>
                                ...
                                <%
								Sql="SELECT max(au_month) as max_au_month FROM as_unitprice_month where  use_yn = 'N'"
								'Response.write Sql
                                set Rs_etc=dbconn.execute(sql)
	                            max_au_month = Rs_etc("max_au_month")
                                rs_etc.Close()

                                Sql="SELECT * FROM as_unitprice_month WHERE au_month = '"&max_au_month&"' and use_yn = 'N'"
								...
								do until rs_etc.eof
			  				        %>
                					<option value="<%=rs_etc("au_code")%>"><%=rs_etc("au_name")%></option>
                			        <%
									rs_etc.movenext()
								loop
								rs_etc.Close()
                                ...
                                <strong>야간작업</strong> <input type="checkbox" name="night" id="night" value="Y">
								<strong>주말작업</strong> <input type="checkbox" name="weekend_work" id="weekend_work" value="Y">
