...
K = 0
if position = "팀장" then
	sql = "select * from memb where bonbu ='"&bonbu&"' and saupbu ='"&saupbu&"' and team ='"&team&"' order by user_name" 
'	sql = "select * from memb where team ='"&team&"' order by user_name" 
	rs_memb.Open sql, Dbconn, 1
	do until rs_memb.eof
		k = k + 1
		ce_tab(k,1) = rs_memb("user_id")
		ce_tab(k,2) = rs_memb("user_name")
		ce_tab(k,3) = rs_memb("user_grade")
		rs_memb.movenext()
	loop
end if
...
'	response.write(sql)
	rs.Open sql, Dbconn, 1
	do until rs.eof
		ce_sum(i,3,1) = ce_sum(i,3,1) + cint(rs("c_cnt"))
		ce_sum(i,3,2) = ce_sum(i,3,2) + cdbl(rs("cost"))
		rs.movenext()
	loop
	rs.close()
next
...
for i = 1 to loop_cnt
	...
	do until rs.eof
		if rs("car_owner") = "개인" then
			ce_sum(i,4,1) = ce_sum(i,4,1) + rs("far")
			ce_sum(i,4,2) = ce_sum(i,4,2) + (rs("far")*25)
		end if

		ce_sum(i,5,2) = ce_sum(i,5,2) + rs("fare")

		if rs("car_owner") = "회사" then
			ce_sum(i,6,2) = ce_sum(i,6,2) + rs("oil_price")
			ce_sum(i,7,2) = ce_sum(i,7,2) + rs("repair_cost")
		end if

		ce_sum(i,8,2) = ce_sum(i,8,2) + rs("parking")
		ce_sum(i,9,2) = ce_sum(i,9,2) + rs("toll")
...
' 카드사용
for i = 1 to loop_cnt
	sql = "select count(*) as c_cnt,sum(cost) as cost,sum(cost_vat) as cost_vat from card_slip where (emp_no='"&ce_tab(i,1)&"') and (slip_date >='"&from_date&"' and slip_date <='"&to_date&"')"
		
	Set rs = Dbconn.Execute (sql)
	if cint(rs("c_cnt")) <>  0 then
		ce_sum(i,10,1) = cint(rs("c_cnt"))
		ce_sum(i,10,2) = cdbl(rs("cost")) + cdbl(rs("cost_vat"))
	  else
		ce_sum(i,10,1) = 0
		ce_sum(i,10,2) = 0  
	end if
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="team_emp_cost_mg.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>조건 검색</dt>
                        <dd>
                            <p>
								...
								<label>
								<strong>조직정보 : </strong><%=team%>&nbsp;<%=position%>
								</label>
				...
				<div class="gView">
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						<thead>
							<tr>
								<th rowspan="3" class="first" scope="col">CE명</th>
								<th rowspan="3" scope="col">차량정보</th>
						...
						for i = 1 to loop_cnt
							...
							if j <> 0 then
' 차량 정보
							sql = "select * from car_info where owner_emp_no ='"&ce_tab(i,1)&"'"
							set rs_car=dbconn.execute(sql)
							if rs_car.eof then
								car_info = "차량없음"
							  else  	
								car_info = rs_car("car_owner") + "차량 , " + rs_car("oil_kind")
							end if
							rs_car.Close()		
						%>
							<tr>
								<td class="first"><%=ce_tab(i,2)%>&nbsp;<%=ce_tab(i,3)%></td>
								<td><%=car_info%></td>
								...
								<td class="right"><%=formatnumber(ce_sum(i,4,1),0)%></td>
								<td class="right"><%=formatnumber(ce_sum(i,4,2),0)%></td>
								<td class="right"><%=formatnumber(ce_sum(i,5,2),0)%></td>
								<td class="right"><%=formatnumber(ce_sum(i,6,2),0)%></td>
								<td class="right"><%=formatnumber(ce_sum(i,7,2),0)%></td>
								<td class="right"><%=formatnumber(ce_sum(i,8,2),0)%></td>
								<td class="right"><%=formatnumber(ce_sum(i,9,2),0)%></td>
								<td class="right"><%=formatnumber(ce_sum(i,10,1),0)%></td>
								<td class="right"><%=formatnumber(ce_sum(i,10,2),0)%></td>
							</tr>
						<%
							end if
						next
						%>
							<tr>
								...
								<th class="right"><%=formatnumber(ce_sum(0,4,1),0)%></th>
								<th class="right"><%=formatnumber(ce_sum(0,4,2),0)%></th>
								<th class="right"><%=formatnumber(ce_sum(0,5,2),0)%></th>
								<th class="right"><%=formatnumber(ce_sum(0,6,2),0)%></th>
								<th class="right"><%=formatnumber(ce_sum(0,7,2),0)%></th>
								<th class="right"><%=formatnumber(ce_sum(0,8,2),0)%></th>
								<th class="right"><%=formatnumber(ce_sum(0,9,2),0)%></th>
								<th class="right"><%=formatnumber(ce_sum(0,10,1),0)%></th>
								<th class="right"><%=formatnumber(ce_sum(0,10,2),0)%></th>
							</tr>
						</tbody>
					</table>
				</div>
