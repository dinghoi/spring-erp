...
be_pg = "met_stock_in_report02.asp"
...
Sql = "SELECT count(*) FROM met_stin_goods " + where_sql + stock_sql + goods_name_sql
...
sql = "select * from met_stin_goods " + where_sql + stock_sql + goods_name_sql + order_sql + " limit "& stpage & "," &pgsize
Rs.Open Sql, Dbconn, 1
'response.write(sql)

title_line = " 품목별 입고 현황 "
%>
	...
	<body>
		<div id="wrap">
			...
			<div id="container">
				...
				<form action="met_stock_in_report02.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>
						<dt>검색조건</dt>
                        <dd>
                            <p>
							  ...
							  %>
                                ...
                                <label>
                                <strong>품목명&nbsp;</strong>
                                   <input name="goods_name" type="text" id="goods_name" value="<%=goods_name%>" style="width:100px; text-align:left; ime-mode:active">
                                </label>
                                <label>
								<strong>입고일자&nbsp;</strong>
			   ...
			   %>
				      <tr>
                        ...
                        <td><%=stin_buy_company%>&nbsp;</td>
                        <td><%=stin_buy_saupbu%>&nbsp;</td>
                        <td><%=rs("stin_stock_name")%>&nbsp;(<%=rs("stin_stock_company")%>)</td>
                        <td><%=rs("stin_goods_name")%>&nbsp;&nbsp;<%=rs("stin_standard")%></td>
                        <td class="right"><%=formatnumber(rs("stin_amt"),0)%></td>
                        <td><%=stin_trade_name%>&nbsp;</td>
                        <td>
                        -
                        </td>
			          </tr>
			  ...
			  %>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
					...
					<div class="btnleft">
                <% if met_grade = "0" then	%>
                    <a href="met_stock_in_excel02.asp?view_condi=<%=view_condi%>&stock=<%=stock%>&goods_name=<%=goods_name%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>" class="btnType04">엑셀다운로드</a>
                <% end if	%>
					</div>
                  	</td>
                    <td>
                    <div id="paging">
                        <a href="met_stock_in_report02.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&stock=<%=stock%>&goods_name=<%=goods_name%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="met_stock_in_report02.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&stock=<%=stock%>&goods_name=<%=goods_name%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
   	        <% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="met_stock_in_report02.asp?page=<%=i%>&view_condi=<%=view_condi%>&stock=<%=stock%>&goods_name=<%=goods_name%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
   	        <% if 	intend < total_page then %>
                        <a href="met_stock_in_report02.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&stock=<%=stock%>&goods_name=<%=goods_name%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[다음]</a> <a href="met_stock_in_report02.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&stock=<%=stock%>&goods_name=<%=goods_name%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
