...
if view_condi <> "" Then
	...
	if condi = "전체" then
		if owner_view = "T" then
			...
			objBuilder.Append "WHERE app_empno = '"&view_condi&"' AND (app_id = '포상발령' OR app_id = '징계발령') "
		else
			'Sql = "SELECT count(*) FROM emp_appoint where (app_emp_name like '%"+view_condi+"%') and (app_id = '포상발령' or app_id = '징계발령')"
			objBuilder.Append "WHERE app_emp_name LIKE '%"&view_condi&"%' AND (app_id = '포상발령' OR app_id = '징계발령') "
		end if
	else
		if owner_view = "T" then
			...
			<div id="container">
				...
				<div class="gView">
					<table cellpadding="0" cellspacing="0" class="tableList">
                        ...
                        <% if rsApp("app_id") = "포상발령" then %>
						      <td class="left">(포상)<%=rsApp("app_id_type")%>&nbsp;</td>
                              <td class="left">&nbsp;</td>
                              <td class="left"><%=rsApp("app_reward")%>&nbsp;</td>
                        <%    elseif rsApp("app_id") = "징계발령" then %>
                              <td class="left">(징계)<%=rsApp("app_id_type")%>&nbsp;</td>
                        ...
                        <% if user_id = "900002" Or user_id = "102592" then %>
                              <td >
                              <a href="#" onClick="pop_Window('/insa/insa_reward_punish_add.asp?app_empno=<%=rsApp("app_empno")%>&emp_name=<%=emp_name%>&owner_view=<%=owner_view%>&u_type=<%=""%>','insa_reward_punish_add_pop','scrollbars=yes,width=750,height=300')">등록</a></td>
							  <td><a href="#" onClick="pop_Window('/insa/insa_reward_punish_add.asp?app_empno=<%=rsApp("app_empno")%>&app_seq=<%=rsApp("app_seq")%>&emp_name=<%=emp_name%>&owner_view=<%=owner_view%>&u_type=<%="U"%>','insa_reward_punish_add_pop','scrollbars=yes,width=750,height=300')">수정</a></td>
                         <% if insa_grade = "0" then %>
                              <td>
                              <a href="#" onClick="reward_punish_del('<%=rsApp("app_empno")%>', '<%=rsApp("app_seq")%>', '<%=emp_name%>');return false;">삭제</a></td>
                         <%     else %>
                              <td>&nbsp;</td>
                         <% end if %>
                         <% end if %>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td>
                  <div id="paging">
                        <a href = "<%=be_pg%>?page=<%=first_page%>&view_condi=<%=view_condi%>&condi=<%=condi%>&owner_view=<%=owner_view%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="<%=be_pg%>insa_reward_punish_mg.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&condi=<%=condi%>&owner_view=<%=owner_view%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
           	...
           	<% if i = int(page) then %>
                      ...
                      <% next %>
           	<% if 	intend < total_page then %>
                        <a href="<%=be_pg%>insa_reward_punish_mg.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&condi=<%=condi%>&owner_view=<%=owner_view%>&ck_sw=<%="y"%>">[다음]</a> <a href="<%=be_pg%>insa_reward_punish_mg.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&condi=<%=condi%>&owner_view=<%=owner_view%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
					...
					<div class="btnRight">
					<% if user_id = "900002" or user_id = "102592" then %>
                    <a href="#" onClick="pop_Window('/insa/insa_reward_punish_add.asp?family_empno=<%=view_condi%>&emp_name=<%=emp_name%>','insa_reward_punish_add_pop','scrollbars=yes,width=750,height=300')" class="btnType04">상벌사항등록</a>
                    <% end if %>
					</div>
                    </td>
			      </tr>
