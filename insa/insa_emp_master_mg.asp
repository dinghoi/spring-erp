...
objBuilder.Append "	FIELD(emtt.emp_job, '사장', '부사장', '총괄대표', '전무이사', '상무이사', '이사', "
			...
			<div id="container">
				...
				<div class="gView">
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						<thead>
							<tr>
								<th class="first" scope="col">사 번</th>
								<th scope="col">성 명</th>
								<th scope="col">직 급</th>
								<th scope="col">소 속 (조직명)</th>
								<th scope="col">조 직 (회사/본부/사업부/팀)</th>
								<th scope="col">상주 회사</th>
								<th scope="col">상주처</th>
								<th scope="col">실 근무지</th>
								<th scope="col">비용 구분</th>
								<th scope="col"></th>
							</tr>
						</thead>
					<tbody>
						...
						Do Until rs.EOF
							...
							<tr>
								...
								<td><a href="#" onClick="pop_Window('/insa/insa_emp_master_modify.asp?view_condi=<%=tEmpCompany%>&emp_no=<%=tEmpNo%>&u_type=U','인사기본사항 변경','scrollbars=yes,width=1250,height=600')"><%=rs("emp_name")%></td>
								<td><%=rs("emp_job")%></td>
								<td>
                                    ...
                                    <a href="#" class="btnType03" onClick="pop_Window('/insa/popup/insa_emp_master_org_select.asp?gubun=org&view_condi=<%=view_condi%>&org_id=<%=j%>','orgselect','scrollbars=yes,width=800,height=400')">선택</a>
								</td>
								<td>
								...
								<td><a href="#" class="btnType04" onClick="changeEmpMasterMonth('<%=tEmpNo%>')">적용</a></td>
							</tr>
						    <%
							rs.MoveNext()
						Loop
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td>
                    <div id="paging">
                        <a href="<%=be_pg%>?page=<%=first_page%>&view_sort=<%=view_sort%>&view_condi=<%=view_condi%>&srchCategory=<%=srchCategory%>&srchWord=<%=srchWord%>&ck_sw=y">[처음]</a>
                  	<%If intstart > 1 Then %>
                        <a href="<%=be_pg%>?page=<%=intstart -1%>&view_sort=<%=view_sort%>&view_condi=<%=view_condi%>&srchCategory=<%=srchCategory%>&srchWord=<%=srchWord%>&ck_sw=y">[이전]</a>
                    <%End If %>
                    ...
                    <%Next %>
                  	...
                  	<%If intend < total_page Then %>
                        <a href="<%=be_pg%>?page=<%=intend+1%>&view_sort=<%=view_sort%>&view_condi=<%=view_condi%>&srchCategory=<%=srchCategory%>&srchWord=<%=srchWord%>&ck_sw=y">[다음]</a>
						<a href="<%=be_pg%>?page=<%=total_page%>&view_sort=<%=view_sort%>&view_condi=<%=view_condi%>&srchCategory=<%=srchCategory%>&srchWord=<%=srchWord%>&ck_sw=y">[마지막]</a>
                    <%Else %>
                        [다음]&nbsp;[마지막]
                    <%End If %>
                    </div>
