...
field_sql = "AND (cait.end_date = '' OR cait.end_date IS NULL OR cait.end_date = '1900-01-01') "
If field_check <> "total" Then
...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function carInfoExcel(o_view, f_check, f_view){
				var url = '/insa/excel/insa_excel_car_info.asp';
				var param;
			...
			//차량 운행자 등록 팝업[허정호_20210720]
			function carDriveUserMod(c_no, c_name, c_year, c_date, e_name, e_no){
				var url = '/insa/insa_car_drvuser_add.asp';
				var pop_name = '차량 운행자 등록';
				var features = 'scrollbars=yes,width=750,height=300';
				var param;

				param = '?car_no='+c_no+'&car_name='+c_name+'&car_year='+c_year+'&car_reg_date='+c_date;
				param += '&owner_emp_name='+e_name+'&owner_emp_no='+e_no+'&u_type=';
			...
			//차량 보험 등록[허정호_20210722]
			function carInsInit(car_no, car_name, car_year, car_reg_date){
				var url = '/insa/insa_car_insurance_add.asp';
				var pop_name = '보험 등록';
				var features = 'scrollbars=yes,width=750,height=410';
				var param;
			...
			<div id="container">
					...
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						%>
							<tr>
								...
								<td>
									<a href="#" onclick="carInsInit('<%=car_no%>', '<%=car_name%>', '<%=car_year%>', '<%=car_reg_date%>');">등록</a>
								</td>
								<td>
									<a href="#" onclick="carDriveUserMod('<%=car_no%>', '<%=car_name%>', '<%=car_year%>', '<%=car_reg_date%>', '<%=owner_emp_name%>', '<%=owner_emp_no%>');">변경</a>
								</td>
								<!--<td>
                					<a href="#" onClick="pop_Window('/insa_car_as_add.asp?car_no=<%=rsCar("car_no")%>&car_name=<%=rsCar("car_name")%>&car_year=<%=rsCar("car_year")%>&car_reg_date=<%=rsCar("car_reg_date")%>&owner_emp_name=<%=owner_emp_name%>&owner_emp_no=<%=rsCar("owner_emp_no")%>&car_use_dept=<%=rsCar("car_use_dept")%>&oil_kind=<%=rsCar("oil_kind")%>&car_owner=<%=rsCar("car_owner")%>&u_type=<%=""%>','car_as_add_popup','scrollbars=yes,width=750,height=350')">등록</a>
								</td>
								<td>
									<a href="#" onClick="pop_Window('/insa_car_penalty_add.asp?car_no=<%=rsCar("car_no")%>&car_name=<%=rsCar("car_name")%>&car_year=<%=rsCar("car_year")%>&car_reg_date=<%=rsCar("car_reg_date")%>&owner_emp_name=<%=owner_emp_name%>&owner_emp_no=<%=rsCar("owner_emp_no")%>&car_use_dept=<%=rsCar("car_use_dept")%>&oil_kind=<%=rsCar("oil_kind")%>&car_owner=<%=rsCar("car_owner")%>&u_type=<%=""%>','car_as_add_popup','scrollbars=yes,width=750,height=410')">등록</a>
								</td>-->
							</tr>
						<%
							rsCar.MoveNext()
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td width="20%">
					<div class="btnCenter">
					<a href="#" onclick="carInfoExcel('<%=owner_view%>', '<%=field_check%>', '<%=field_view%>');" class="btnType04">엑셀다운로드</a>
					</div>
                  	</td>
				    <td>
					<%
				    ...
				    <td width="20%">
					<div class="btnCenter">
                    <a href="#" onClick="pop_Window('/insa/insa_car_info_add.asp','car_info_add_popup','scrollbars=yes,width=750,height=500')" class="btnType04">신규차량등록</a>
					</div>
                    </td>
			      </tr>
				  </table>
