...
'### Request & Params
'===================================================
Dim be_pg, cfm_use, cfm_user_dept, cfm_comment, cfm_use_dept
Dim view_condi, ck_sw, owner_view, rsConf, title_line
'Dim emp_name

be_pg = "/insa/insa_confirm_list.asp"
...
if view_condi <> "" Then
	...
	objBuilder.Append "	AND (isNull(emtt.emp_end_date) OR emtt.emp_end_date = '1900-01-01' "
	objBuilder.Append "		OR emtt.emp_end_date  = '0000-00-00') "
	...
	if owner_view = "C" then
		...
		objBuilder.Append "	AND emtt.emp_no = '"&view_condi&"' "
	end If

	objBuilder.Append "ORDER BY emtt.emp_no, emtt.emp_company, emtt.emp_bonbu, "
	objBuilder.Append "emtt.emp_saupbu, emtt.emp_team ASC "
...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function s_sinchung(val, val2) {
				var actionUrl = "";
				var list = $("#dataList").html();
				var inEmpNo = "";
				var inName = "";
				var cfmUse = "";
				var cfmUseDept = "";
				...
				$.each( $("#dataList tr"), function(){
					inEmpNo = $.trim($(this).find("td").eq(0).text());
					if( val==inEmpNo ){
						inName = $.trim($(this).find("td").eq(1).text());
						cfmUse = $.trim($(this).find("select[name='cfm_use'] option:selected").val());
						cfmUseDept = $.trim($(this).find(":text[name='cfm_use_dept']").val());
						cfmComment = $.trim($(this).find(":text[name='cfm_comment']").val());
				...
				*/
				...
				document.frm.in_empno.value = inEmpNo;
				document.frm.in_name.value = inName;
				document.frm.cfm_use.value = cfmUse;
				document.frm.cfm_use_dept.value = cfmUseDept;
				document.frm.cfm_comment.value = cfmComment;

				if( val2=="cert" ){
					if (!confirm("재직증명서를 신청하시겠습니까?")) return;
					actionUrl = "/insa/insa_certificate_print.asp";
				}else{
					if (!confirm("경력증명서를 신청하시겠습니까?")) return;
					actionUrl = "/insa/insa_certificate_career.asp";
				}
	...
	<body>
		<div id="wrap">
			...
			<div id="container">
				...
				<form action="/insa/insa_confirm_list.asp?ck_sw=n" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>
						<dt>조건 검색</dt>
                        <dd>
							...
							<strong>조건 : </strong>
                                ...
                                <a href="#" onclick="javascript:frmcheck();"><img src="/image/but_ser1.jpg" alt="검색">&nbsp;조건입력후 검색버튼을 꼭 클릭하십시요!</a>
                            </p>
						</dd>
					</dl>
				</fieldset>
                  <input type="hidden" name="in_empno" value="<%=emp_no%>">
                  <input type="hidden" name="in_name" value="">
                  <input type="hidden" name="cfm_use" value="">
                  <input type="hidden" name="cfm_use_dept" value="">
                  <input type="hidden" name="cfm_comment" value="">
			</form>
				<div class="gView">
						...
						if view_condi <> "" then
	           			...
	           			%>
							<tr>
                                ...
                                <td><%=rsConf("emp_birthday")%>&nbsp;</td>
                                <td><%=rsConf("org_company")%>&nbsp;</td>
                                <td><%=rsConf("org_name")%>&nbsp;</td>
                                <td class="left">
                                <select name="cfm_use" value="<%=cfm_use%>" style="width:130px">
                                    ...
                                    <option value='적격심사서류제출용' <%If cfm_use = "적격심사서류제출용" then %>selected<% end if %>>적격심사서류제출용</option>
                                </select>
                                </td>
                                <td class="left">
								<input name="cfm_use_dept" type="text" style="width:100px" onKeyUp="checklength(this,30)" value="<%=cfm_use_dept%>">
                                </td>
                                <td class="left">
								<input name="cfm_comment" type="text" style="width:170px" onKeyUp="checklength(this,50)" value="<%=cfm_comment%>">
                                </td>
                                <td>
                                <input type="image" name="rptCert$ctl00$btnRequest" src="/image/b_certifi.jpg" alt="재직증명서 신청" onclick="s_sinchung('<%=rsConf("emp_no")%>','cert');return false;" style="border-width:0px;" />
                                </td>
                                <td>
                                <input type="image" name="rptCert$ctl01$btnRequest" src="/image/b_certifi.jpg" alt="경력증명서 신청" onclick="s_sinchung('<%=rsConf("emp_no")%>','carrer');return false;" style="border-width:0px;" />
                                </td>
							</tr>
						<%
							rsConf.movenext()
