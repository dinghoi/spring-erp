	...
	<body>
		<div id="wrap">
			...
			<div id="container">
				...
				<form action="/insa/insa_appoint_mg.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>
						<dt>◈조건 검색◈</dt>
				...
				<div class="gView">
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						If view_condi <> "" Then
							objBuilder.Append "SELECT emtt.emp_org_baldate, emtt.emp_grade_date, emtt.emp_type, emtt.emp_no, emtt.emp_name, "
							objBuilder.Append "	emtt.emp_grade, emtt.emp_job, emtt.emp_position, emtt.emp_in_date, emtt.emp_org_name, "
							objBuilder.Append "	emtt.emp_first_date, emtt.emp_reside_place, emtt.emp_birthday, emtt.emp_company, emtt.emp_bonbu, "
							objBuilder.Append "	emtt.emp_saupbu, emtt.emp_team, "
						...
						%>
							<tr>
                                ...
                                <td>
                                <a href="#" onClick="pop_Window('/insa/insa_card00.asp?emp_no=<%=rs("emp_no")%>&be_pg=<%=be_pg%>&page=<%=page%>&view_sort=<%=view_sort%>&date_sw=<%=date_sw%>&page_cnt=<%=page_cnt%>','emp_card0_pop','scrollbars=yes,width=1250,height=650')"><%=rs("emp_name")%></a>
								</td>
                                <td><%=rs("emp_grade")%>&nbsp;</td>
                                <td><%=rs("emp_job")%>&nbsp;</td>
							    ...
							    <td>
                                <a href="/insa/insa_appoint_add.asp?emp_no=<%=rs("emp_no")%>&emp_name=<%=rs("emp_name")%>&be_pg=<%=be_pg%>&u_type=<%="U"%>">발령</a>
                                </td>
							</tr>
						<%
								rs.MoveNext()
