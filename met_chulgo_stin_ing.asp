...
if condi = "전체" then  
	   ...
	   where_sql = " WHERE (chulgo_id = '"&chulgo_id&"') and (rele_stock = '"&stock_condi&"') and (chulgo_date >= '"+from_date+"' and chulgo_date <= '"+to_date+"') "
end if
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="met_chulgo_stin_ing.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>검색</dt>
                        <dd>
                            <p>
                               <strong>의뢰창고 : </strong>
                                    ...
                                    <a href="#" class="btnType03" onClick="pop_Window('met_stockin_search.asp?gubun=<%="mv_stin"%>','stockin_search_pop','scrollbars=yes,width=600,height=400')">찾기</a>
                ...
                <h3 class="stit" style="font-size:12px;">※ 의뢰창고 찾기를 하시고 검색을 하십시요!</h3>
				<div class="gView">
				  <table cellpadding="0" cellspacing="0" class="tableList">
				    ...
				    <thead>
                      <tr>
                        ...
                        <th scope="col">의뢰(No)</th>
                        <th scope="col">의뢰창고</th>
                        <th scope="col">의뢰담당</th>
                        <th scope="col">적요</th>
				    ...
				    <tbody>
				      ...
				      <tr>
                        ...
                        <td>
						<a href="#" onClick="pop_Window('met_chulgo_cust_detail.asp?chulgo_date=<%=rs("chulgo_date")%>&chulgo_stock=<%=rs("chulgo_stock")%>&chulgo_seq=<%=rs("chulgo_seq")%>&u_type=<%=""%>','met_chulgo_detail_pop','scrollbars=yes,width=930,height=650')"><%=rs("chulgo_date")%>&nbsp;<%=rs("chulgo_seq")%></a>
                        </td>
                        ...
                        <td>
        <% if rs("chulgo_type") = "출고완료" or rs("chulgo_type") = "부분출고" then	%>
                        <a href="#" onClick="pop_Window('met_chulgo_stin_add.asp?chulgo_date=<%=rs("chulgo_date")%>&chulgo_stock=<%=rs("chulgo_stock")%>&chulgo_seq=<%=rs("chulgo_seq")%>&u_type=<%=""%>','met_move_stin_add_pop','scrollbars=yes,width=1230,height=650')">등록</a>
		<%   else	%>
								-
		<% end if	%>
                        </td>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td>
                    <div id="paging">
                        <a href="met_chulgo_stin_ing.asp?page=<%=first_page%>&condi=<%=condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="met_chulgo_stin_ing.asp?page=<%=intstart -1%>&condi=<%=condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
   	        <% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="met_chulgo_stin_ing.asp?page=<%=i%>&condi=<%=condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
   	        <% if 	intend < total_page then %>
                        <a href="met_chulgo_stin_ing.asp?page=<%=intend+1%>&condi=<%=condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[다음]</a> <a href="met_chulgo_stin_ing.asp?page=<%=total_page%>&condi=<%=condi%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
