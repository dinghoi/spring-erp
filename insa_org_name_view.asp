...
where_sql = " WHERE (isNull(org_end_date) or org_end_date = '1900-01-01' or org_end_date = '0000-00-00') and (org_company = '"&view_condi&"') and (org_name like '%"&condi&"%')"
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="insa_org_name_view.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>회사 검색</dt>
				  ...
				  <table cellpadding="0" cellspacing="0" class="tableList">
				    ...
				    <tbody>
				      ...
				      <tr>
						...
						<td><a href="#" onClick="pop_Window('insa_org_owner_modify.asp?org_code=<%=rs("org_code")%>&u_type=<%="U"%>','insa_org_modi_pop','scrollbars=yes,width=1250,height=400')">변경</a>&nbsp;</td>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td>
                    <div id="paging">
                        <a href="insa_org_name_view.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&condi=<%=condi%>&ck_sw=<%="y"%>">[처음]</a>
						<% if intstart > 1 then %>
							<a href="insa_org_name_view.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&condi=<%=condi%>&ck_sw=<%="y"%>">[이전]</a>
						<% end if %>
						<% for i = intstart to intend %>
							<% if i = int(page) then %>
								<b>[<%=i%>]</b>
							<% else %>
								<a href="insa_org_name_view.asp?page=<%=i%>&view_condi=<%=view_condi%>&condi=<%=condi%>&ck_sw=<%="y"%>">[<%=i%>]</a>
							<% end if %>
						<% next %>
						<% if 	intend < total_page then %>
							<a href="insa_org_name_view.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&condi=<%=condi%>&ck_sw=<%="y"%>">[다음]</a> <a href="insa_org_name_view.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&condi=<%=condi%>&ck_sw=<%="y"%>">[마지막]</a>
							<%	else %>
							[다음]&nbsp;[마지막]
						<% end if %>
                    </div>
