...
cost_month			= replaceXSS(Request.form("cost_month"))
cost_month_end		= replaceXSS(Request.form("cost_month_end"))
team					= replaceXSS(Request.form("team"))
saupbu				= replaceXSS(Request.form("saupbu"))
...
if cost_grade <> "0" then
	...
	if (position = "본부장" and saupbu = "") or cost_grade = "1" then
		saupbu = "본인"
...
if cost_month_end = "" then
	cost_month_end = mid(cstr(now()),1,4) + mid(cstr(now()),6,2)
end If

'sql = "select count(*) from emp_master_month where emp_month = '"&cost_month&"'"
sql = "select count(*) from emp_master_month where emp_month between '"&cost_month&"' and '" & cost_month_end & "' "
...
if total_record = 0 then
	...
	if position = "팀장" and cost_grade > "2" then
		sql = ""
		sql = sql & "select * from emp_master_month "
		sql = sql & "where 1=1 "
		'sql = sql & "and emp_month = '"&cost_month&"' "
		sql = sql & "and emp_month between '"&cost_month&"' and '" & cost_month_end & "' "
		sql = sql & sql & "and emp_team = '"&team&"' "
		sql = sql & "and (emp_end_date = '1900-01-01' or isnull(emp_end_date) or emp_end_date >= '"&from_date&"') order by emp_team, emp_name"
	  else
		if saupbu = "본인" then
			'sql = "select * from emp_master_month where emp_no = '"&user_id&"' and emp_month = '"&cost_month&"' "
			sql = "select * from emp_master_month where emp_no = '"&user_id&"' and emp_month between '"&cost_month&"' and '" & cost_month_end & "' "
		  else
			sql = "select * from emp_master_month where emp_saupbu = '"&saupbu&"' "
			'sql = sql & "and emp_month = '"&cost_month&"' "
			sql = sql & "and emp_month between '"&cost_month&"' and '" & cost_month_end & "' "
			sql = sql & "and (emp_end_date = '1900-01-01' or isnull(emp_end_date) or emp_end_date >= '"&from_date&"') "
			sql = sql & "order by emp_team, emp_name"
		end if
	end if 
end if
...
title_line = "조직별 개인별 비용사용 및 정산현황"
%>
	...
	<body>
		<div id="wrap">
			...
			<div id="container">
				...
				<form action="org_person_cal_report_jm.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>조건 검색</dt>
                        <dd>
                            <p>
								...
								&nbsp;&nbsp;<strong>발생년월&nbsp;</strong>(예201401) : 
                                	...
                                	~ <input name="cost_month_end" type="text" value="<%=cost_month_end%>" style="width:70px">
								</label>
							<% if (position = "팀장" or position = "사업부장") and cost_grade <> "0" then	%>
                                <label> 
							...
							<%
								if position = "본부장" then
									sql_org="select org_name from emp_org_mst where (org_level = '사업부') and (org_bonbu = '"&bonbu&"' or org_empno = '"&user_id&"') group by org_name order by org_saupbu asc"
								  else
...
'							if cost_sum <> 0 then
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td width="25%">
					<div class="btnCenter">
                    <a href="org_person_cal_excel.asp?cost_month=<%=cost_month%>&saupbu=<%=saupbu%>&team=<%=team%>" class="btnType04">엑셀다운로드</a>
					</div>                  
                    </td>
				    <td width="50%">
                    </td>
