			...
			function chkfrm() {
				...
				if(document.frm.out_yn.value =="Y") {
					if (eval("document.frm.user_type[1].checked")) {
						if(document.frm.reside_place1.value =="") {
							alert('상주처를 입력하세요');
							frm.reside_place.focus();
							return false;}}}
				if(document.frm.out_yn.value =="Y") {
					if (eval("document.frm.user_type[1].checked")) {
						if(document.frm.reside_company1.value =="선택") {
							alert('상주 회사를 선택하세요');
							frm.reside_company.focus();
							return false;}}}
				if(document.frm.hp.value =="") {
					alert('핸드폰 번호를 입력하세요');
					frm.hp.focus();
					return false;}
				if(document.frm.grade.value =="선택") {
					alert('관리등급을 선택하세요');
					frm.grade.focus();
					return false;}
					...
					<table cellpadding="0" cellspacing="0" class="tableWrite">
						...
						<tbody>
							...
							<tr>
								...
								<td class="left"><input name="hp" type="text" id="hp" style="width:120px" onKeyUp="checklength(this,13);checkNum(this);" value="<%=hp%>"></td>
								<th>이메일</th>
