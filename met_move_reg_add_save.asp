...
'2014-01-25 기존에 설치사진 첨부 (종료)
    ...
    if	u_type = "U" then
	        ...
	        rele_seq = "00"
			sql="select max(rele_seq) as max_no from met_mv_reg where rele_date = '"&rele_date&"' and rele_stock = '"&rele_stock&"'"
	        set rs=dbconn.execute(sql)
		
	        if	isnull(rs("max_no"))  then
		           rele_seq = "01"
	            else
		           max_seq = "00" + cstr((int(right(rs("max_no"),2)) + 1))
		           rele_seq = cstr(right(max_seq,2))
	        end if
	end if
