...
if	mg_ce = "" then
	...
	elseif sido = "대전지사" then
		...
		sql = sql + " AND (GUGUN <> '제천시' AND GUGUN <> '단양군') " ' 충북제천시와 단양군이 대전지사에서 강원지사로 배정이 변경됨 (2019.01.18)  정상원 과장 요구
	...
	elseif sido = "강원지사" then
		...
		sql = sql + " OR (GUGUN = '제천시' OR GUGUN = '단양군') " ' 충북제천시와 단양군이 대전지사에서 강원지사로 배정이 변경됨 (2019.01.18)  정상원 과장 요구
	else
		sql = "select * from as_acpt WHERE "+com_sql+type_sql+" (as_process = '접수' or as_process = '입고' or as_process = '연기')"
