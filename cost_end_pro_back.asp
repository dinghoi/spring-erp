...
' 유류비 단가 및 유류비 계산
	...
	do until rs.eof
		...
		if rs("team") = "본사팀" or rs("team") = "공사팀" or rs("team") = "RepairCenter" or rs("team") = "사업지원팀" then
			oil_unit_id = "1"
		  else
			oil_unit_id = "2"
		end if
...
' 퇴사 여부 체크		
		...
		loop
		rs.close()
		if rs_emp("emp_team") = "본사팀" or rs_emp("emp_team") = "공사팀" or rs_emp("emp_team") = "RepairCenter" or rs_emp("emp_team") = "사업지원팀" then
			oil_unit_id = "1"
		  else
			oil_unit_id = "2"
		end if
...
' 급여 SUM
	sql = "select pmg_company,pmg_bonbu,pmg_saupbu,pmg_team,pmg_org_name,pmg_id,sum(pmg_give_total - pmg_job_support) as cost,sum(pmg_give_total) as tot_cost from pay_month_give where (pmg_saupbu = '"&saupbu&"') and (pmg_yymm ='"&end_month&"') and (pmg_id ='1') group by pmg_company,pmg_bonbu,pmg_saupbu,pmg_team,pmg_org_name"
	rs.Open sql, Dbconn, 1
	do until rs.eof
...
' 2015-04-27
' 4대보험료 
		insure_tot = clng(rs("tot_cost")) * insure_tot_per / 100		
		sort_seq = 2
	...
	' 소득세 종업원분 
		income_tax = clng(rs("tot_cost")) * income_tax_per / 100		
		sort_seq = 3
	...
	' 연차수당
		annual_pay = clng(rs("tot_cost")) * annual_pay_per / 100		
		sort_seq = 4
	...
	' 퇴직충당금
		retire_pay = clng(rs("tot_cost")) * retire_pay_per / 100		
		sort_seq = 5
...
' DB SUM 야특근
	sql = "select emp_company,bonbu,saupbu,team,org_name,cost_detail,sum(overtime_amt) as cost from overtime where (cancel_yn = 'N')  and (saupbu = '"&saupbu&"') and (work_date >='"&from_date&"' and work_date <='"&to_date&"') group by emp_company,bonbu,saupbu,team,org_name,cost_detail"
	rs.Open sql, Dbconn, 1
	do until rs.eof
					
		sql = "select * from org_cost where cost_year ='"&cost_year&"' and emp_company ='"&rs("emp_company")&"' and bonbu ='"&rs("bonbu")& _
		"' and saupbu ='"&rs("saupbu")&"' and team ='"&rs("team")&"' and org_name ='"&rs("org_name")&"' and cost_id ='야특근' and cost_detail ='"&rs("cost_detail")&"'"
		set rs_cost=dbconn.execute(sql)
	
		if rs_cost.eof or rs_cost.bof then
			sql = "insert into org_cost (cost_year,emp_company,bonbu,saupbu,team,org_name,cost_id,cost_detail,cost_amt_"&cost_month&") values ('"&cost_year&"','"&rs("emp_company")&"','"&rs("bonbu")&"','"&rs("saupbu")&"','"&rs("team")&"','"&rs("org_name")&"','야특근','"&rs("cost_detail")&"',"&rs("cost")&")"
			dbconn.execute(sql)
		  else
			sql = "update org_cost set cost_amt_"&cost_month&"="&rs("cost")&" where cost_year ='"&cost_year&"' and emp_company ='"&rs("emp_company")&"' and bonbu ='"&rs("bonbu")&"' and saupbu ='"&rs("saupbu")&"' and team ='"&rs("team")&"' and org_name ='"&rs("org_name")&"' and cost_id ='야특근' and cost_detail ='"&rs("cost_detail")&"'"
			dbconn.execute(sql)
		end if		
		rs.movenext()
	loop
...
'	sql = "select emp_company,bonbu,saupbu,team,org_name,reside_place,reside_company,account,sum(cost) as cost from card_slip where (end_sw = 'Y') and (saupbu = '"&saupbu&"') and (slip_date >='"&from_date&"' and slip_date <='"&to_date&"') group by emp_company,bonbu,saupbu,team,org_name,reside_place,reside_company,account"
	sql = "select emp_company,bonbu,saupbu,team,org_name,account,sum(cost) as cost from card_slip where (saupbu = '"&saupbu&"') and (slip_date >='"&from_date&"' and slip_date <='"&to_date&"') and card_type not like '%주유%' group by emp_company,bonbu,saupbu,team,org_name,account"
	rs.Open sql, Dbconn, 1
	do until rs.eof
