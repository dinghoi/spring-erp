...
objBuilder.Append "SELECT as_company, saupbu, visit_cnt, remote_cnt, charge_per, cost_amt "
objBuilder.Append "FROM company_as "
objBuilder.Append "WHERE (as_month = '"& cost_month &"')"& condi_sql &" "
objBuilder.Append "ORDER BY as_company "

rs.Open objBuilder.ToString(), Dbconn, 1
objBuilder.Clear()

title_line = "부문공동비 AS 배분 기준 현황(변경전)"
%>
					...
					<dl>
                        ...
                        <dd>
							...
							<%
								...
								objBuilder.Append "FROM company_as "
								objBuilder.Append "WHERE (saupbu <> '') AND (as_month = '"& cost_month &"') "
