...
if condi = "" then
	  ...
	  SQL = "SELECT emp_no, emp_person1, emp_person2, emp_name, emp_grade, emp_position, "
	  SQL = SQL & "	emp_in_date, emp_org_name, emp_company, emp_bonbu, emp_saupbu, emp_team, "
	  ...
	  SQL = SQL & "WHERE (isNull(emp_end_date) or emp_end_date = '1900-01-01' or emp_end_date = '0000-00-00') "
	  SQL = SQL & "	AND org_company = '"&view_condi&"' "
	  SQL = SQL & "	AND emp_no < '900000' "
	  SQL = SQL & "	ORDER BY emp_in_date, emp_no ASC "
else
	if owner_view = "C" then
		...
		SQL = SQL & "WHERE (isNull(emp_end_date) or emp_end_date = '1900-01-01' or emp_end_date = '0000-00-00') "
		SQL = SQL & "	AND org_company = '"&view_condi&"' "
		SQL = SQL & "	AND emp_name like '%"&condi&"%' "
		SQL = SQL & "	ORDER BY emp_in_date, emp_no ASC "
	else
...
<table  border="0" cellpadding="0" cellspacing="0">
    ...
    <%
		if view_condi <> "" then
			do until rs.eof
			  ...
			  Sql = "SELECT bank_name, account_no, account_holder FROM pay_bank_account where emp_no = '"&emp_no&"'"
  ...
  <tr valign="middle" class="style11">
    ...
    <td width="400"><div align="center" class="style1"><%=rs("org_company")%>-<%=rs("org_bonbu")%>-<%=rs("org_team")%></div></td>
   </tr>
	<%
		Rs.MoveNext()
		Loop
