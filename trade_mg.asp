	...
	<body>
		<div id="wrap">
			...
			<div id="container">
				...
				<form action="trade_mg.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>
						<dt>조건 검색</dt>
				...
				<div class="gView">
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						do until rs.eof
							...
							sql_type = "select type_name from type_code where etc_type='91' and etc_seq ='"&rs("mg_group")&"'"
	           			...
	           			%>
							<tr>
								<td class="first"><%=rs("trade_code")%></td>
								<td><%=rs("group_name")%>&nbsp;</td>
								<td><%=rs("trade_name")%></td>
								...
								<td><%=view_use%></td>
								<td>
									<a href="#" onClick="pop_Window('trade_add.asp?trade_code=<%=rs("trade_code")%>&u_type=<%="U"%>','trade_add_pop','scrollbars=yes,width=750,height=360')">변경</a>
								</td>
							</tr>
						<%
							rs.movenext()
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td>
                  <div id="paging">
                        <a href = "trade_mg.asp?page=<%=first_page%>&use_sw=<%=use_sw%>&view_condi=<%=view_condi%>&condi=<%=condi%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="trade_mg.asp?page=<%=intstart -1%>&use_sw=<%=use_sw%>&view_condi=<%=view_condi%>&condi=<%=condi%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
           	<% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="trade_mg.asp?page=<%=i%>&use_sw=<%=use_sw%>&view_condi=<%=view_condi%>&condi=<%=condi%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
           	<% if 	intend < total_page then %>
                        <a href="trade_mg.asp?page=<%=intend+1%>&use_sw=<%=use_sw%>&view_condi=<%=view_condi%>&condi=<%=condi%>&ck_sw=<%="y"%>">[다음]</a> <a href="trade_mg.asp?page=<%=total_page%>&use_sw=<%=use_sw%>&view_condi=<%=view_condi%>&condi=<%=condi%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
				    ...
				    <td width="25%">
					<div class="btnRight">
					<a href="#" onClick="pop_Window('trade_add.asp','trade_add_pop','scrollbars=yes,width=800,height=360')" class="btnType04">회사및거래처등록</a>
					</div>
                    </td>
			      </tr>
				  </table>
