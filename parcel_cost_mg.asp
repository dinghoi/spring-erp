	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="parcel_cost_mg.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>조건 검색</dt>
				...
				<div class="gView">
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						do until rs.eof
							...
							belong_org = rs("emp_company") + "/" + rs("team")
							if rs("team") = "" then
								belong_org = rs("emp_company") + "/" + rs("saupbu")
							end if
							if rs("saupbu") = "" then
								belong_org = rs("emp_company") + "/" + rs("bonbu")
							end if
							customer_no = mid(rs("customer_no"),1,3) + "-" + mid(rs("customer_no"),4,2) + "-" + mid(rs("customer_no"),6)
						%>
							<tr>
								<td class="first"><%=belong_org%></td>
								<td><%=rs("company")%></td>
								<td><%=rs("slip_date")%></td>
								<td><%=rs("customer")%></td>
								<td><%=customer_no%></td>
							...
							<%   if rs("reg_id") = user_id then	%>
                                <a href="#" onClick="pop_Window('parcel_cost_add.asp?slip_date=<%=rs("slip_date")%>&slip_seq=<%=rs("slip_seq")%>&u_type=<%="U"%>','parcel_cost_add_pop','scrollbars=yes,width=800,height=280')">수정</a>
							<%     else	%>
								불가
                            <%	 end if	%>
							<%  else	%>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td width="30%">
					<div class="btnRight">
					<a href="#" onClick="pop_Window('parcel_cost_add.asp','parcel_cost_add_pop','scrollbars=yes,width=800,height=280')" class="btnType04">운반비 세금계산서 등록</a>
					</div>                  
                    </td>
			      </tr>
				  </table>
