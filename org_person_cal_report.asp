...
if total_record = 0 then
	...
	if position = "팀장" and cost_grade > "2" then
		sql = "select * from emp_master_month where emp_month = '"&cost_month&"' and emp_team = '"&team&"' and (emp_end_date = '1900-01-01' or isnull(emp_end_date) or emp_end_date >= '"&from_date&"') order by emp_team, emp_name"
	  else
		if saupbu = "본인" then
			sql = "select * from emp_master_month where emp_month = '"&cost_month&"' and emp_no = '"&user_id&"'"
		  else
			sql = "select * from emp_master_month where emp_month = '"&cost_month&"' and emp_saupbu = '"&saupbu&"' and (emp_end_date = '1900-01-01' or isnull(emp_end_date) or emp_end_date >= '"&from_date&"') order by emp_team, emp_name"
		end if
	end if 
end if
