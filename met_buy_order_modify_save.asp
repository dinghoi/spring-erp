...
'	on Error resume next
	...
	old_order_buy_no = request.form("old_order_buy_no")
	old_order_goods_type = request.form("old_order_goods_type")
	...
	for i = 1 to 20	
		...
		if code_tab(i) = "" or isnull(code_tab(i)) then
			...
			bqty_tab(i) = int(request.form("bqty"&i))
			old_qty_tab(i) = int(request.form("old_qty"&i))
		...
		if code_tab(i) = "" or isnull(code_tab(i)) then
			exit for
			  ...
			  if  not Rs_buy.eof then
			      bg_order_qty = (Rs_buy("bg_order_qty") - old_qty_tab(i)) + qty_tab(i)
			...
			exit for
			 ...
			 if qty_tab(i) <> 0 then
				 ...
				 goods_seq = right(("00" + cstr(j)),2)
		  	     sql="insert into met_order_goods (og_order_no,og_order_seq,og_order_date,og_seq,og_goods_code,og_goods_type,og_goods_gubun,og_standard,og_goods_name,og_bg_qty,og_qty,og_unit_cost,og_amt,og_ing,reg_date,reg_user) values ('"&order_no&"','"&order_seq&"','"&order_date&"','"&goods_seq&"','"&code_tab(i)&"','"&goods_type(i)&"','"&goods_gubun(i)&"','"&goods_standard(i)&"','"&goods_name(i)&"','"&bqty_tab(i)&"','"&qty_tab(i)&"','"&buy_cost(i)&"','"&buy_amt(i)&"','"&ing_type_tab(i)&"',now(),'"&user_name&"')"
