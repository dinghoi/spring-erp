...
'입고정리
			sql="select * from met_mv_in_goods where (mvin_in_date = '"&in_date&"') and (mvin_in_stock = '"&in_no&"') and (mvin_in_seq = '"&in_no_seq&"') and (in_goods_seq = '"&in_goods_seq&"') and (in_goods_code = '"&mod_goods_code&"')"
	        set Rs_in=dbconn.execute(sql)

            if not Rs_in.eof then
			       cg_qty = Rs_in("in_cg_qty")
							 
				   in_cg_qty = in_cg_qty - mod_chul_qty
							 
	               sql = "update met_mv_in_goods set in_cg_qty='"&in_cg_qty&"' where (mvin_in_date = '"&in_date&"') and (mvin_in_stock = '"&in_no&"') and (mvin_in_seq = '"&in_no_seq&"') and (in_goods_seq = '"&in_goods_seq&"') and (in_goods_code = '"&mod_goods_code&"')"
...
'    end if
	...
	if filenm <> "" then
		...
		if filenm <> "" then 
	  	...
	  	filename = old_chulgo_att_file
	end if

'입고품목 출고수량 update
    j = 1
	sql = "select * from met_mv_in_goods where (mvin_in_date = '"&in_date&"') and (mvin_in_stock = '"&in_no&"') and (mvin_in_seq = '"&in_no_seq&"') and (in_goods_seq = '"&in_goods_seq&"') and (in_goods_code = '"&code_tab(j)&"')"
	
    set Rs_stin=dbconn.execute(sql)
    if not Rs_stin.eof then
 	       cg_qty = Rs_stin("in_cg_qty")
		   cg_qty = cg_qty + qty_tab(j)
		   
           sql = "Update met_mv_in_goods set chulgo_no='"&rele_no&"',chulgo_seq='"&rele_seq&"',in_cg_qty='"&cg_qty&"' where (mvin_in_date = '"&in_date&"') and (mvin_in_stock = '"&in_no&"') and (mvin_in_seq = '"&in_no_seq&"') and (in_goods_seq = '"&in_goods_seq&"') and (in_goods_code = '"&code_tab(j)&"')"
