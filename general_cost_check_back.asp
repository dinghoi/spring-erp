...
<html>
	<head>
		...
		<script type="text/javascript">
			function chkall(){
				var tar = document.getElementsByName('confirm_yn[]');
			alert(tar);		
				Tnum = Tar.Length;
			alert("bbbb");		
				if(Tnum>0){
			alert("bbbb");		
					for (i=0;i<Tnum;I++){
			alert("dddd");		
						Tar[i].checked=(this.checked)?true:false;
				}}
			}
		</script>
		<script type="text/javascript">
		$(function(){
			$("#check_all").bclick,function(){
                if($(this).attr("checked")){
                    $('input[name="confirm_yn"]').attr("checked", true);
                }else{
                    $('input[name="confirm_yn"]').attr("checked", false);
                }
            });
        });		
        </script>
			...
			<div id="container">
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td width="20%">
					<div class="btnRight">
                    <span class="btnType04"><input type="button" value="확인저장" onclick="javascript:frm1check();" NAME="Button1"></span>
					</div>                  
                    </td>
			      </tr>
				  </table>
