...
Set Rs_jgo = Server.CreateObject("ADODB.Recordset")
...
if u_type = "U" then
	...
	j = 0
	sql = "select * from met_chulgo_reg_goods where (rele_date = '"&rele_date&"') and (rele_no = '"&rele_no&"') and (rele_seq = '"&rele_seq&"') ORDER BY rl_goods_seq,rl_goods_code ASC"
	...
	do until rs.eof
		...
		pummok_tab(8,j) = rs("rl_service_no")
		pummok_tab(9,j) = rs("rl_trade_name")
		pummok_tab(10,j) = rs("rl_trade_dept")
		pummok_tab(11,j) = rs("rl_bigo")
		
		Sql="select * from met_stock_gmaster where stock_code = '" &chulgo_stock& "' and stock_goods_code='"&rs("rl_goods_code")&"'"
		Set Rs_jgo=DbConn.Execute(Sql)
		if Rs_jgo.eof or Rs_jgo.bof then
			    amount_tab(2,j) = 0
		   else
				amount_tab(2,j) = Rs_jgo("stock_JJ_qty")
		end if
		Rs_jgo.close()
	...
	<body onload="pummok_list_view();">
		<div id="container">				
			<div class="gView">
                ...
                <form method="post" name="frm" action="met_chulgo_reg_modify_save.asp" enctype="multipart/form-data">
                    <table cellpadding="0" cellspacing="0" summary="" class="tableWrite">
						...
						<tbody>
							...
							<tr>
							  <th>출고요청일</th>
							  <td class="left"><input name="chulgo_date" type="text" value="<%=chulgo_date%>" style="width:120px;text-align:center" id="datepicker1"></td>
                              <th>출고요청창고</th>
            	...
            	<table cellpadding="0" cellspacing="0" class="tableWrite">
						...
						<thead>
							<tr>
								<th class="first; left" colspan="11" scope="col" style=" border-bottom:1px solid #e3e3e3;"><a href="#" onClick="pop_pummok_del()" class="btnType03">선택삭제</a>&nbsp;<a href="#" onClick="pop_pummok()" class="btnType03">출고의뢰 품목선택</a></th>
							</tr>
							<tr>
						...
						%>
			  				<tr id="pummok_list<%=i%>" style="display:none">
                                ...
                                <input name="goods_name<%=i%>" type="text" id="goods_name<%=i%>" value="<%=pummok_tab(4,i)%>" style="width:100px" readonly="true"></td>
								<td><input name="goods_standard<%=i%>" type="text" id="goods_standard<%=i%>" value="<%=pummok_tab(5,i)%>" style="width:100px"></td>
                                <td><input name="jqty<%=i%>" type="text" id="jqty<%=i%>" style="width:60px;text-align:right" value="<%=formatnumber(amount_tab(2,i),0)%>" readonly="true"></td>
                                ...
                                <td>
                                <input name="service_no<%=i%>" type="text" id="service_no<%=i%>" value="<%=pummok_tab(8,i)%>" style="width:80px">
                                </td>
                                <td>
                                <input name="trade_name<%=i%>" type="text" id="trade_name<%=i%>" value="<%=pummok_tab(9,i)%>" style="width:100px">
                                </td>
                                <td>
                                <input name="trade_dept<%=i%>" type="text" id="trade_dept<%=i%>" value="<%=pummok_tab(10,i)%>" style="width:100px">
                                </td>
                                <td>
                                <input name="r_bigo<%=i%>" type="text" id="r_bigo<%=i%>" value="<%=pummok_tab(11,i)%>" style="width:150px">
...
<br>
			...
			<% end if	%>        
                ...
                <input type="hidden" name="old_rele_no" value="<%=rele_no%>">
