...
be_pg = "met_import_nwin_report01.asp"
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="met_import_nwin_report01.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>검색조건</dt>
			   ...
			   %>
				      <tr>
                        ...
                        <td>
                        <a href="#" onClick="pop_Window('met_import_nwin_detail01.asp?stin_in_date=<%=stin_in_date%>&stin_order_no=<%=stin_order_no%>&stin_order_seq=<%=stin_order_seq%>&u_type=<%=""%>','met_import_nwin_detail01_pop','scrollbars=yes,width=1300,height=650')"><%=rs("stin_order_no")%>&nbsp;<%=rs("stin_order_seq")%></a>
                        </td>
                        <td><%=rs("stin_buy_company")%>(<%=rs("stin_buy_saupbu")%>)</td>
                        <td><%=rs("stin_stock_name")%></td>
                        <td class="left"><%=bg_goods_name%>&nbsp;외</td>
                        <td class="right"><%=formatnumber(rs("stin_cost"),0)%></td>
                        <td><%=rs("stin_trade_name")%></td>
                        <td><%=rs("po_date")%></td>
                        <td><%=rs("po_number")%></td>
                        <td>
                        <a href="#" onClick="pop_Window('met_import_nwin_modify01.asp?stin_in_date=<%=stin_in_date%>&stin_order_no=<%=stin_order_no%>&stin_order_seq=<%=stin_order_seq%>&u_type=<%="U"%>','met_import_nwin_modify01_pop','scrollbars=yes,width=1300,height=650')">수정</a>
                        </td>
			          </tr>
		      <%
							rs.movenext()
			  ...
			  %>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td width="20%">
					<div class="btnCenter">
                    <a href="met_import_nwin_excel01.asp?view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>" class="btnType04">엑셀다운로드</a>
					</div>                  
                  	</td>
                    <td>
                    <div id="paging">
                        <a href="met_import_nwin_report01.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="met_import_nwin_report01.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
   	        <% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="met_import_nwin_report01.asp?page=<%=i%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
   	        <% if 	intend < total_page then %>
                        <a href="met_import_nwin_report01.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[다음]</a> <a href="met_import_nwin_report01.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&from_date=<%=from_date%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
                ...
                <% if met_grade2 = "2" or user_id = "900002" then	%>				                                   
                        <a href="#" onClick="pop_Window('met_import_nwin_add01.asp?view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&u_type=<%=""%>','met_import_nwin_add01_popup','scrollbars=yes,width=1300,height=650')" class="btnType04">외자입고 등록</a>
                <% end if	%>			      
					</div>                  
                    </td>
			      </tr>
