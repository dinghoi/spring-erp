					...
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						%>
							<tr>
                              ...
                              <td><a href="#" onClick="pop_Window('insa_language_add.asp?lang_empno=<%=rs("lang_empno")%>&emp_name=<%=emp_name%>&owner_view=<%=owner_view%>&u_type=<%=""%>','insa_language_add_pop','scrollbars=yes,width=750,height=300')">등록</a></td>
							  <td><a href="#" onClick="pop_Window('insa_language_add.asp?lang_empno=<%=rs("lang_empno")%>&lang_seq=<%=rs("lang_seq")%>&emp_name=<%=emp_name%>&owner_view=<%=owner_view%>&u_type=<%="U"%>','insa_language_add_pop','scrollbars=yes,width=750,height=300')">수정</a></td>
                         <% if insa_grade = "0" then %>     
                              <td>
                              <a href="#" onClick="language_del('<%=rs("lang_empno")%>', '<%=rs("lang_seq")%>', '<%=emp_name%>', '<%=owner_view%>');return false;">삭제</a></td>
                         <%     else %>
                              <td>&nbsp;</td>
                         <% end if %>                                          
							</tr>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
					...
					<% if owner_view = "T" then 
							  ...
							  rs_emp.close()
				    %>
                    <a href="#" onClick="pop_Window('insa_language_add.asp?lang_empno=<%=view_condi%>&emp_name=<%=emp_name%>','insa_language_add_pop','scrollbars=yes,width=750,height=300')" class="btnType04">어학사항 등록</a>
					<% end if %>
                    </div>                  
                    </td>
			      </tr>
