...
' 본사 에서 입고받은 내역으로 ce에게 출고처리 하는 프로그램 입니다...met_culgo_move_add02과 유사
...
mvin_in_stock=Request("stock_code")
mvin_in_seq=Request("mvin_in_seq")
in_goods_seq=Request("in_goods_seq")
in_goods_code=Request("stock_goods_code")
...
if not Rs_jago.eof then
    code_tab(i) = Rs_jago("in_goods_code")
	goods_name(i) = Rs_jago("in_goods_name")
	goods_type(i) = Rs_jago("in_goods_type")
	goods_gubun(i) = Rs_jago("in_goods_gubun")
	goods_standard(i) = Rs_jago("in_standard")
	goods_grade(i) = Rs_jago("in_goods_grade")
	j_qty_tab(i) = Rs_jago("in_qty")
	c_qty_tab(i) = Rs_jago("in_cg_qty")
end if
Rs_jago.close()
	...
	<body onload="pummok_list_view();">
		<div id="container">				
			<div class="gView">
                ...
                <form method="post" name="frm" action="met_chulgo_move_add03_save.asp" enctype="multipart/form-data">
				<div class="gView">
            	...
            	<table cellpadding="0" cellspacing="0" class="tableWrite">
						...
						<%
							for i = 1 to 20
							     jj_qty = j_qty_tab(i) - c_qty_tab(i)
						%>
			  				<tr id="pummok_list<%=i%>" style="display:none">
                                ...
                                <td align="right"><%=formatnumber(jj_qty,0)%>
                                <input type="hidden" name="jqty<%=i%>" value="<%=formatnumber(j_qty_tab(i),0)%>" ID="Hidden1">
                                <input type="hidden" name="jj_qty<%=i%>" value="<%=formatnumber(jj_qty,0)%>" ID="Hidden1">
                                </td>
			...
			<% end if	%>                          
                ...
                <input type="hidden" name="in_goods_seq" value="<%=in_goods_seq%>">
                <input type="hidden" name="in_goods_code" value="<%=in_goods_code%>">
				</form>
                </div>
			</div>
		</div>		
