			...
			<div id="container">
				...
				<div class="gView">
					<table cellpadding="0" cellspacing="0" class="tableList">
                        ...
                        <% if rs("sign_yn") = "N" and rs("slip_stat") = "1" then	%>
                                <a href="#" onClick="pop_Window('sales_slip_add.asp?slip_id=<%=rs("slip_id")%>&slip_no=<%=rs("slip_no")%>&slip_seq=<%=rs("slip_seq")%>&u_type=<%="U"%>','sales_slip_add_pop','scrollbars=yes,width=1230,height=650')">수정</a>
						<%   else	%>
								-
						<% end if	%>
                                </td>
                        ...
                        <% if rs("sign_yn") = "N" then	%>
                                <a href="#" onClick="delcheck('<%=rs("slip_id")%>','<%=rs("slip_no")%>','<%=rs("slip_seq")%>','<%=rs("slip_stat")%>');">삭제</a>
						<%   else	%>
                        		-
						<% end if	%>
                                </td>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td width="15%">
					<div class="btnCenter">
                    <a href="sales_slip_excel.asp?sign_yn=<%="N"%>&from_date=<%=from_date%>&to_date=<%=to_date%>&slip_id=<%=slip_id%>&view_date=<%=view_date%>&field_check=<%=field_check%>&field_view=<%=field_view%>" class="btnType04">엑셀다운로드</a>
					</div>                  
                  	</td>
				    <td>
                    <div id="paging">
