...
order_Sql = " ORDER BY emp_company,emp_no,emp_in_date ASC" 

if view_condi = "전체" then
       where_sql = " WHERE (emp_end_date > '"&to_date&"' or isNull(emp_end_date) or emp_end_date = '1900-01-01' or emp_end_date = '') and (emp_no < '900000') and (emp_in_date < '"&to_date&"')"
   else
       where_sql = " WHERE (emp_end_date > '"&to_date&"' or isNull(emp_end_date) or emp_end_date = '1900-01-01' or emp_end_date = '') and (emp_company = '"&view_condi&"') and (emp_no < '900000') and (emp_in_date < '"&to_date&"')"
end if	   
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="insa_month_count_mg.asp" method="post" name="frm">
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td width="20%">
					<div class="btnCenter">
                    <a href="insa_excel_month_count.asp?view_condi=<%=view_condi%>&to_date=<%=to_date%>" class="btnType04">엑셀다운로드</a>
					</div>                  
                  	</td>
				    <td>
                    <div id="paging">
                        <a href="insa_month_count_mg.asp?page=<%=first_page%>&view_sort=<%=view_sort%>&view_condi=<%=view_condi%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="insa_month_count_mg.asp?page=<%=intstart -1%>&view_sort=<%=view_sort%>&view_condi=<%=view_condi%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
                  	<% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="insa_month_count_mg.asp?page=<%=i%>&view_sort=<%=view_sort%>&view_condi=<%=view_condi%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
                  	<% if 	intend < total_page then %>
                        <a href="insa_month_count_mg.asp?page=<%=intend+1%>&view_sort=<%=view_sort%>&view_condi=<%=view_condi%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[다음]</a> <a href="insa_month_count_mg.asp?page=<%=total_page%>&view_sort=<%=view_sort%>&view_condi=<%=view_condi%>&to_date=<%=to_date%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
