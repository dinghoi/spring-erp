...
be_pg = "insa_pay_msum_report.asp"
...
do until rs.eof
    ...
    sum_research_pay 	= sum_research_pay + int(rs("pmg_research_pay"))
    ...
    sum_give_tot 		= sum_give_tot     + int(rs("pmg_give_total"))
	  ...
	  pmg_research_pay = rs("pmg_research_pay")
...
if view_condi = "전체" then
	Sql = "select * from pay_month_deduct where (de_yymm = '"+pmg_yymm+"' ) and (de_id = '1')"
else
	Sql = "select * from pay_month_deduct where (de_yymm = '"+pmg_yymm+"' ) and (de_id = '1') and (de_company = '"+view_condi+"')"
end if
'Response.write Sql&"<br>"
...
'Response.write sum_nps_amt&"<br>"    '국민연금
'Response.write sum_nhis_amt&"<br>"    '건감보험
'Response.write sum_epi_amt&"<br>" '고용보험
'Response.write sum_longcare_amt&"<br>" '장기요양보험
'Response.write sum_income_tax&"<br>" '소득세
'Response.write sum_wetax&"<br>" '지방소득세
'Response.write sum_other_amt1&"<br>" '기타공제
'Response.write sum_sawo_amt&"<br>" '경조회비
'Response.write sum_hyubjo_amt&"<br>" '협조비
'Response.write sum_school_amt&"<br>" '학자금대출
'Response.write sum_nhis_bla_amt&"<br>" '건강보험료정산
'Response.write sum_long_bla_amt&"<br>" '장기요양보험정산
'Response.write sum_year_incom_tax&"<br>" '연말정산소득세
'Response.write sum_year_wetax&"<br>" '연말정산지방세
'Response.write sum_year_incom_tax2&"<br>" '연말재정산소득세
'Response.write sum_year_wetax2&"<br>" '연말재정산지방세


sum_deduct_tot = sum_nps_amt+sum_nhis_amt+sum_epi_amt+sum_longcare_amt+sum_income_tax+sum_wetax+sum_other_amt1+sum_sawo_amt+sum_hyubjo_amt+sum_school_amt+sum_nhis_bla_amt+sum_long_bla_amt+sum_year_incom_tax+sum_year_wetax+sum_year_incom_tax2+sum_year_wetax2

sum_curr_pay = sum_give_tot - sum_deduct_tot

curr_yyyy = mid(cstr(pmg_yymm),1,4)
curr_mm = mid(cstr(pmg_yymm),5,2)
title_line = cstr(curr_yyyy) + "년 " + cstr(curr_mm) + "월 " + " 급여 지급항목별 집계"
	...
	<body oncontextmenu="return false" ondragstart="return false" onselectstart="return false">
		<div id="wrap">
			...
			<div id="container">
				...
				<form action="insa_pay_msum_report.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>
						<dt>◈ 검색◈</dt>
					...
					<table cellpadding="0" cellspacing="0" class="tableList">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <th style="border-bottom:2px solid #515254; background:#F5FFFA">연구비</th>
								<td class="right" style=" border-bottom:2px solid #515254;"><%=formatnumber(sum_research_pay,0)%></td>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				  	<td width="25%">
				  		<div class="btnleft">
				  			<a href="insa_pay_msum_excel.asp?view_condi=<%=view_condi%>&pmg_yymm=<%=pmg_yymm%>&to_date=<%=to_date%>" class="btnType04">엑셀다운로드</a>
				  		</div>
				  	</td>
                    <td width="50%"></td>
				    <td width="25%">
