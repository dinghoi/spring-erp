...
'	on Error resume next
	...
	dev_inst_cnt = cint(abc("dev_inst_cnt"))
...
'	end if
	...
	if (filenm1.length + filenm2.length + filenm3.length + filenm4.length + filenm4.length) > 1024*1024*50  then 
    	response.write "<script language=javascript>"
      	response.write "alert('파일 용량 50M를 넘으면 안됩니다.');"
	...
	if week_day = 1 or week_day = 7 then
		...
		sql = "select * from holiday where holiday = '"&visit_date&"'"
	...
	if	juso_mod_ck = "1" then		
		sql = "Update as_acpt set sido ='"&sido&"',gugun ='"&gugun&"',dong ='"&dong&"',addr ='"&addr&"',mg_ce_id='"&user_id& _
		"',mg_ce='"&user_name&"',visit_date='"&visit_date&"',visit_time='"&visit_time&"',as_process='완료',as_type='"&as_type& _
		"',as_history='"&as_history&"',dev_inst_cnt="&dev_inst_cnt&",ran_cnt="&ran_cnt& ",work_man_cnt="&work_man_cnt&",alba_cnt="&alba_cnt& _
		",reside_place='"&reside_place&"',team='"&team&"',reside='"&reside&"',reside_company='"&reside_company& _
		"',mod_date=now(),mod_id='"&user_id&"',before_process='접수' where acpt_no = "&int(acpt_no)
		dbconn.execute(sql)
		sql="insert into old_juso (acpt_no,sido,gugun,dong,addr) values ('"&acpt_no&"','"&o_sido&"','"&o_gugun&"','"&o_dong&"','"&o_addr&"')"
		dbconn.execute(sql)
	  else
		sql = "Update as_acpt set mg_ce_id='"&user_id&"',mg_ce='"&user_name&"',visit_date='"&visit_date&"',visit_time='"&visit_time& _
		"',as_process='완료',as_type='"&as_type&"',as_history='"&as_history&"',dev_inst_cnt="&dev_inst_cnt&",ran_cnt="&ran_cnt& _
		",work_man_cnt="&work_man_cnt&",alba_cnt="&alba_cnt&",reside_place='"&reside_place&"',team='"&team&"',reside='"&reside& _
		"',reside_company='"&reside_company&"',mod_date=now(),mod_id='"&user_id&"',before_process='접수' where acpt_no = "&int(acpt_no)
		dbconn.execute(sql)
	end if
	...
	if work_man_cnt < 2 then
		...
		sql="insert into ce_work(acpt_no,mg_ce_id,work_id,work_date,holiday_yn,as_type,company,emp_company,bonbu,saupbu,team,org_name"& _
		...
		"','"&user_id&"','2','"&visit_date&"','"&holiday_yn&"','"&as_type&"','"&company&"','"&emp_company&"','"&bonbu&"','"&saupbu& _
	...
	sql = "insert into att_file (acpt_no,company,dept,sido,gugun,mg_ce_id,mg_ce,mg_group,visit_date,as_type,att_file1,att_file2,att_file3"& _
	",att_file4,att_file5) values ('"&acpt_no&"','"&company&"','"&dept&"','"&sido&"','"&gugun&"','"&user_id&"','"&user_name&"','"&mg_group& _
	"','"&visit_date&"','"&as_type&"','"&filename1&"','"&filename2&"','"&filename3&"','"&filename4&"','"&filename5&"')"
	dbconn.execute(sql)
