...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			//검색폼 체크
			function chkfrm() {
				if ($("#srchCompany optiopn:selected").val() == "") {
					alert ("회사를 선택하시기 바랍니다");
					$("#srchCompany").focus();
					return false;
				}
				//if ($("#pmg_yymm_start optiopn:selected").val() == "") {
				if ($("#pmg_yymm optiopn:selected").val() == "") {
					alert ("검색기간을 선택하시기 바랍니다");
					$("#pmg_yymm_start").focus();
					return false;
				}
				return true;
			}
					...
					<dl>					
                        ...
                        <dd>
                            <p>
								...
								<label>
								<strong>검색기간 : </strong>
                                  <select name="emp_month" id="emp_month" type="text" style="width:90px">
                                    <%	for i = 24 to 1 step -1	%>
                                    <option value="<%=month_tab(i,1)%>" <%If emp_month = month_tab(i,1) then %>selected<% end if %>><%=month_tab(i,2)%></option>
                                    <%	next	%>
                                 </select>
								</label>
								<!--
								...
								<a href="insa_pay_total_info_excel.asp?srchCompany=<%=srchCompany%>&emp_month=<%=emp_month%>" class="btnType04">엑셀다운로드</a>
					...
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						<thead>
							<tr>
								...
								<th scope="col" colspan="2" style=" border-bottom:1px solid #e3e3e3;"><%=mid(emp_month_prev,1,4)%>년&nbsp;<%=mid(emp_month_prev,5,2)%>월
								<!--
								&nbsp;<%=mid(pmg_yymm_start,5,2)%>월
									~ <%=mid(pmg_yymm_end,1,4)%>년&nbsp;<%=mid(pmg_yymm_end,5,2)%>월
								-->
								</th>
								<th scope="col"  colspan="2" style=" border-bottom:1px solid #e3e3e3;"><%=mid(emp_month,1,4)%>년&nbsp;<%=mid(emp_month,5,2)%>월
								<!--
									~ <%=mid(pmg_yymm_end,1,4)%>년&nbsp;<%=mid(pmg_yymm_end,5,2)%>월
								-->
