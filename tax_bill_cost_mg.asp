...
base_sql = "select * from general_cost where (slip_date >='"&from_date&"' and slip_date <='"&to_date&"') and (slip_gubun ='비용') and (tax_bill_yn = 'Y') "
order_sql = " ORDER BY slip_date ASC"

sql = base_sql + posi_sql + order_sql
Rs.Open Sql, Dbconn, 1

title_line = "계산서 일반 비용관리"
%>
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="tax_bill_cost_mg.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>조건 검색</dt>
					...
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						%>
							<tr>
								...
								<td><%=rs("account")%>-<%=rs("account_item")%></td>
								<td><%=rs("slip_memo")%></td>
								<td><%=rs("reg_user")%></td>
								<td>
							<% if rs("end_yn") = "C" or rs("end_yn") = "N" then %>
							<%   if rs("reg_id") = user_id then	%>
                                <a href="#" onClick="pop_Window('tax_bill_cost_add.asp?slip_date=<%=rs("slip_date")%>&slip_seq=<%=rs("slip_seq")%>&u_type=<%="U"%>','tax_bill_cost_add_pop','scrollbars=yes,width=800,height=280')">수정</a>
							<%     else	%>
								불가
                            <%	 end if	%>
							<%  else	%>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td width="20%">
					<div class="btnCenter">
                    <a href="tax_bill_cost_excel.asp?from_date=<%=from_date%>&to_date=<%=to_date%>&slip_gubun=<%="비용"%>" class="btnType04">엑셀다운로드</a>
					</div>
                    </td>                
				    <td width="50%">
                    </td>
				    <td width="30%">
					<div class="btnRight">
					<a href="#" onClick="pop_Window('tax_bill_cost_add.asp','tax_bill_cost_add_pop','scrollbars=yes,width=800,height=280')" class="btnType04">계산서 일반 비용 등록</a>
					</div>                  
                    </td>
			      </tr>
				  </table>
