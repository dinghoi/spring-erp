...
'							acpt_date = replace(acpt_date,"-","/")
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td width="15%">
					<div class="btnCenter">
                    <a href="excel_down_asset.asp?from_date=<%=from_date%>&to_date=<%=to_date%>&date_sw=<%=date_sw%>&process_sw=<%=process_sw%>&field_check=<%=field_check%>&field_view=<%=field_view%>" class="btnType04">엑셀다운로드</a>
					</div>
                  	</td>
				    <td>
                    <div id="paging">
                        <a href="as_list_asset.asp?page=<%=first_page%>&from_date=<%=from_date%>&to_date=<%=to_date%>&date_sw=<%=date_sw%>&process_sw=<%=process_sw%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>&company=<%=company%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="as_list_asset.asp?page=<%=intstart -1%>&from_date=<%=from_date%>&to_date=<%=to_date%>&date_sw=<%=date_sw%>&process_sw=<%=process_sw%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>&company=<%=company%>">[이전]</a>
                      <% end if %>
