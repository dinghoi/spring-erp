	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="account_grade_mg.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>조건 검색</dt>
				...
				<div class="gView">
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						i = 0
						do until rs.eof
							if rs("account_grade") = 0 then
								account_grade_view = "사용가능"
							  else
								account_grade_view = "권한없음"
							end if
	           			...
	           			%>
							<tr>
								...
								<td><%=account_grade_view%></td>
								<td><a href="#" onClick="pop_Window('account_grade_mod.asp?user_id=<%=rs("user_id")%>&u_type=<%="U"%>','cost_grade_pop','scrollbars=no,width=800,height=170')">변경</a></td>
							</tr>
						<%
							rs.movenext()
						loop
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td>
                  <div id="paging">
                        <a href = "account_grade_mg.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&condi=<%=condi%>&use_yn=<%=use_yn%>&emp_yn=<%=emp_yn%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="account_grade_mg.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&condi=<%=condi%>&use_yn=<%=use_yn%>&emp_yn=<%=emp_yn%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
       	<% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                    <a href="account_grade_mg.asp?page=<%=i%>&view_condi=<%=view_condi%>&condi=<%=condi%>&use_yn=<%=use_yn%>&emp_yn=<%=emp_yn%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
       	<% if 	intend < total_page then %>
                        <a href="account_grade_mg.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&condi=<%=condi%>&use_yn=<%=use_yn%>&emp_yn=<%=emp_yn%>&ck_sw=<%="y"%>">[다음]</a> <a href="account_grade_mg.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&condi=<%=condi%>&use_yn=<%=use_yn%>&emp_yn=<%=emp_yn%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
