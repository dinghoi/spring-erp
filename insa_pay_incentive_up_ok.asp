...
'	on Error resume next
    ...
    if rowcount > -1 then
        for i=2 to (rowcount-1)
            ...
            ' 지급항목
            pmg_base_pay     = toString(xgr(11,i),"0") ' 지급액계

            pmg_base_pay	  = 0
            pmg_meals_pay	  = 0
            pmg_research_pay  = 0	'연구수당 (신규추가)
            pmg_postage_pay	  = 0
            pmg_re_pay		  = 0
            pmg_overtime_pay  = 0
            pmg_car_pay		  = 0
			...
			sql = "select * from pay_month_give where pmg_yymm = '"&pmg_yymm&"' and pmg_id = '2' and pmg_emp_no = '"&emp_no&"' and pmg_company = '"&pmg_company&"'"
			...
			if Rs_give.eof or Rs_give.bof then
				sql="INSERT INTO pay_month_give "&_
				    ...
				    ")  "&_
				    "VALUES "&_
				    "(  '"&pmg_yymm&"', '2', '"&emp_no&"', '"&pmg_company&"', '"&pmg_date&"' "&_
...
'response.write(sql&"<br>")
		...
		sql = "select * from pay_month_deduct where de_yymm = '"&pmg_yymm&"' and de_id = '2' and de_emp_no = '"&emp_no&"' and de_company = '"&pmg_company&"'"
		set Rs_dct=dbconn.execute(sql)				
		if Rs_dct.eof or Rs_dct.bof then
