...
<%
    ...
    search_value = unescape(request.form("search[value]"))
    ...
    ' 사업부
    if saupbu <> "" then
        sql_saupbu_cond = " AND saupbu = '"& saupbu &"' " 
    Else
        sql_saupbu_cond = " "
    end if
    ...
    if search_value <> "" then
        sql_search_cond = " AND company like '%"& search_value &"%' " 
    Else
        sql_search_cond = " "
    end if

    recordsTotal = 0 ' 총 레코드수 (페이징을 위하여......)

    sql = "SELECT count(*) AS recordsTotal                                                    " & chr(13) &_ 
          "  FROM (                                                                           " & chr(13) &_ 
          "           SELECT saupbu, company                                                  " & chr(13) &_ 
          "             FROM saupbu_sales a                                                   " & chr(13) &_ 
          "            WHERE ( sales_date >= '"& date1 &"' AND sales_date <= '"& date2 &"' )  " & chr(13) &_ 
          "              AND saupbu <> '' AND company <> ''                                   " & chr(13) &_ 
                         sql_saupbu_cond                                                        & chr(13) &_ 
                         sql_search_cond                                                        & chr(13) &_ 
          "         GROUP BY saupbu, company                                                  " & chr(13) &_ 
          "       ) A                                                                         "          

          ' 디버깅 용...
          sql_debug = "insert into temp_debug  (debug_text) values ('"& Replace(sql, "'", "''")     &"') "
          dbconn.execute(sql_debug)
    ...
    DataTablesQueryToJSON(dbconn, sql, recordsTotal, recordsTotal).Flush
