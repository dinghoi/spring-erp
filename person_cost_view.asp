...
if cost_id = "인건비" and cost_detail = "기타" then
...
if cost_id = "교통비" and cost_detail = "차량수리비" then
	sql = "select org_name,run_date as slip_date,user_name,user_grade,run_memo as slip_memo,repair_cost as cost FROM transit_cost where (cancel_yn = 'N') and (run_date >= '"&from_date&"' and run_date <= '"&to_date&"') and team = '"&team&"' and car_owner = '회사'"&tran_com_sql&tran_bonbu_sql&tran_saupbu_sql&" order by org_name,user_name,run_date asc"
	rs.Open sql, Dbconn, 1
end if

if cost_id = "법인카드" then
	sql = "select card_slip.org_name as org_name,card_slip.slip_date,memb.user_name,memb.user_grade,concat(card_slip.customer,'-',card_slip.account_item) as slip_memo,card_slip.cost FROM card_slip INNER JOIN memb ON card_slip.emp_no = memb.user_id where card_type not like '%주유%' and (slip_date >= '"&from_date&"' and slip_date <= '"&to_date&"') and card_slip.team = '"&team&"' and card_slip.account = '"&cost_detail&"'"&card_com_sql&card_bonbu_sql&card_saupbu_sql&" order by card_slip.org_name,memb.user_name, card_slip.slip_date asc"
	rs.Open sql, Dbconn, 1
end if
