...
be_pg = "insa_pay_year_nhis_mg.asp"
...
if condi = "" then
       Sql = "select * from emp_master where (isNull(emp_end_date) or emp_end_date = '1900-01-01' or (emp_end_date >= '"&st_es_date&"' and emp_end_date < '"&st_in_date&"')) and (emp_company = '"+view_condi+"')  and (emp_no < '900000') ORDER BY emp_no ASC limit "& stpage & "," &pgsize 
   else 
       if owner_view = "C" then  
             Sql = "select * from emp_master where (isNull(emp_end_date) or emp_end_date = '1900-01-01' or (emp_end_date >= '"&st_es_date&"' and emp_end_date < '"&st_in_date&"')) and (emp_company = '"+view_condi+"') and (emp_name like '%"+condi+"%') ORDER BY emp_no ASC limit "& stpage & "," &pgsize 
          else
             Sql = "select * from emp_master where (isNull(emp_end_date) or emp_end_date = '1900-01-01' or (emp_end_date >= '"&st_es_date&"' and emp_end_date < '"&st_in_date&"')) and (emp_company = '"+view_condi+"') and (emp_no = '"+condi+"') ORDER BY emp_no ASC limit "& stpage & "," &pgsize 
	   end if
end if

Rs.Open Sql, Dbconn, 1

title_line = "건강보험 표준월액 "
%>
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="insa_pay_year_nhis_mg.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>◈ 검색◈</dt>
					...
					<table cellpadding="0" cellspacing="0" class="tableList">
                        ...
                        <%
                          ...
                          %>
                                ...
                                <td><a href="#" onClick="pop_Window('insa_pay_year_nhis_add.asp?emp_no=<%=rs("emp_no")%>&emp_name=<%=rs("emp_name")%>&inc_yyyy=<%=inc_yyyy%>&u_type=<%="U"%>','insa_income_add_pop','scrollbars=yes,width=800,height=250')">등록</a></td>
							</tr>
						<%
							rs.movenext()
						loop
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td>
                    <div id="paging">
                        <a href = "insa_pay_year_nhis_mg.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&inc_yyyy=<%=inc_yyyy%>&owner_view=<%=owner_view%>&condi=<%=condi%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="insa_pay_year_nhis_mg.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&inc_yyyy=<%=inc_yyyy%>&owner_view=<%=owner_view%>&condi=<%=condi%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
           	<% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="insa_pay_year_nhis_mg.asp?page=<%=i%>&view_condi=<%=view_condi%>&inc_yyyy=<%=inc_yyyy%>&owner_view=<%=owner_view%>&condi=<%=condi%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
           	<% if 	intend < total_page then %>
                        <a href="insa_pay_year_nhis_mg.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&inc_yyyy=<%=inc_yyyy%>&owner_view=<%=owner_view%>&condi=<%=condi%>&ck_sw=<%="y"%>">[다음]</a> <a href="insa_pay_year_nhis_mg.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&inc_yyyy=<%=inc_yyyy%>&owner_view=<%=owner_view%>&condi=<%=condi%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
                    ...
                    <% if owner_view = "T" then 
							  ...
							  rs_emp.close()
				    %>
					<a href="#" onClick="pop_Window('insa_pay_year_nhis_add.asp?emp_no=<%=emp_no%>&inc_yyyy=<%=inc_yyyy%>&emp_name=<%=emp_name%>&u_type=<%=""%>','insa_income_add_pop','scrollbars=yes,width=800,height=250')" class="btnType04">건강보험표준월액 등록</a>
					</div> 
                    <% end if %>                 
                    </td>
			      </tr>
