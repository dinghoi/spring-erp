...
objBuilder.Append "CALL USP_COST_END_MG_SEL('"&emp_no&"', '"&cost_grade&"');"
...
<html>
			...
			function setCostEnd(end_url, end_month, dept, end_yn, type){
				...
				$.ajax({
					...
					, success: function(data){
						...
						//alert(data);
						//location.href="/cost_end/cost_end_mg.asp";
						return;
					}
					, beforeSend: function(){
						var width = 0;
	...
	<body>
		<div id="wrap">
					...
					<dl>
						...
						<dd>
							<p>
								...
								(영업부 월마감,인사부 조직코드마감, 인사마감[<%=rsEmpOrgList("max_org_month")%>] 확인)
							</p>
						</dd>
					</dl>
				</fieldset>
						...
						<%
							...
							Do Until rsEmpOrgList.EOF
								...
								'Else
							  	'	jik_yn = "N"
								'End If
								objBuilder.Append "CALL USP_COST_END_ORG_SEL('"&rsEmpOrgList("org_name")&"');"
							...
							<tr>
								...
								<td><%=reg_date%>&nbsp;</td>
							  	<td>
									<%
									If cancel_yn = "Y" Then
										'Response.write "<a href='/cost_end/cost_end_cancel.asp?saupbu="&rsEmpOrgList("org_name")&"&end_month="&end_month&"' class='btnType03'>마감취소</a>"
										Response.Write "<a href='#' onclick='setCostEnd(""/cost_end/cost_end_cancel.asp"", """&end_month&""", """&rsEmpOrgList("org_name")&""", """", ""C"");' class='btnType03'>마감</a>"
									Else
										Response.write "취소불가"
									End If
									%>
								...
								<td>
									...
									If now_month > new_month Then
										...
										Response.Write "<a href='#' onclick='setCostEnd(""/cost_end/cost_end_pro.asp"", """&new_month&""", """&rsEmpOrgList("org_name")&""", """&end_yn&""", ""S"");' class='btnType03'>마감</a>"
									Else
										Response.write "마감불가"
									End If
									%>
							...
							'=====	사업부외나머지	=====
							Dim rsCostEndEtcList

							If cost_grade = "0" Then
								objBuilder.Append "USP_COST_END_ORG_SEL('사업부외나머지');"
							...
							<tr bgcolor="#FFE8E8">
								...
								<td><%=reg_date%>&nbsp;</td>
							  	<td>
									<%
									If cancel_yn = "Y" Then
										Response.write "<a href='/cost_end/cost_bonbu_end_cancel.asp?saupbu=사업부외나머지&end_month="&end_month&"' class='btnType03'>마감취소</a>"
									Else
										Response.write "취소불가"
									End If
									%>
								...
								<td>
									...
									If now_month > new_month Then
										...
										Response.Write "<a href='#' onclick='setCostEnd(""/cost_end/cost_bonbu_end_pro.asp"", """&new_month&""", """", """&end_yn&""", ""S"");' class='btnType03'>마감</a>"
									Else
										Response.write "마감불가"
									End If
									%>
							...
							If resideEndViewYn = "Y" Then
								Dim rsCostEndResideList

								objBuilder.Append "CALL USP_COST_END_ORG_SEL('상주비용');"
							...
							<tr bgcolor="#FFFFCC">
							  ...
							  <td>
									<%
									If cancel_yn = "Y" Then
										Response.Write "<a href='/cost_end/company_cost_end_cancel.asp?end_month="&end_month&"'  class='btnType03'>마감취소</a>"
									Else
										Response.Write "취소불가"
									End If
									%>
								...
								<td>
									...
									If now_month > new_month Then
										...
										Response.Write "<a href='#' onclick='setCostEnd(""/cost_end/company_cost_end_pro.asp"", """&new_month&""", """", """&end_yn&""", ""S"");' class='btnType03'>마감</a>"
									Else
										Response.Write "마감불가"
									End If
									%>
								...
								objBuilder.Append "CALL USP_COST_END_ORG_SEL('공통비/직접비배분');"
							...
							<tr bgcolor="#CCFFFF">
								...
								<td><%=reg_date%>&nbsp;</td>
							  	<td>
							  		<%
							  		If cancel_yn = "Y" Then
							  			Response.Write "<a href='/cost_end/company_as_sum_cancel.asp?end_month="&end_month&"' class='btnType03'>마감취소</a>"
									Else
										Response.Write "취소불가"
									End If
									%>
								...
								<td>
									<%
									If now_month > new_month Then
										Response.Write "<a href='/cost_end/company_as_sum_pro.asp?end_month="&new_month&"&end_yn="&end_yn&"' class='btnType03'>마감</a>"
										'Response.Write "마감(작업중)"
									Else
										Response.Write "마감불가"
									End If
									%>
			...
			<div>
				<!--<a href='/cost/cost_end_proc.asp?end_month=<%''=new_month%>' class='btnType03'>마감</a>-->
				<a href='#'  onclick="setCostEnd('/cost_end/cost_end_all.asp', '<%=new_month%>', '', '', 'A');" class='btnType03'>마감</a>
			</div>
		</div>
	</div>
	</body>
