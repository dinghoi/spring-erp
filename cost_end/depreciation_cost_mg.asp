...
SQL = "SELECT sum(price) as price,sum(cost) as cost,sum(cost_vat) as cost_vat "
SQL = SQL & "FROM general_cost "
SQL = SQL & "WHERE (slip_gubun = '상각비') "
SQL = SQL & "	and (slip_date >='"&from_date&"' and slip_date <='"&to_date&"') " & posi_sql & gubun_sql
	...
	<body>
		<div id="wrap">
			...
			<div id="container">
				...
				<form action="/cost/depreciation_cost_mg.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>
						<dt>조회조건</dt>
					...
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						%>
							...
							<%   if (rs("reg_id") = user_id) or (rs("emp_no") = user_id) or cost_grade = "0" or position ="사업부장" or position = "본부장"  then	%>
                                <a href="#" onClick="pop_Window('/cost/depreciation_cost_add.asp?slip_date=<%=rs("slip_date")%>&slip_seq=<%=rs("slip_seq")%>&u_type=<%="U"%>','depreciation_cost_add_pop','scrollbars=yes,width=800,height=200')">수정</a>
							<%     else	%>
								불가
                            <%	 end if	%>
							<%  else	%>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td>
                    <div id="paging">
                        <a href="/cost/depreciation_cost_mg.asp?page=<%=first_page%>&slip_month=<%=slip_month%>&account=<%=account%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="/cost/depreciation_cost_mg.asp?page=<%=intstart -1%>&slip_month=<%=slip_month%>&account=<%=account%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
<% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="/cost/depreciation_cost_mg.asp?page=<%=i%>&slip_month=<%=slip_month%>&account=<%=account%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
<% if 	intend < total_page then %>
                        <a href="/cost/depreciation_cost_mg.asp?page=<%=intend+1%>&slip_month=<%=slip_month%>&account=<%=account%>&ck_sw=<%="y"%>">[다음]</a>
                        <a href="/cost/depreciation_cost_mg.asp?page=<%=total_page%>&slip_month=<%=slip_month%>&account=<%=account%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
				    ...
				    <td width="24%">
					<div class="btnRight">
					<a href="#" onClick="pop_Window('/cost/depreciation_cost_add.asp','depreciation_cost_add_pop','scrollbars=yes,width=800,height=200')" class="btnType04">상각비등록</a>
					</div>
                    </td>
			      </tr>
				  </table>
