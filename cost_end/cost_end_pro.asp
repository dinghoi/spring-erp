...
org_company	=	f_Request("org_company")
...
%>
<!--#include virtual="/cost_end/inc/inc_cost_end_oil.asp" -->
<%
'개인 경비 정산(교통비, 야특근, 카드)
...
emp_cnt = 0
%>
<!--#include virtual="/cost_end/inc/inc_cost_end_person.asp" -->
<%
'월별 인사마스터 구성 여부 파악
If emp_cnt > 0 Then
	'4대보험 및 급여 SUM 처리
...
%>
	<!--#include virtual="/cost_end/inc/inc_cost_end_sum_insure.asp" -->
<%
	'상여/알바비 SUM 처리
	Dim rsBonus, arrBonus, rsAlba, arrAlba, company
%>
	<!--#include virtual="/cost_end/inc/inc_cost_end_sum_bonus.asp" -->
<%
	'DB SUM 일반 경비
	Dim rsGeneral, rs_endGeneral, rsGeneralEnd, arrGeneralEnd, slip_date, slip_seq
	Dim arrGeneral, account, rsEctCost, arrEtcCost
%>
	<!--#include virtual="/cost_end/inc/inc_cost_end_sum_cost.asp" -->
<%
	'DB SUM 교통비
	Dim rsTransit, rsRepair, arrTransit, arrRepair
%>
	<!--#include virtual="/cost_end/inc/inc_cost_end_sum_transit.asp" -->
<%
	'카드비용 집계
	Dim rsCardTran, rsCardSlip, arrCardTran, arrCardSlip
%>
	<!--#include virtual="/cost_end/inc/inc_cost_end_sum_card.asp" -->
<%
