...
order_Sql = " ORDER BY goods_type,goods_group,goods_code ASC"
	...
	<body onLoad="condi_view()">
		<div id="wrap">
			...
			<div id="container">
				...
				<form action="met_goods_code_mg.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>
						<dt> 품목 검색</dt>
				  ...
				  <table cellpadding="0" cellspacing="0" class="tableList">
				    ...
				    <thead>
				      <tr>
                        ...
                        <th scope="col">품목명</th>
                        <th scope="col">규격</th>
                        <th scope="col">모델/Part_No.</th>
                        <th scope="col">상세설명</th>
                        <th scope="col">등록일</th>
                        <th scope="col">적정<br>재고</th>
                        <th scope="col">상태</th>
                        <th scope="col">사용</th>
                        <th scope="col">수정</th>
			          </tr>
              ...
              <% end if %>
                        ...
                        <td><%=rs("goods_date")%>&nbsp;</td>
              <% if rs("goods_security_yn") = "Y" then  %>
                        <td class="right" style="text-align:right"><%=formatnumber(rs("goods_security_cnt"),0)%>&nbsp;</td>
              <%      else %>
                        <td>&nbsp;</td>
              <% end if %>
                        <td><%=rs("goods_grade")%>&nbsp;</td>
                        <td><%=rs("goods_used_sw")%>&nbsp;</td>
                        <td><a href="#" onClick="pop_Window('met_goods_code_add.asp?goods_code=<%=rs("goods_code")%>&goods_name=<%=rs("goods_name")%>&goods_type=<%=rs("goods_type")%>&u_type=<%="U"%>','met_goods_code_pop','scrollbars=yes,width=750,height=500')">수정</a>&nbsp;</td>
			          </tr>
				      <%
							rs.movenext()
						loop
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td width="20%">
					<div class="btnCenter">
                    <a href="met_goods_code_excel.asp?goods_type=<%=goods_type%>&view_c=<%=view_c%>&field_check=<%=field_check%>&field_gubun=<%=field_gubun%>&field_grade=<%=field_grade%>&field_group=<%=field_group%>&field_name=<%=field_name%>&field_code=<%=field_code%>&field_stand=<%=field_stand%>" class="btnType04">엑셀다운로드</a>
					</div>
                  	</td>
				    <td>
                    <div id="paging">
                        <a href="met_goods_code_mg.asp?page=<%=first_page%>&goods_type=<%=goods_type%>&view_c=<%=view_c%>&field_check=<%=field_check%>&field_gubun=<%=field_gubun%>&field_grade=<%=field_grade%>&field_group=<%=field_group%>&field_name=<%=field_name%>&field_code=<%=field_code%>&field_stand=<%=field_stand%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="met_goods_code_mg.asp?page=<%=intstart -1%>&goods_type=<%=goods_type%>&view_c=<%=view_c%>&field_check=<%=field_check%>&field_gubun=<%=field_gubun%>&field_grade=<%=field_grade%>&field_group=<%=field_group%>&field_name=<%=field_name%>&field_code=<%=field_code%>&field_stand=<%=field_stand%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
   	        <% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="met_goods_code_mg.asp?page=<%=i%>&goods_type=<%=goods_type%>&view_c=<%=view_c%>&field_check=<%=field_check%>&field_gubun=<%=field_gubun%>&field_grade=<%=field_grade%>&field_group=<%=field_group%>&field_name=<%=field_name%>&field_code=<%=field_code%>&field_stand=<%=field_stand%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
   	        <% if 	intend < total_page then %>
                        <a href="met_goods_code_mg.asp?page=<%=intend+1%>&goods_type=<%=goods_type%>&view_c=<%=view_c%>&field_check=<%=field_check%>&field_gubun=<%=field_gubun%>&field_grade=<%=field_grade%>&field_group=<%=field_group%>&field_name=<%=field_name%>&field_code=<%=field_code%>&field_stand=<%=field_stand%>&ck_sw=<%="y"%>">[다음]</a> <a href="met_goods_code_mg.asp?page=<%=total_page%>&goods_type=<%=goods_type%>&view_c=<%=view_c%>&field_check=<%=field_check%>&field_gubun=<%=field_gubun%>&field_grade=<%=field_grade%>&field_group=<%=field_group%>&field_name=<%=field_name%>&field_code=<%=field_code%>&field_stand=<%=field_stand%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
				    ...
				    <td width="20%">
					<div class="btnCenter">
                    <a href="#" onClick="pop_Window('met_goods_code_add.asp?goods_type=<%=goods_type%>&u_type=<%=""%>','met_goods_code_pop','scrollbars=yes,width=750,height=500')" class="btnType04">신규 품목 등록</a>
					</div>
                    </td>
			      </tr>
				  </table>
