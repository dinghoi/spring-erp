...
objBuilder.Append "		OR as_type = '이전설치공사' OR as_type = '랜공사', cnt, 0)) AS 'setting', "
objBuilder.Append "	SUM(IF(as_type = '예방점검', cnt, 0)) AS 'testing', "
objBuilder.Append "	SUM(IF(as_type = '장비회수', cnt, 0)) AS 'collect', "
objBuilder.Append	" SUM(cnt) AS as_cnt "
objBuilder.Append "FROM ( "
objBuilder.Append "	SELECT asat.company, trat.saupbu AS bonbu, as_type, COUNT(*) AS cnt "
			...
			<div id="container">
				...
				<div class="gView">
					...
					<tr>
						...
						<tbody>
						<%
						Dim i, fault_sum, setting_sum, testing_sum, collect_sum, as_cnt_sum
						...
						Do Until rsComCost.EOF
							...
							as_cnt_sum = as_cnt_sum + CLng(rsComCost("as_cnt"))
						%>
							<tr>
								<td class="first"><%=i%></td>
								...
								<td class="right"><%=FormatNumber(rsComCost("as_cnt"), 0)%>&nbsp;</td>
								<td>&nbsp;</td>
							</tr>
						<%
							rsComCost.MoveNext()
						...
						%>
							<tr>
								...
								<td colspan="2" bgcolor="#FFE8E8" class="right"><%=FormatNumber(as_cnt_sum, 0)%>&nbsp;건</td>
