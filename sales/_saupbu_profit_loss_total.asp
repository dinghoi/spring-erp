...
cost_year = Request.Form("cost_year")	'조회 년도
...
'	saupbu_tab(i) = "솔루션사업부"
	...
	Set rsCostStats = Server.CreateObject("ADODB.RecordSet")

	rsCostStats.Open objBuilder.ToString(), DBConn, 1
	objBuilder.Clear()
...
'objBuilder.Append "eomt.org_bonbu, "
...
Set rsSaleStats = Server.CreateObject("ADODB.RecordSet")
rsSaleStats.Open objBuilder.ToString(), DBConn, 1
objBuilder.Clear()
...
Set rsProfitStats = Server.CreateObject("ADODB.RecordSet")
rsProfitStats.Open objBuilder.ToString(), DBConn, 1
objBuilder.Clear()
...
Set rsEtcStats = Server.CreateObject("ADODB.RecordSet")
rsEtcStats.Open objBuilder.ToString(), Dbconn, 1
objBuilder.Clear()
	...
	<body>
		<div id="wrap">
			...
			<div id="container">
				...
				<form action="/sales/saupbu_profit_loss_total.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>
						<dt>조건 검색</dt>
							...
							<tr>
							  <td style="border-left:1px solid #e3e3e3;">비용</td>
								...
								<%If(k < 13 And sum_amt(i, 2, k) > 0) And (saupbu_tab(i) <> "회사간거래") Then %>
									<a href="#" onClick="pop_Window('saupbu_profit_loss_report2.asp?cost_year=<%=cost_year%>&cost_mm=<%=k%>&sales_saupbu=<%=saupbu_tab(i)%>','saupbu_profit_loss_report_pop','scrollbars=yes,width=1230,height=650')">
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td width="25%">
					<div class="btnCenter">
                    <a href="saupbu_profit_loss_total_excel.asp?cost_year=<%=cost_year%>" class="btnType04">엑셀다운로드</a>
					</div>
                    </td>
				    <td width="50%">
                    </td>
