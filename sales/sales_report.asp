...
'===================================================
Dim ck_sw, page
Dim sales_month, field_check, field_view
Dim sales_yymm, cost_year
Dim start_page, pgsize, stpage
Dim total_page
Dim where_sql, order_sql, field_sql
Dim sum_price, sum_cost, sum_cost_vat
Dim rsCount, total_record
Dim rs_sum, rs, title_line, sales_saupbu

Dim be_pg, pg_url, from_date, to_date, work_month

be_pg = "/sales/sales_report.asp"
...
'sales_month = f_Request("sales_month")
...
'pg_url = "&sales_month="&sales_month&"&sales_saupbu="&sales_saupbu&"&field_check="&field_check&"&field_view="&field_view
...
If field_check = "total" Then
	...
	field_sql = ""
Else
	field_sql = " AND (sst."&field_check&" LIKE '%"&field_view&"%') "
End If
...
If sales_saupbu = "전체" Then
	...
	ElseIf empProfitViewSI = "Y" Then
		where_sql = "AND sst.saupbu IN ('SI1본부', 'SI2본부') "
	ElseIf empProfitViewNI = "Y" Then
		where_sql = "AND sst.saupbu IN ('NI본부', 'ICT본부') "
	Else
		where_sql = "AND sst.saupbu = '"&bonbu&"' "
	End If
Else
	where_sql = "AND sst.saupbu = '"&sales_saupbu&"' "
End If

order_sql = " ORDER BY sst.sales_date ASC "
	...
	<body>
		<div id="wrap">
			...
			<div id="container">
				...
				<form action="/sales/sales_report.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>
						<dt>조회조건</dt>
                        <dd>
                            <p>
								...
								If empProfitViewAll = "Y" Then
									'기타사업부 제외
									'objBuilder.Append "	AND saupbu IN (SELECT saupbu FROM sales_org WHERE sales_year >= '"&cost_year&"' AND sort_seq <> '9' GROUP BY saupbu) "
									objBuilder.Append "	AND saupbu IN (SELECT saupbu FROM sales_org WHERE sales_year >= '"&cost_year&"' GROUP BY saupbu) "
								ElseIf empProfitViewSI = "Y" Then
									objBuilder.Append "	AND saupbu IN ('SI1본부', 'SI2본부') "
								ElseIf empProfitViewNI = "Y" Then
									objBuilder.Append "	AND saupbu IN ('NI본부', 'ICT본부') "
								Else
								...
								<strong>세부조건</strong>
									<select name="field_check" id="field_check" style="width:100px">
										...
										<option value="sales_memo" <%If field_check = "sales_memo" Then %>selected<%End If %>>품목명</option>
									</select>
								</label>
                                <label>
