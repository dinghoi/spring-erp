...
sql = sql + " where "+com_sql0+" (Cast(acpt_date as date) >= '" + from_date + "' AND Cast(acpt_date as date) <= '"+to_date+"') and (as_process = '대체' or as_process = '완료' or as_process = '취소') "
...
'현재까지 미처리
sql = "select as_type, count(*) as end_cnt from as_acpt"
sql = sql + " where "+com_sql0+" (Cast(acpt_date as date) <= '"+to_date+"') and (as_process = '접수' or as_process = '입고' or as_process = '연기') "
sql = sql + " GROUP BY as_type Order By as_type Asc"
Rs.Open Sql, Dbconn, 1 
...
'기간내입고건
sql = "select as_type, count(*) as end_cnt from as_acpt"
sql = sql + " where "+com_sql0+" (Cast(acpt_date as date) >= '" + from_date + "' AND Cast(acpt_date as date) <= '"+to_date+"') and (as_process = '입고') "
sql = sql + " GROUP BY as_type Order By as_type Asc"
Rs.Open Sql, Dbconn, 1 
...
'현재까지 입고건
sql = "select as_type, count(*) as end_cnt from as_acpt"
sql = sql + " where "+com_sql0+" (as_process = '입고') "
sql = sql + " GROUP BY as_type Order By as_type Asc"
Rs.Open Sql, Dbconn, 1 
...
sql = sql + " WHERE "+com_sql+type_sql+" (as_acpt.as_process = '대체' or as_acpt.as_process = '완료' or as_acpt.as_process = '취소')"
...
sql = sql + " WHERE "+com_sql+type_sql+" (as_acpt.as_process = '접수' or as_acpt.as_process = '입고' or as_acpt.as_process = '연기')"
