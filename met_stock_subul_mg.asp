			...
			<div id="container">
				  ...
				  <table cellpadding="0" cellspacing="0" class="tableList">
				    ...
				    <thead>
				      <tr>
                        ...
                        <th colspan="2" scope="col" style=" border-bottom:1px solid #e3e3e3;">기초</th>
                        ...
                        <th colspan="2" scope="col" style=" border-bottom:1px solid #e3e3e3;">기말</th>
			          </tr>
                      <tr>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td width="20%">
					<div class="btnCenter">
                    <a href="met_stock_subul_excel.asp?view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&owner_view=<%=owner_view%>&condi=<%=condi%>&stock=<%=stock%>" class="btnType04">엑셀다운로드</a>
					</div>                  
                  	</td>
				    <td>
                    <div id="paging">
                        <a href="met_stock_subul_mg.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&owner_view=<%=owner_view%>&condi=<%=condi%>&stock=<%=stock%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="met_stock_subul_mg.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&owner_view=<%=owner_view%>&condi=<%=condi%>&stock=<%=stock%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
   	        ...
   	        <% if i = int(page) then %>
                      ...
                      <% next %>
   	        <% if 	intend < total_page then %>
                        <a href="met_stock_subul_mg.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&owner_view=<%=owner_view%>&condi=<%=condi%>&stock=<%=stock%>&ck_sw=<%="y"%>">[다음]</a> <a href="met_stock_subul_mg.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&owner_view=<%=owner_view%>&condi=<%=condi%>&stock=<%=stock%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
				    ...
				    <td width="20%">
					<div class="btnCenter">
                    <a href="#" onClick="pop_Window('met_stock_subul_print.asp?view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&owner_view=<%=owner_view%>&condi=<%=condi%>&stock=<%=stock%>','met_buy_order_print_pop','scrollbars=yes,width=1250,height=600')" class="btnType04">품목별 수불현황 출력</a>
					</div>                  
                    </td>
			      </tr>
				  </table>
