...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function delcheck() 
				...
				if (a==true) {
					document.frm.action = "parcel_cost_del_ok.asp";
					document.frm.submit();
				return true;
				}
				return false;
	...
	<body>
			<div id="container">
				...
				<form action="parcel_cost_add_save.asp" method="post" name="frm">
				<div class="gView">
				  <table cellpadding="0" cellspacing="0" class="tableWrite">
                    ...
                    <% end if	%>
				      ...
				      <tr>
				        ...
				        <th>운반비구분</th>
				        <td class="left">
                        <select name="account" id="account" style="width:100px">
                        	<option value="">선택</option>
							<option value='택배비' <%If account= "택배비" then %>selected<% end if %>>택배비</option>
							<option value='용달비' <%If account= "용달비" then %>selected<% end if %>>용달비</option>
							<option value='퀵서비스' <%If account= "퀵서비스" then %>selected<% end if %>>퀵서비스</option>
						</select>
                        </td>
			          </tr>
				      <tr>
