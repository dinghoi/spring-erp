...
be_pg = "met_goods_pum_jaego_mg.asp"
...
order_Sql = " ORDER BY stock_goods_code,stock_goods_gubun,stock_goods_name,stock_goods_standard ASC"
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="met_goods_pum_jaego_mg.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>검색조건</dt>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td width="20%">
					<div class="btnCenter">
                    <a href="met_goods_pum_jaego_excel.asp?view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&field_check=<%=field_check%>&field_view=<%=field_view%>" class="btnType04">엑셀다운로드</a>
					</div>                  
                  	</td>
				    <td>
                    <div id="paging">
                        <a href="met_goods_pum_jaego_mg.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="met_goods_pum_jaego_mg.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
   	        <% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="met_goods_pum_jaego_mg.asp?page=<%=i%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
   	        <% if 	intend < total_page then %>
                        <a href="met_goods_pum_jaego_mg.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[다음]</a> <a href="met_goods_pum_jaego_mg.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
				    ...
				    <td width="20%">
					<div class="btnCenter">
                    <a href="#" onClick="pop_Window('met_goods_pum_jaego_print.asp?view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&field_check=<%=field_check%>&field_view=<%=field_view%>&u_type=<%=""%>','met_goods_jaego_popup','scrollbars=yes,width=1250,height=600')" class="btnType04">재고현황 출력</a>
					</div>                  
                    </td>
			      </tr>
				  </table>
