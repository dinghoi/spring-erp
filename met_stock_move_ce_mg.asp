...
be_pg = "met_stock_move_ce_mg.asp"
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="met_stock_move_ce_mg.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>검색 조건</dt>
				...
				<div class="gView">
                  ...
                  <% if  rs("stock_in_qty") > 0 then    %>                        
                        <a href="#" onClick="pop_Window('met_pummok_mvin_detail.asp?stock_goods_code=<%=rs("stock_goods_code")%>&stock_goods_type=<%=rs("stock_goods_type")%>&stock_code=<%=rs("stock_code")%>&stock_name=<%=rs("stock_name")%>','met_pummok_stin_detail_pop','scrollbars=yes,width=930,height=650')"><%=formatnumber(rs("stock_in_qty"),0)%>&nbsp;</a>
                  <%    else   %>
                 ...
                 <% if stock_jj_qty > 0 and user_id = "900002" then    %>  
                        <a href="#" onClick="pop_Window('met_chulgo_move_add02.asp?view_condi=<%=view_condi%>&stock_code=<%=stock_code%>&stock_goods_type=<%=stock_goods_type%>&stock_goods_code=<%=stock_goods_code%>&u_type=<%=""%>','met_chulgo_move_add02_pop','scrollbars=yes,width=1230,height=650')">CE</a>
                 <%    else     %>
                        -
                 <%  end if     %>  
                        </td>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td>
                    <div id="paging">
                        <a href="met_stock_move_ce_mg.asp?page=<%=first_page%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&owner_view=<%=owner_view%>&condi=<%=condi%>&stock=<%=stock%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="met_stock_move_ce_mg.asp?page=<%=intstart -1%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&owner_view=<%=owner_view%>&condi=<%=condi%>&stock=<%=stock%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
   	        <% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="met_stock_move_ce_mg.asp?page=<%=i%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&owner_view=<%=owner_view%>&condi=<%=condi%>&stock=<%=stock%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
   	        <% if 	intend < total_page then %>
                        <a href="met_stock_move_ce_mg.asp?page=<%=intend+1%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&owner_view=<%=owner_view%>&condi=<%=condi%>&stock=<%=stock%>&ck_sw=<%="y"%>">[다음]</a> <a href="met_stock_move_ce_mg.asp?page=<%=total_page%>&view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&owner_view=<%=owner_view%>&condi=<%=condi%>&stock=<%=stock%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
                 ...
                 <% if user_id = "900002" then    %>  
                    <a href="#" onClick="pop_Window('met_chulgo_move_add01.asp?view_condi=<%=view_condi%>&goods_type=<%=goods_type%>&chulgo_id=<%=chulgo_id%>&u_type=<%=""%>','met_chulgo_move_add01_pop','scrollbars=yes,width=1230,height=650')" class="btnType04">CE출고 등록(다품목)</a>
                 <%  end if     %>                      
					</div>                  
                    </td>
			      </tr>
