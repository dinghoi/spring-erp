...
'2014-01-25 기존에 설치사진 첨부 (종료)
	...
	visit_date_old = abc("visit_date_old")
	visit_time_old = abc("visit_time_old")
	saupbu = abc("emp_saupbu")
...
'	sms_yn = abc("sms_yn")
	...
	into_reason = Replace(into_reason,"'","&quot;")
	maker = abc("maker")
	...
	as_history = Replace(as_history,"'","&quot;")
	if as_type = "방문처리" or as_type = "원격처리" then
		dev_inst_cnt = 1
		ran_cnt = 0
		work_man_cnt = 1
		alba_cnt = 0
...
'	end if
	...
	if as_process = "완료" then
		week_day = WeekDay(visit_date)
		if week_day = 1 or week_day = 7 then
			holiday_yn = "Y"
		  else
			sql = "select * from nkp.holiday where holiday = '"&visit_date&"'"
			set rs_hol = dbconn.execute(sql)
	...
	cowork_yn = abc("cowork_yn")
	cowork_reside_place = abc("cowork_reside_place")
	cowork_reside_company = abc("cowork_reside_company")

	if cowork_yn = "Y" then
		reside_company = abc("cowork_reside_company")
		reside_place = abc("cowork_reside_place")
	else
	end if

	as_unit = abc("as_unit")
	night = abc("night")
	weekend_work = abc("weekend_work")
...
'2014-07-12폴더 생성
	...
	'개발 테스트 경로
	'path_nm = "G:\workspace\web\att_file\" + company
...
'2014-01-25 기존에 설치사진 첨부 (종료)
	...
	as_unitprice = ""
if as_process = "완료" then
	if as_unit = "AU07" or as_unit = "AU08" then
		sql = "select * from nkp.as_unitprice_month where au_code='" & as_unit & "'"
		'Response.write sql
		set rs_unitprice = DbConn.Execute(sql)

		if night = "Y" or weekend_work = "Y" Then
			as_unitprice = rs_unitprice("as_unitprice2")
			'Response.write rs("as_unitprice2")
		Else
			as_unitprice = rs_unitprice("as_unitprice1")
			'Response.write rs("as_unitprice1")
	...
	if as_process = "완료" or as_process = "취소" then
		if write_date <> "" and write_cnt <> "" then
			...
			sql = "Update itft2005.as_acpt set dept ='"&dept&"', addr ='"&addr&"',mg_ce_id='"&mg_ce_id&"',mg_ce='"&mg_ce&"',as_memo='"&as_memo& _
			 "',cowork_yn='"&cowork_yn&"',request_date='"&request_date&"',request_time='"&request_time&"',visit_date='"&visit_date&"',visit_time='"&visit_time& _
		...
		sql = "Update nkp.as_acpt set acpt_user ='"&acpt_user&"', user_grade ='"&user_grade&"',dept ='"&dept&"', addr ='"&addr&"',mg_ce_id='"&mg_ce_id&"',mg_ce='"&mg_ce&"',as_memo='"&as_memo& _
		"',cowork_yn='"&cowork_yn&"',request_date='"&request_date&"',request_time='"&request_time&"',visit_date='"&visit_date&"',visit_time='"&visit_time& _
		...
		"',as_unit='"&as_unit&"',night='"&night&"',weekend_work='"&weekend_work&"',saupbu='"&saupbu&"',as_standard_money='"&as_unitprice& _
		...
		if as_process = "완료" and work_man_cnt < 2 then
			...
			sql="insert into nkp.ce_work (acpt_no,mg_ce_id,work_id,work_date,holiday_yn,as_type,company,emp_company,bonbu,saupbu,team,org_name"& _
			...
			"','"&mg_ce_id&"','2','"&visit_date&"','"&holiday_yn&"','"&as_type&"','"&company&"','"&rs("emp_company")&"','"&rs("bonbu")& _
			"','"&rs("saupbu")&"','"&team&"','"&org_name&"','"&reside_place&"','"&reside&"','"&reside_company&"',"&work_man_cnt& _
			","&dev_inst_cnt&","&ran_cnt&","&alba_cnt&","&person_amt&",'"&user_id&"',now())"
			dbconn.execute(sql)
		  else
			sql = "Update nkp.ce_work set work_date ='"&visit_date&"', holiday_yn ='"&holiday_yn&"' where work_id = '2' and acpt_no = "&int(acpt_no)
			dbconn.execute(sql)
		end if
	...
	if as_process = "입고" then
		if write_date <> "" and write_cnt <> "" then
			...
			sql = "Update itft2005.as_acpt set into_reason ='"&into_reason&"', as_process ='"&as_process&"',as_history='"&as_history& _
		...
		sql="insert into nkp.ce_work (acpt_no,mg_ce_id,work_id,work_date,holiday_yn,as_type,company,emp_company,bonbu,saupbu,team,org_name"& _
		",reside_place,reside,reside_company,work_man_cnt,dev_inst_cnt,ran_cnt,alba_cnt,person_amt,reg_id,reg_date) values ('"&acpt_no& _
		"','"&mg_ce_id&"','3','"&in_date&"','"&holiday_yn&"','"&as_type&"','"&company&"','"&rs("emp_company")&"','"&rs("bonbu")& _
		"','"&rs("saupbu")&"','"&team&"','"&org_name&"','"&reside_place&"','"&reside&"','"&reside_company&"',"&work_man_cnt&","&dev_inst_cnt& _
		","&ran_cnt&","&alba_cnt&","&person_amt&",'"&user_id&"',now())"
		dbconn.execute(sql)
	end if
	if as_process = "접수" or as_process = "연기" then
		sql = "Update nkp.as_acpt set acpt_user ='"&acpt_user&"', user_grade ='"&user_grade&"',dept ='"&dept&"', addr ='"&addr&"',mg_ce_id='"&mg_ce_id&"',mg_ce='"&mg_ce&"',as_memo='"&as_memo& _
		"',cowork_yn='"&cowork_yn&"',request_date='"&request_date&"',request_time='"&request_time&"',into_reason='"&into_reason&"',as_process='"&as_process& _
		"',as_type='"&as_type&"',maker='"&maker&"',as_device='"&as_device&"',asets_no='"&asets_no &"',serial_no='"&serial_no& _
...
' 변경 History 저장
	...
	if be_pg = "as_list.asp" then
	  ...
	  elseif be_pg = "as_list_reside.asp" then
		url = "as_list_reside.asp?page="&page&"&from_date="&from_date&"&to_date="&to_date&"&date_sw="&date_sw&"&process_sw="&process_sw&"&field_check="&field_check&"&field_view="&field_view&"&ck_sw="&check_sw&"&company="&condi_com
	  else
		url = "as_list_ce.asp?page="&page&"&view_sort="&view_sort&"&view_c="&view_c&"&ck_sw="&check_sw
	end if

	if ((visit_date <> visit_date_old) or (visit_time <> visit_time_old)) and (as_process = "완료") then
		sql = "insert into nkp.as_mod_visit_datetime (acpt_no,mod_date,visit_date_old,visit_time_old,visit_date,visit_time) "&_
		      " values ('"&acpt_no&"',now(),'"&visit_date_old&"','"&visit_time_old&"','"&visit_date&"','"&visit_time&"')"
		dbconn.execute(sql)
	end if
	...
	response.write"<script language=javascript>"
'	response.write"alert('"&as_unitprice&"');"
