...
<html>
	<head>
		...
		<script type="text/javascript">
			function alba_list(draft_no,draft_man,draft_tax_id,draft_date,sign_no,bank_name,account_no,account_name)
			{
	...
	<body>
		<div id="container">
				...
				<form action="alba_person_search.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
                        <dd>
				...
				<div class="gView">
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						%>
							<tr>
								<td class="first">
                                <a href="#" onClick="alba_list('<%=draft_no%>','<%=draft_man%>','<%=draft_tax_id%>','<%=draft_date%>','<%=sign_no%>','<%=bank_name%>','<%=account_no%>','<%=account_name%>');"><%=rs("draft_man")%>(<%=rs("draft_no")%>)</a>
                                </td>
						...
						%>
                            ...
                            <tr>
								<td colspan="5">
					            <div class="btnRight">
					            <a href="#" onClick="pop_Window('alba_person_add.asp','insa_alba_add_pop','scrollbars=yes,width=750,height=400')" class="btnType04">아르바이트 등록</a>
					           </div>  
                               </td>
							</tr>
