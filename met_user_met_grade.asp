...
<html>
	<head>
		...
		<script type="text/javascript">		
            ...
            document.frm.action = "met_user_met_grade_ok.asp";
            document.frm.submit();
            }	
		</script>
	...
	<body>
			<div id="container">
				...
				<form action="met_user_met_grade.asp?ck_sw=<%="n"%>" method="post" name="frm">
                <fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
                        <dd>
				...
				<div class="gView">
					<table cellpadding="0" cellspacing="0" class="tableWrite">
						...
						<tbody>
                    <% 
					        user_empno = rs("user_id")
						   
						    m_grade = rs("met_grade")
							if m_grade = "0" then
							    grade_name =  "마스터권한"
							end if
							if m_grade = "1" then
					...
					%>	
							...
							<tr>
								...
								<td class="left"><%=rs("met_grade")%>&nbsp;(<%=grade_name%></td>
							</tr>
                            <tr>
								<th class="first">변경할 권한</th>
								<td class="left">
								<input name="met_grade" type="text" id="met_grade" style="width:30px" value="<%=m_grade%>">
                                &nbsp;&nbsp;( 0: 마스터, 1: 일반, 2: NW사업부 마스터 )
                                </td> 
							</tr>
						</tbody>
					</table>
