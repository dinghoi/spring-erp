...
'2014-01-25 기존에 설치사진 첨부 (종료)
	...
	for i = 1 to 20	
		...
		buy_tab(i) = qty_tab(i) * buy_cost(i)
	next
	...
	if	u_type = "U" then
	        ...
	        buy_seq = "00"
			sql="select max(buy_no) as max_no from met_buy where buy_date = '"&buy_date&"'"
	...
	for i = 1 to 20
		if code_tab(i) = "" or isnull(code_tab(i)) then
			...
			bg_seq = right(("00" + cstr(i)),2)
		  	sql="insert into met_buy_goods (bg_no,buy_seq,bg_date,bg_seq,bg_goods_code,bg_goods_type,bg_goods_gubun,bg_goods_name,bg_unit,bg_standard,bg_qty,bg_unit_cost,bg_buy_amt,bg_ing,reg_date,reg_user) values ('"&buy_no&"','"&buy_seq&"','"&buy_date&"','"&bg_seq&"','"&code_tab(i)&"','"&goods_type(i)&"','"&goods_gubun(i)&"','"&goods_name(i)&"','','"&goods_standard(i)&"','"&qty_tab(i)&"','"&buy_cost(i)&"','"&buy_tab(i)&"','0',now(),'"&user_name&"')"
			dbconn.execute(sql)
		end if
	next
...
'	response.write"location.replace('met_buy_ing_mg.asp');"
