...
' 전월 금액 전체 SUM 처리
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td>
                    <div id="paging">
                        <a href="ceo_card_person_mg.asp?page=<%=first_page%>&slip_month=<%=slip_month%>&emp_yn=<%=emp_yn%>&emp_name=<%=emp_name%>&sort_condi=<%=sort_condi%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="ceo_card_person_mg.asp?page=<%=intstart -1%>&slip_month=<%=slip_month%>&emp_yn=<%=emp_yn%>&emp_name=<%=emp_name%>&sort_condi=<%=sort_condi%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
<% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="ceo_card_person_mg.asp?page=<%=i%>&slip_month=<%=slip_month%>&emp_yn=<%=emp_yn%>&emp_name=<%=emp_name%>&sort_condi=<%=sort_condi%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
<% if 	intend < total_page then %>
                        <a href="ceo_card_person_mg.asp?page=<%=intend+1%>&slip_month=<%=slip_month%>&emp_yn=<%=emp_yn%>&emp_name=<%=emp_name%>&sort_condi=<%=sort_condi%>&ck_sw=<%="y"%>">[다음]</a> 
                        <a href="ceo_card_person_mg.asp?page=<%=total_page%>&slip_month=<%=slip_month%>&emp_yn=<%=emp_yn%>&emp_name=<%=emp_name%>&sort_condi=<%=sort_condi%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
