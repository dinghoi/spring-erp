...
'	to_date = cstr(dateadd("d",-1,to_date))
	...
	if field_check = "total" then
	sql = "select * from saupbu_sales where  (sales_date >= '" + from_date  + "' and sales_date <= '" + to_date  + "') "
  else
	sql = "select * from saupbu_sales where  (sales_date >= '" + from_date  + "' and sales_date <= '" + to_date  + "') and (" + field_check + " like '%" + field_view + "%' ) "
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="sales_report2.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>조회조건</dt>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    <td width="24%">
					<div class="btnCenter">
                    <a href="sales_report_excel.asp?sales_date=<%=sales_date%>&sales_saupbu=<%=sales_saupbu%>&field_check=<%=field_check%>&field_view=<%=field_view%>" class="btnType04">엑셀다운로드</a>
					</div>                  
                  	</td>
				    <td>
                    <div id="paging">
                        <a href="sales_report2.asp?page=<%=first_page%>&sales_date=<%=sales_date%>&sales_saupbu=<%=sales_saupbu%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="sales_report2.asp?page=<%=intstart -1%>&sales_date=<%=sales_date%>&sales_saupbu=<%=sales_saupbu%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
<% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="sales_report2.asp?page=<%=i%>&sales_date=<%=sales_date%>&sales_saupbu=<%=sales_saupbu%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
<% if 	intend < total_page then %>
                        <a href="sales_report2.asp?page=<%=intend+1%>&sales_date=<%=sales_date%>&sales_saupbu=<%=sales_saupbu%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[다음]</a> 
                        <a href="sales_report2.asp?page=<%=total_page%>&sales_date=<%=sales_date%>&sales_saupbu=<%=sales_saupbu%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
