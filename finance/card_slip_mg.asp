	...
	<body>
		<div id="wrap">
			...
			<div id="container">
				...
				<form action="/finance/card_slip_mg.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>
						<dt>조건 검색</dt>
                        <dd>
							<p>
                                ...
                                <select name="owner_company" id="owner_company" style="width:120px">
									...
									objBuilder.Append "SELECT org_company FROM emp_org_mst "
						...
						%>
							<tr>
                                ...
                                <% If rsCard("end_sw") = "Y" Then %>
                                    마감
                                <% Else %>
                                    <a href="#" onClick="pop_Window('/finance/card_slip_mod.asp?approve_no=<%=rsCard("approve_no")%>&cancel_yn=<%=rsCard("cancel_yn")%>','카드전표수정','scrollbars=yes,width=800,height=300')">수정</a>
                                <% End If %>
                                </td>
							</tr>
						<%
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
					...
					<a href = "/finance/card_slip_excel.asp?slip_month=<%=slip_month%>&owner_company=<%=owner_company%>&card_type=<%=card_type%>&field_check=<%=field_check%>&field_view=<%=field_view%>" class="btnType04">내역다운</a>
					<% If end_count = 0 Then %>
				    ...
				    <td>
                    <div id="paging">
                        <a href="/finance/card_slip_mg.asp?page=<%=first_page%>&slip_month=<%=slip_month%>&owner_company=<%=owner_company%>&card_type=<%=card_type%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[처음]</a>
                  		<% If intstart > 1 Then %>
                            <a href="/finance/card_slip_mg.asp?page=<%=intstart -1%>&slip_month=<%=slip_month%>&owner_company=<%=owner_company%>&card_type=<%=card_type%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[이전]</a>
                        <% End If %>
                        <% For i = intstart To intend %>
                            <% If i = int(page) Then %>
                                <b>[<%=i%>]</b>
                            <% Else %>
                                <a href="/finance/card_slip_mg.asp?page=<%=i%>&slip_month=<%=slip_month%>&owner_company=<%=owner_company%>&card_type=<%=card_type%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                            <% End If %>
                        <% Next %>
						<% If intend < total_page Then %>
                            <a href="/finance/card_slip_mg.asp?page=<%=intend+1%>&slip_month=<%=slip_month%>&owner_company=<%=owner_company%>&card_type=<%=card_type%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[다음]</a>
                            <a href="/finance/card_slip_mg.asp?page=<%=total_page%>&slip_month=<%=slip_month%>&owner_company=<%=owner_company%>&card_type=<%=card_type%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	Else %>
                            [다음]&nbsp;[마지막]
                        <% End If %>
                    </div>
