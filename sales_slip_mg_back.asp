...
date_sql = "where (" + view_date + " >='" + from_date  + "' and " + view_date + " <= '" + to_date  + "') "
	...
	<body>
		<div id="wrap">			
			...
			<div id="container">
				...
				<form action="sales_slip_mg.asp" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>조건검색</dt>
                        <dd>
                            <p>
                                ...
                                <select name="slip_id" id="slip_id" style="width:80px">
                                    ...
                                    <option value="O" <% if slip_id = "O" then %>selected<% end if %>>수주전표</option>
                                    <option value="W" <% if slip_id = "W" then %>selected<% end if %>>대기전표</option>
                                </select>
				...
				<div class="gView">
					<table cellpadding="0" cellspacing="0" class="tableList">
						...
						do until rs.eof
							if rs("slip_id") = "O" then
								slip_id_view = "수주전표"
							end if
							if rs("slip_id") = "W" then
								slip_id_view = "대기전표"
							end if
						%>
							<tr>
								...
								<td><%=rs("collect_date")%></td>
							  	<td>
                                <a href="#" onClick="pop_Window('sales_slip_add.asp?slip_no=<%=rs("slip_no")%>&u_type=<%="U"%>','general_cost_add_pop','scrollbars=yes,width=1230,height=600')">수정</a>
                                </td>
								<td>삭제</td>
							</tr>
						<%
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
				    ...
				    <td>
                    <div id="paging">
                        <a href="sales_slip_mg.asp?page=<%=first_page%>&from_date=<%=from_date%>&to_date=<%=to_date%>&slip_id=<%=slip_id%>&view_date=<%=view_date%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[처음]</a>
                  	<% if intstart > 1 then %>
                        <a href="sales_slip_mg.asp?page=<%=intstart -1%>&from_date=<%=from_date%>&to_date=<%=to_date%>&slip_id=<%=slip_id%>&view_date=<%=view_date%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[이전]</a>
                      <% end if %>
                      <% for i = intstart to intend %>
                  	<% if i = int(page) then %>
                        <b>[<%=i%>]</b>
                      <% else %>
                        <a href="sales_slip_mg.asp?page=<%=i%>&from_date=<%=from_date%>&to_date=<%=to_date%>&slip_id=<%=slip_id%>&view_date=<%=view_date%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[<%=i%>]</a>
                      <% end if %>
                      <% next %>
                  	<% if 	intend < total_page then %>
                        <a href="sales_slip_mg.asp?page=<%=intend+1%>&from_date=<%=from_date%>&to_date=<%=to_date%>&slip_id=<%=slip_id%>&view_date=<%=view_date%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[다음]</a> 
                        <a href="sales_slip_mg.asp?page=<%=total_page%>&from_date=<%=from_date%>&to_date=<%=to_date%>&slip_id=<%=slip_id%>&view_date=<%=view_date%>&field_check=<%=field_check%>&field_view=<%=field_view%>&ck_sw=<%="y"%>">[마지막]</a>
                        <%	else %>
                        [다음]&nbsp;[마지막]
                      <% end if %>
                    </div>
				    ...
				    <td width="15%">
					<div class="btnCenter">
					<a href="#" onClick="pop_Window('sales_slip_add.asp','sales_slip_add_pop','scrollbars=yes,width=1230,height=600')" class="btnType04">영업전표등록</a>
                    </div>                  
                    </td>
			      </tr>
				  </table>
