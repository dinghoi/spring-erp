...
if	mg_ce = "" then
  ...
  elseif sido = "강원지사" then 
    ...
    sql = sql + " and (Cast(acpt_date as date) >= '" + from_date + "' AND Cast(acpt_date as date) <= '"+to_date+"') and sido in ('강원')"
		...
		sql = sql + "  and (Cast(acpt_date as date) >= '" + from_date + "' AND Cast(acpt_date as date) <= '"+to_date+"') Order By acpt_date Asc"
	end if
  else
	sql = "select * from as_acpt"
					...
					<table cellpadding="0" cellspacing="0" summary="" class="tableView">
						...
						<tbody>
							<tr>
							  ...
							  <td><a href = "as_michulri_excel.asp?from_date=<%=from_date%>&to_date=<%=to_date%>&sido=<%=sido%>&company=<%=company%>&as_type=<%=as_type%>&mg_ce=<%=mg_ce%>&mg_ce_id=<%=mg_ce_id%>&mg_group=<%=mg_group%>" class="btnType04">엑셀다운로드</a>
							  </td>
					      	</tr>
						</tbody>
					</table>
