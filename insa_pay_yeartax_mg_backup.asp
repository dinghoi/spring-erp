...
if ck_sw = "n" then
	...
	y_householder = request("y_householder")
    y_foreign = request("y_foreign")
    y_disab = request("y_disab")
    y_woman = request("y_woman")
    y_single = request("y_single")
    y_blue = request("y_blue")
end if
...
Sql = "select * from pay_month_give where (pmg_id = '1') and (pmg_emp_no = '"&emp_no&"') and (pmg_company = '"&emp_company&"')"
...
Rs_give.close()
'상여금
Sql = "select * from pay_month_give where (pmg_id = '2') and (pmg_emp_no = '"&emp_no&"') and (pmg_company = '"&emp_company&"')"
Rs_give.Open Sql, Dbconn, 1
Set Rs_give = DbConn.Execute(SQL)
do until Rs_give.eof
       pmg_yymm = Rs_give("pmg_yymm")
	...
	<body>
		<div id="wrap">			
		  ...
		  <div id="container">
				...
				<form action="insa_pay_yeartax_mg.asp?ck_sw=<%="n"%>" method="post" name="frm">
				<fieldset class="srch">
					<legend>조회영역</legend>
					<dl>					
						<dt>◈ 검색◈</dt>
                ...
                <h3 class="stit">* 부양가족 및 당해년도 소득&nbsp;&nbsp;<a href="#" class="btnType03" onClick="javascript:frmcheck();">조회</a>&nbsp;(소득금액은 이전근무지 소득과 합산해서 보여짐)</h3>
				</form>
			<form method="post" name="frm1" action="insa_pay_yeartax_mg_save.asp">
            ...
            <% '                <span class="btnType01"><input type="button" value="닫기" onclick="javascript:goAction();"></span> %>
                    ...
                    <input type="hidden" name="inc_yyyy" value="<%=inc_yyyy%>" ID="inc_yyyy">
                    ...
                    <input type="hidden" name="y_householder" value="<%=y_householder%>" ID="y_householder">
                    <input type="hidden" name="y_foreign" value="<%=y_foreign%>" ID="y_foreign">
                    <input type="hidden" name="y_disab" value="<%=y_disab%>" ID="y_disab">
                    <input type="hidden" name="y_woman" value="<%=y_woman%>" ID="y_woman">
                    <input type="hidden" name="y_single" value="<%=y_single%>" ID="y_single">
              <input type="hidden" name="y_blue" value="<%=y_blue%>" ID="y_blue">
              </form>                    
		  </div>
		</div>				
	</div>        				
