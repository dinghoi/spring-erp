...
url = "nkp_main.asp?board_gubun="&board_back&"&page="&page&"&condi="&condi&"&condi_value="&condi_value&"&ck_sw=y"					
...
//			   history.back() ;
						...
						<tbody>
							...
							<tr>
								...
								<% if not rs_a.eof Then %> 
									<a href="board_view.asp?board_seq=<%=rs_a("board_seq")%>&board_gubun=<%=rs_a("board_gubun")%>&page=<%=page%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>"><%=rs_a("board_title")%></a>
								<% Else %>
									처음 자료입니다.
								<% End If %>
                                </td>
							...
							<tr>
								...
								<% if not rs_p.eof Then %> 
									<a href="board_view.asp?board_seq=<%=rs_p("board_seq")%>&board_gubun=<%=rs_p("board_gubun")%>&page=<%=page%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>"><%=rs_p("board_title")%></a>
								<% Else %>
									처음 자료입니다.
								<% End If %>
                                </td>
	...
	<form method="post" name="frm" action="board_del.asp?page=<%=page%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>">
	<div align=center>
		<p>
		<span class="btnType04"><input type="button" value="목록" onclick="javascript:goBefore();"></span>
		<span class="btnType04"><input type="button" value="수정" onclick="pop_Window('board_write.asp?board_seq=<%=rs("board_seq")%>&page=<%=page%>&condi=<%=condi%>&condi_value=<%=condi_value%>&ck_sw=<%="y"%>&u_type=<%="U"%>','board_write_popup','scrollbars=yes,width=1250,height=600')"></span>
