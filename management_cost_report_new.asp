						...
						<dl>
							...
							<dd>
								<p>
									...
									<%
									'전사공통비 총액
									sql = "select sum(cost_amt_"&cost_mm&") as tot_cost from company_cost where cost_year ='"&cost_year&"' and cost_center = '전사공통비'"
									'Response.write sql&"<br>"
									Set rs_etc = DbConn.Execute(SQL)
									tot_cost_amt = clng(rs_etc("tot_cost")) ' (1)전사공통비 = (2) + (3)
									tot_cost_amt_h1 = round(tot_cost_amt/2,0)        ' (2)전사공통비 총액의 50%
									tot_cost_amt_h2 = tot_cost_amt - tot_cost_amt_h1 ' (3)전사공통비 총액의 50%
									%>
									전사공통비 총액 : <%=formatnumber(tot_cost_amt,0)%> = <%=formatnumber(tot_cost_amt_h1,0)%> + <%=formatnumber(tot_cost_amt_h2,0)%>
                                </p>
							</dd>
						</dl>
					</fieldset>
